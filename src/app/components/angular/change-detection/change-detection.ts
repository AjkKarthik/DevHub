...
ngAfterViewInit() { this.height.set(this.el.nativeElement.offsetHeight); }`,
  ...
  quiz: QuizQuestion[] = [
    ...
    },
    {
      ...
      options: [
        'markForCheck() checks now; detectChanges() schedules for next cycle',
        'markForCheck() schedules a re-check on the next CD cycle; detectChanges() runs CD synchronously right now',
        'They are identical — one is just an alias for the other',
        'markForCheck() only works with Default strategy; detectChanges() works with OnPush',
      ],
      answer: 1,
      explanation: 'markForCheck() is asynchronous — it tells Angular "check this component next time CD runs." detectChanges() is synchronous — it immediately walks the component subtree and updates the DOM. Use detectChanges() when you need the DOM updated before measuring layout.',
    },
    {
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'Do signals work with OnPush automatically?',
      a: 'Yes. Signals are deeply integrated with Angular\'s reactive graph. When a template reads <code>count()</code>, Angular records that this component depends on <code>count</code>. When <code>count.set()</code> or <code>count.update()</code> is called, Angular immediately marks <em>only</em> the components that read <code>count</code> as dirty — regardless of their CD strategy, and regardless of whether a zone event triggered it. <code>markForCheck()</code> is not needed for signal-based state.',
    },
    {
