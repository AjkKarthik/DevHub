...
export function usePolling(url: string) {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'A directive calls destroyRef.onDestroy(fn) to register a cleanup callback, then LATER — while the directive is still very much alive — calls the returned cancel function. Does fn ever run at all, even when the directive is eventually actually destroyed?', options: ['Yes, fn always runs on actual destruction regardless of whether cancel() was called earlier', 'No — calling the returned cancel function permanently removes fn from the destroy callback list; when the directive is later destroyed for real, fn will not run since it was already unregistered', 'fn runs immediately when cancel() is called, not at actual destruction', 'Calling cancel() throws an error if the directive has not been destroyed yet'], answer: 1, explanation: 'The function returned by onDestroy() is a full unregistration, not a "run now instead of later" trigger — calling it removes fn from DestroyRef\'s internal callback list entirely. So if you cancel a registered destroy callback while the component/directive is still alive, that callback is gone: it will NOT fire later when the component eventually is actually destroyed, since it is no longer in the list Angular iterates through at destruction time. This makes onDestroy()\'s cancel function genuinely useful for conditionally-needed cleanup — e.g. registering a cleanup for a resource that might get manually disposed of earlier, and canceling the now-redundant destroy-time cleanup to avoid a double-dispose.' },
