...
// /blog/angular-signals`,
  ...
  qna: QnaItem[] = [
    { q: 'A route is pre-rendered with SSG at build time, but its data (say, a product\'s price) changes in the backend an hour after the build. Without triggering a full rebuild, is there any way for users to see the updated price on that statically-generated page?', a: 'Not from the static HTML alone — SSG output is genuinely frozen at build time, so the served HTML shows the price as of the build until the next rebuild/redeploy. The common mitigation is NOT pure SSG but Angular\'s per-route RenderMode mixing: keep the page\'s initial shell/layout statically pre-rendered for fast first paint, but have the price-sensitive portion hydrate into a client-side fetch (or use Incremental Static Regeneration-style revalidation if the hosting platform supports it) so the number itself updates after hydration even though the surrounding HTML was generated at build time. This hybrid is exactly why Angular 19\'s per-route RenderMode exists — pure SSG and pure SSR are opposite ends of a spectrum, and most real pages want a deliberate mix rather than an all-or-nothing choice.' },
