...
// }`,
  ...
  quiz: QuizQuestion[] = [
    { q: 'A custom validator accidentally returns `undefined` instead of `null` when the control is valid (a common typo — falling off the end of a function with no explicit return). Does Angular treat this the same as returning null?', options: ['No — undefined is treated as invalid since only null explicitly signals "valid"', 'Yes — Angular treats undefined exactly the same as null for validator return values, since both are falsy and Angular checks the returned value with a loose falsy check, not a strict null comparison', 'undefined causes Angular to throw a runtime error', 'undefined marks the control as pending indefinitely'], answer: 1, explanation: 'Angular\'s form validation checks the validator\'s return value for truthiness/falsiness rather than a strict `=== null` comparison, so a validator that returns undefined (e.g. by omitting a return statement when the value is valid) behaves identically to one that explicitly returns null — both are treated as "no error." This is a forgiving detail that saves a common bug, but it is still best practice to explicitly `return null` for clarity, since relying on implicit undefined can look like an accidental incomplete implementation to a future reader even though it works correctly.' },
