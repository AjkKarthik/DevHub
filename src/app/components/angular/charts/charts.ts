...
export class ReactiveChartComponent {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you bridge Angular signals to Chart.js for reactive updates?', a: 'Use <code>effect()</code> inside <code>afterNextRender()</code>: <code>effect(() =&gt; { this.chart.data.datasets[0].data = this.chartData(); this.chart.update(); })</code>. The effect re-runs whenever the <code>chartData</code> signal changes, keeping Chart.js in sync without manual subscriptions.' },
  ];
