...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Host Directives',             route: '/angular/host-directives',    badge: 'Core',      available: true,
  ...
  { title: '@let Template Variables',     route: '/angular/let-template-vars',  badge: 'Core',      available: true,
  ...
  { title: 'Signal Effects Deep-Dive',    route: '/angular/signal-effects',     badge: 'Core',      available: true,
  ...
  { title: 'Standalone Migration',        route: '/angular/standalone-migration', badge: 'Core',    available: true,
  ...
  { title: 'Error Handling Patterns',     route: '/angular/error-handling-patterns', badge: 'Core', available: true,
    description: 'Layered Angular error handling: global ErrorHandler for logging, HTTP interceptors for API errors, and typed component signals for inline UI feedback.',
    keyPoints: ['GlobalErrorHandler catches uncaught errors — forward to Sentry/Datadog', 'HTTP interceptor: 401 redirect, 5xx toast, throwError to rethrow for components', 'Typed LoadState<T> signal — explicit idle/loading/success/error states'] },
  { title: 'Mock Service Worker (MSW)',    route: '/angular/msw',                    badge: 'Testing', available: true,
    description: 'Intercept HTTP at the network layer with MSW — full Angular HTTP stack (interceptors, error handling) runs in tests; share handlers with Storybook and dev.',
    keyPoints: ['setupServer (msw/node) for Jest/Vitest; setupWorker for browser/Storybook', 'Use provideHttpClient() — NOT HttpClientTestingModule (they conflict)', 'server.use() per-test overrides + afterEach(server.resetHandlers()) for isolation'] },
  { title: 'Accessibility (a11y)',         route: '/angular/accessibility',           badge: 'Core',    available: true,
    description: 'WCAG 2.1 AA Angular apps using CDK FocusMonitor, LiveAnnouncer, cdkTrapFocus, and semantic HTML — keyboard nav, screen readers, focus management.',
    keyPoints: ['FocusMonitor tracks keyboard vs mouse focus — show rings only for keyboard', 'LiveAnnouncer.announce() for dynamic updates — polite vs assertive', 'cdkTrapFocus confines Tab inside modals; restore focus to trigger on close'] },
  { title: 'Micro-Frontends',             route: '/angular/micro-frontends',         badge: 'Platform', available: true,
    description: 'Split large Angular apps into independently deployable pieces with Native Federation — shell + remotes, singleton sharing, dynamic remote discovery.',
    keyPoints: ['Native Federation works with Angular esbuild builder; Module Federation needs Webpack', 'shareAll({ singleton: true }) — one Angular runtime shared across all remotes', 'Fetch remote URLs at runtime from a config API — deploy remotes without touching the shell'] },
  { title: 'Angular DevTools',            route: '/angular/angular-devtools',        badge: 'Core',     available: true,
    description: 'Profile and debug Angular apps with the Chrome extension — Component Tree inspection, Profiler flame charts, and ng.* console helpers.',
    keyPoints: ['Profiler flame chart: width = duration; "why?" tooltip shows CD trigger', 'ng.getComponent($0) in console returns the component instance for a DOM element', 'Wide bars in every cycle → OnPush + signals; read "why?" before changing strategy'] },
  { title: 'Bundle Optimization',         route: '/angular/bundle-optimization',     badge: 'Platform', available: true,
    description: 'Shrink Angular bundles with source-map-explorer analysis, route lazy loading, @defer for template splitting, tree-shakeable imports, and build budgets.',
    keyPoints: ['source-map-explorer finds the culprit; measure before optimising', 'loadComponent / loadChildren — initial bundle = shell only; features load on demand', '@defer (on viewport) for below-fold components; never defer LCP elements'] },
  ...
  { title: 'Route Guards',                route: '/angular/route-guards',       badge: 'Router',    available: true,
  ...
  { title: 'Typed Forms Deep-Dive',       route: '/angular/typed-forms',        badge: 'Forms',     available: true,
  ...
  { title: 'HTTP Interceptors',           route: '/angular/http-interceptors',  badge: 'HTTP',      available: true,
