...
export class ThemeComponent {
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A component uses viewChild() combined with effect() to react to a view child becoming available, instead of implementing ngAfterViewInit. What subtle difference in TIMING exists between when ngAfterViewInit historically fired and when the effect actually runs after the view child signal resolves?',
      a: 'ngAfterViewInit fires synchronously as part of Angular\'s change detection pass, immediately after the view (and its children) finish initializing, within that same synchronous CD cycle. An effect() reacting to viewChild(), by contrast, is scheduled as a microtask-like reactive notification — it does not run synchronously inline with the change detection pass that populated the view child signal, but rather on the next tick of Angular\'s effect scheduler, which in practice usually means very shortly after but not necessarily perfectly synchronously with the render that made the element available. For code that must run in the exact same synchronous frame as view initialization (rare, but occasionally needed for precise DOM measurement ordering), ngAfterViewInit\'s synchronous guarantee is subtly different from the effect-based pattern\'s microtask-scheduled timing — usually invisible in practice, but worth knowing if debugging a timing-sensitive edge case.',
    },
    {
