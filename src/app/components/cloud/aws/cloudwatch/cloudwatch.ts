...
async function processOrder(event: any) { return { success: true }; }`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A composite alarm is built from three child alarms with AND logic, and one of the three child alarms is deleted or accidentally left in INSUFFICIENT_DATA state indefinitely (e.g., its underlying metric stopped being published). What happens to the composite alarm\'s ability to ever fire?',
      a: 'With AND logic, the composite alarm requires every child alarm to be in ALARM state simultaneously to itself transition to ALARM — a child stuck in INSUFFICIENT_DATA (not ALARM) means that condition of the AND expression can never be satisfied, so the composite alarm can never fire regardless of how badly the other two conditions degrade, silently disabling the entire alerting chain without any explicit error. This is a real operational risk with composite alarms: they introduce a dependency on every child alarm continuing to report valid data, and a quietly broken or deleted child (a common side effect of infrastructure changes that stop emitting a metric) turns the whole composite alarm into a silent no-op rather than a visibly broken alert — worth periodically auditing composite alarms\' child alarm health, not just the composite\'s own state.',
    },
    {
