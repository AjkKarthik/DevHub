...
// aws ecs register-task-definition --cli-input-json "$(echo taskDef | jq .)"`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A team on ECS wants to adopt a popular CNCF tool (say, a specific Kubernetes Operator or a Helm-packaged observability stack) that only ships as a Kubernetes manifest/Helm chart. Can they use it on ECS, and what does that imply about "just migrate to EKS later if you need the ecosystem" as a strategy?',
      a: 'No — Kubernetes manifests, CRDs, and Operators are fundamentally Kubernetes-API-shaped resources with no ECS equivalent concept, so a tool packaged only as a Helm chart cannot run on ECS at all, regardless of how the underlying container image itself is built (the image is portable; the deployment/orchestration definition is not). This makes the ECS-vs-EKS decision less reversible in practice than it first appears: a team that picks ECS for its simplicity and later discovers it needs a Kubernetes-ecosystem-only tool faces a genuine migration (rewriting deployment definitions, learning K8s networking/RBAC concepts, likely re-architecting service discovery) rather than a quick swap — which is why the decision is worth weighing against the team\'s medium-term tooling needs, not just today\'s complexity budget.',
    },
    {
