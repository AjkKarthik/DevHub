...
console.log({ userItem, postItem, commentItem, gsi });`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A single-table design ships to production with entity types USER, POST, and COMMENT sharing overloaded GSIs. Six months later, a new access pattern is needed that none of the existing GSIs support. What does adding support for it actually involve, compared to adding a new query to a normalized relational schema?',
      a: 'In a relational schema, a new access pattern is usually just a new SQL query (possibly with a new index) against the existing normalized tables — the schema itself rarely needs to change. In single-table DynamoDB design, an unanticipated access pattern often means adding a new GSI with a new key-overloading scheme (deciding what composite values go into its partition/sort keys across every entity type that needs to appear in results for that pattern), and potentially a backfill migration to populate that GSI\'s attributes on existing items that predate it — DynamoDB has a hard limit of 20 GSIs per table, and each GSI change is a schema-level decision, not just a query. This is the concrete cost behind the abstract advice to "know your access patterns upfront": in DynamoDB, discovering a new pattern late is a data-modeling and migration exercise, not a five-minute query change.',
    },
  ];
