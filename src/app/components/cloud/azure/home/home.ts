...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Azure Key Vault', route: '/azure', badge: 'Identity', description: 'Securely store and access secrets, keys, and certificates — access policies, RBAC, and private endpoints.', keyPoints: ['Secrets, keys, and certificates in one service', 'Access via Managed Identity — no credentials in code', 'Key Vault references in App Service settings', 'Soft delete and purge protection', 'Private endpoint for VNet isolation'], available: false },
  { title: 'Azure Service Bus', route: '/azure', badge: 'App Services', description: 'Enterprise messaging — queues, topics/subscriptions, sessions, dead-letter, and message ordering.', keyPoints: ['Queue: point-to-point; Topic: publish-subscribe', 'Message sessions for ordered processing', 'Dead-letter queue for failed messages', 'Peek-lock vs receive-delete modes', 'AMQP protocol; SDK for .NET, Java, Python'], available: false },
  { title: 'Azure Container Apps', route: '/azure', badge: 'Compute', description: 'Serverless Kubernetes — deploy containers without managing clusters, built-in Dapr and KEDA scaling.', keyPoints: ['Managed Kubernetes under the hood', 'KEDA-based autoscaling including scale to zero', 'Dapr sidecar: service discovery, state, pub/sub', 'Ingress: HTTP/HTTPS with custom domains', 'Container Apps Environment = shared network boundary'], available: false },
  { title: 'Azure Cache for Redis', route: '/azure', badge: 'Databases', description: 'Fully managed Redis — cache-aside, session storage, pub/sub, and geo-replication tiers.', keyPoints: ['Basic/Standard/Premium tiers', 'Cache-aside pattern with expiry TTL', 'Redis pub/sub for real-time messaging', 'Persistence and geo-replication (Premium)', 'Access via Managed Identity with Entra ID'], available: false },
  { title: 'Azure API Management', route: '/azure', badge: 'App Services', description: 'Publish, secure, and monitor APIs — policies, rate limiting, caching, developer portal, and backends.', keyPoints: ['Inbound/outbound/backend policy pipeline', 'Rate limiting: rate-limit-by-key policy', 'JWT validation policy for auth', 'Developer portal for API docs and keys', 'Backends: HTTP, Logic Apps, Functions'], available: false },
  { title: 'Azure Bicep Deep-dive',      route: '/azure', badge: 'Foundations', available: false,
    description: 'Azure-native IaC with Bicep — modules, loops, conditions, param files, and migration from ARM templates.',
    keyPoints: ['Bicep compiles to ARM JSON — same engine, far cleaner syntax', 'Modules: split resources into reusable .bicep files, reference with module keyword', 'Loops: [for item in array: { ... }] to deploy N identical resources', '@secure() on param types: prevents values from appearing in deployment history', 'bicep decompile: convert existing ARM JSON to Bicep starting point'] },
  { title: 'Azure Cheat Sheet', route: '/azure', badge: 'Reference', description: 'Azure CLI commands, resource naming, portal shortcuts, and common service quick reference.', keyPoints: ['az group create / az deployment group create', 'Resource naming: rg-, vnet-, vm-, sa- conventions', 'az account set --subscription; az login --tenant'], available: false },
];
