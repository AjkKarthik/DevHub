...
const quiz: QuizQuestion[] = [
  ...
  { q: 'A pod with no memory limit set is running on a node that is running low on total memory. What determines which pod the kernel OOM-kills first?', options: ['Kubernetes always kills the newest pod first, regardless of resource usage', 'The kernel OOM killer favors killing pods with the lowest QoS class (BestEffort first, then Burstable, Guaranteed last) via oom_score_adj, since unlimited/low-request pods are considered the safest to sacrifice', 'The pod using the LEAST memory is killed first to free the most headroom', 'All pods on the node are killed simultaneously to reset node state'], answer: 1, explanation: 'Kubernetes sets each container\'s oom_score_adj based on its QoS class: BestEffort pods (no requests or limits set) get the highest (worst) score and are killed first under node memory pressure, Burstable pods are next, and Guaranteed pods (requests == limits) are protected until last. This is why setting requests/limits isn\'t just about your own pod\'s behavior — it also determines your pod\'s priority for survival when a NEIGHBORING pod on the same node causes memory pressure, even if your own pod never exceeded its own limit.' },
];
