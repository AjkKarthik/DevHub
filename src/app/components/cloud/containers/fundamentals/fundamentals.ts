...
const qna: QnaItem[] = [
  ...
  { q: 'What is a "microVM" (e.g. Firecracker) and why do some container platforms use it as a middle ground?', a: 'A microVM is a stripped-down virtual machine — using a minimal hypervisor and a pared-down device model — designed to boot in tens of milliseconds instead of the seconds-to-minutes a traditional VM takes, while still providing true hardware-level isolation (a separate kernel per microVM) rather than the shared-kernel isolation of containers. Platforms like AWS Lambda and Fargate use Firecracker microVMs specifically to get container-like startup speed and density with VM-level security isolation between untrusted workloads, addressing the exact tradeoff between containers (fast, weaker isolation) and full VMs (slow, strong isolation).' },
];
