...
const quiz: QuizQuestion[] = [
  ...
  { q: 'If you apply an Ingress-only NetworkPolicy to a pod, what happens to that pod\'s OUTBOUND (egress) traffic?', options: ['Egress is automatically blocked too, since any policy locks down all traffic directions', 'Egress remains completely unrestricted — a NetworkPolicy only restricts the traffic DIRECTIONS listed in its policyTypes, so an Ingress-only policy leaves egress exactly as open as if no policy existed', 'Egress is allowed only to pods within the same NetworkPolicy', 'Egress requires a separate CNI plugin to function at all'], answer: 1, explanation: 'A NetworkPolicy\'s policyTypes field explicitly declares which traffic directions it governs. A policy with policyTypes: [Ingress] and ingress rules only restricts INCOMING traffic to the selected pods — it has no effect whatsoever on egress, which remains fully open (assuming no other egress policy applies) even though the pod now has "a NetworkPolicy" attached. This is a common misconfiguration: teams assume adding any NetworkPolicy locks down all traffic, when in fact each direction must be locked down independently and explicitly.' },
];
