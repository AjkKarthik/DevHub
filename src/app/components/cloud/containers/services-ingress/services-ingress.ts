...
const qna: QnaItem[] = [
  ...
  { q: 'Why can a misconfigured Pod-level dnsConfig or dnsPolicy cause DNS resolution to silently work in one namespace but fail in another?', a: 'A Pod\'s dnsPolicy defaults to ClusterFirst, which routes DNS queries to CoreDNS for cluster-internal names and falls through to the upstream/host resolver for anything else — but if a Pod explicitly sets dnsPolicy: Default (using the NODE\'s resolv.conf instead of CoreDNS) or a custom dnsConfig with the wrong nameserver/search domains, short names like just "myservice" (relying on the namespace-scoped search domain CoreDNS provides) stop resolving even though fully-qualified names like myservice.namespace.svc.cluster.local might still work by chance — this namespace-search-domain dependency is why DNS problems can appear namespace-specific even though the root cause is a Pod-level DNS policy misconfiguration, not a namespace setting.' },
];
