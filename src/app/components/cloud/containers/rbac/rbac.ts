...
const qna: QnaItem[] = [
  ...
  { q: 'Why can kubectl auth can-i --list give a misleading picture of a service account\'s TRUE blast radius if that SA can create RoleBindings itself?', a: 'kubectl auth can-i --list shows only the DIRECT permissions currently granted — it does not reveal privilege escalation paths. If a service account has rolebindings/create or clusterrolebindings/create permission (even without broad direct access to other resources), it can bind itself (or create a new subject) to an existing powerful ClusterRole it\'s merely allowed to REFERENCE, effectively granting itself much broader access than the direct listing implies. Auditing must also check for these escalation-capable verbs (create on rolebindings/clusterrolebindings, or escalate/bind on roles) separately from the straightforward permission list, since the direct list alone understates real risk.' },
];
