...
const qna: QnaItem[] = [
  ...
  { q: 'Why can kubectl auth can-i --list give a misleading picture of a service account\'s TRUE blast radius if that SA can create RoleBindings itself?', a: 'kubectl auth can-i --list shows only the DIRECT permissions currently granted — it does not reveal privilege escalation paths. Kubernetes\' own RBAC authorizer blocks a subject from binding to a Role/ClusterRole granting MORE than it already has, unless that subject also holds the special bind verb on roles/clusterroles (or the escalate verb to edit a Role/ClusterRole directly) — so rolebindings/create ALONE is not enough to self-escalate. The real risk is a service account holding BOTH rolebindings/clusterrolebindings create AND bind (or escalate) together, since that combination lets it reference an existing powerful ClusterRole it could never have been granted directly. Auditing must check for this specific combination, not just create in isolation, since the direct permission listing alone doesn\'t surface it.' },
];
