...
const qna: QnaItem[] = [
  ...
  {
    q: 'What HEALTHCHECK options control how long Docker waits before the first check counts, and why does that matter for slow-starting apps?',
    a: 'The start-period option (HEALTHCHECK --start-period=40s ...) gives the container a grace window where failing checks do not count toward the unhealthy threshold — without it, a JVM app that takes 30 seconds to warm up would get marked unhealthy (and potentially restarted by an orchestrator) before it even finished booting, since the first few health checks would fail during normal startup, not due to an actual problem. Setting start-period to comfortably exceed your app\'s typical cold-start time avoids this false-positive unhealthy state.',
  },
  { q: 'Why does BuildKit\'s cache invalidate an entire COPY layer if even a single file in the copied directory changes, and how do multi-stage builds mitigate this?', a: 'Docker layer caching works on the ENTIRE result of an instruction — COPY . . hashes all copied file contents together, so changing any single file (even a README) invalidates that layer and every layer after it, forcing a full rebuild of everything downstream including expensive steps like npm install. The standard mitigation is copying dependency manifests (package.json, requirements.txt) and installing dependencies in an early layer BEFORE copying the rest of the source code — since manifest files change far less often than source code, that expensive install layer stays cached across most rebuilds, and only the final COPY . . layer (which is comparatively cheap) gets invalidated by routine code changes.' },
];
