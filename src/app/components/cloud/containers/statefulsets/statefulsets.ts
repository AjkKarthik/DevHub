...
const quiz: QuizQuestion[] = [
  ...
  { q: 'If a StatefulSet has 3 replicas and pod web-1 (the middle pod) crashes, does Kubernetes recreate web-1, or shift web-2 down to fill the gap?', options: ['It shifts all higher-numbered pods down by one to keep names contiguous', 'Kubernetes always recreates the pod with the SAME ordinal identity (web-1) — ordinals are never reassigned or shifted, even temporarily', 'A new pod is created with the next available ordinal (web-3)', 'The StatefulSet pauses until manually resolved'], answer: 1, explanation: 'StatefulSet pod identities are permanently tied to their ordinal index — web-1\'s name, stable network identity, and associated PVC are fixed for that ordinal slot regardless of restarts. When web-1 crashes, Kubernetes recreates a pod with the exact same name web-1, reattaches the same PersistentVolumeClaim, and it resumes with the same stable DNS name — this predictability (never renumbering or shifting) is precisely what makes StatefulSets suitable for clustered systems that track peers by stable identity (e.g. Kafka broker IDs derived from ordinal).' },
];
