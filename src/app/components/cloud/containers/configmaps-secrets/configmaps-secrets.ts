...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What size limit applies to both ConfigMaps and Secrets, and what should you use instead if you exceed it?', options: ['There is no size limit for either', 'Both are limited to 1MiB total size (etcd\'s per-object limit) — for larger data, mount a volume backed by a PVC, object storage, or a ConfigMap generated from a file split across multiple objects', 'Secrets are limited to 1MiB but ConfigMaps are unlimited', 'The limit is 1GiB, matching typical container image layer sizes'], answer: 1, explanation: 'Both ConfigMaps and Secrets are stored as etcd objects and are capped at 1MiB total size (etcd\'s hard limit for any single object, to keep the cluster\'s key-value store performant). Trying to create a larger ConfigMap or Secret fails outright. Large configuration files, datasets, or binary blobs belong on a PersistentVolume, object storage (S3/GCS), or should be split across multiple smaller ConfigMaps if they must stay in Kubernetes-native storage.' },
];
