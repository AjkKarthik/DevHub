...
// ['outbound|8080||payment.prod.svc.cluster.local']`,
  ...
  { q: 'A custom EnvoyFilter is supposed to add a Lua filter to a proxy\'s HTTP filter chain, but the filter never seems to execute. `istioctl proxy-config listeners` and `clusters` both look correct — what is the next debugging step to confirm whether the filter was actually applied to the running Envoy?', a: 'Pull the FULL live configuration Envoy is actually running with `istioctl proxy-config all pod/name -o json` (or hit the admin API\'s /config_dump endpoint directly on port 15000) and search the raw filter chain for your custom filter by name — listeners/clusters/routes subcommands show a summarized view that can look "correct" at a glance while omitting or misordering a specific HTTP filter, whereas the raw config_dump is the ground truth of exactly what Envoy loaded, including filter ordering (which matters — EnvoyFilter insertion points like BEFORE/AFTER/FIRST relative to existing filters can silently place your filter somewhere it never gets invoked, e.g. after the router filter has already terminated the chain).' },
  ];
