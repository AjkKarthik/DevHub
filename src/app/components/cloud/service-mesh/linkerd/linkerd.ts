...
console.log(getTrafficSplit(100));`,
  ...
  quiz: QuizQuestion[] = [
    ...
    {
      q: 'A team picks Linkerd specifically for its ~10MB proxy footprint (vs Envoy\'s ~50-80MB) to run on a resource-constrained edge cluster. Six months later they need JWT-based request authentication that Linkerd\'s CRDs don\'t directly support. What is actually true about Linkerd\'s extensibility model compared to Istio\'s EnvoyFilter/WASM approach?', options: ['Linkerd supports the exact same WASM filter extension mechanism as Istio', 'Linkerd deliberately does not expose a low-level filter-injection escape hatch analogous to EnvoyFilter — its proxy (linkerd2-proxy, written in Rust) is intentionally simpler and less pluggable than Envoy, so unsupported features generally require either handling them at the application layer or an external gateway/authorization service in front of the mesh, rather than injecting custom mesh-level logic', 'Linkerd requires switching its underlying proxy to Envoy to gain extensibility', 'Both meshes have identical extensibility since both use xDS internally'], answer: 1, explanation: 'Linkerd\'s smaller footprint and lower operational complexity are a direct consequence of a genuinely simpler proxy with a narrower feature surface — linkerd2-proxy does not implement Envoy\'s general-purpose filter chain or WASM extension points, by design, to stay lean and auditable. This is a real, sometimes-underestimated tradeoff: features Istio would solve with an EnvoyFilter or a WASM plugin (custom JWT validation logic, protocol-specific processing) generally are NOT achievable inside Linkerd\'s data plane at all — they need to be handled at the application layer, via an API gateway sitting in front of the mesh, or by accepting the limitation, which is worth confirming BEFORE committing to Linkerd for a use case with evolving L7 policy needs.' },
    {
  ...
  { q: 'A pod is injected with the Linkerd proxy, but a request from it to another meshed pod arrives as PLAINTEXT (not mTLS) — something Linkerd\'s "automatic mTLS with no PERMISSIVE mode" design is supposed to make impossible. What is the most likely explanation, given Linkerd has no configurable permissive mode to accidentally leave on?', a: 'The most common real cause is that one side of the connection is NOT actually meshed — the client pod might have the linkerd-proxy sidecar, but the destination pod lacks the linkerd.io/inject annotation (or injection failed silently, or the destination is a non-Kubernetes external endpoint), so the meshed side\'s outbound traffic to an unmeshed destination has no peer proxy to negotiate mTLS with and falls back to plain traffic by necessity, not by misconfiguration. Since Linkerd has no PERMISSIVE-mode dial to accidentally leave open, mixed mTLS/plaintext traffic in an all-injected Linkerd deployment almost always traces back to an injection gap on one side of the connection — checking `linkerd viz edges` (which shows per-connection TLS status) is the fastest way to spot exactly which service pair is missing mTLS and why.' },
  ];
