...
console.log(JSON.stringify(getMigrationPlan('production', true), null, 2));`,
  ...
  { q: 'What is the traffic redirection mechanism in Istio ambient mesh?', a: 'Ambient mesh uses ztunnel, a per-node Layer 4 proxy, for traffic redirection instead of per-pod iptables rules. ztunnel intercepts all TCP traffic from pods on the node using a transparent socket interception mechanism. For L7 processing, ambient mesh uses waypoint proxies (full Envoy instances) deployed per namespace or workload. This eliminates the need for sidecar containers entirely while still providing mTLS, telemetry, and policy enforcement.' },
  { q: 'How does ambient mesh handle security policy without sidecars?', a: 'In ambient mesh, ztunnel enforces L4 security policy including mTLS and L4 authorization policies for all traffic passing through the node. PeerAuthentication and AuthorizationPolicy resources are respected by ztunnel at L4. For L7 policies like HTTPRoute rules or L7 AuthorizationPolicies, traffic is routed through a waypoint proxy that is deployed separately. This split between L4 ztunnel and optional L7 waypoint proxy gives ambient mesh its layered security architecture without requiring sidecars.' },
  ];
