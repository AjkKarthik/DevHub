...
console.log(generateMultiClusterDR('payment', 'production', 'us-east1', 'eu-west1', 500));`,
  ...
  { q: 'What is the difference between primary-remote and multi-primary Istio multi-cluster modes?', a: 'In primary-remote mode, one cluster hosts the Istio control plane (primary) and remote clusters only run Envoy sidecars that connect to the primary control plane. The primary cluster manages configuration and certificates for all clusters. This simplifies operations but the primary cluster is a single point of failure for the control plane. In multi-primary mode, each cluster runs its own Istio control plane, providing better fault isolation. Clusters trust each other by sharing a common root CA. Both modes support cross-cluster service discovery and load balancing via endpoint discovery.' },
