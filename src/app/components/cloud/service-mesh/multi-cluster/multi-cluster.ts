  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        'There is no Istiod-to-Istiod peer protocol in multi-primary — each Istiod learns about the OTHER cluster\'s services and endpoints purely by independently watching that cluster\'s own Kubernetes API server (via the remote-secret credentials described above), the same way it watches its own local cluster. `remotePilotAddress` is a real Istio config field, but it belongs to the DIFFERENT Primary-Remote topology (it points a remote cluster\'s sidecar injector at the primary\'s Istiod for xDS) — it plays no role in multi-primary\'s config exchange.',
    ...
    },
    {
      ...
      points: [
        ...
        'Kiali multi-cluster: Kiali has supported multi-cluster service graphs since its initial experimental "Cluster Boxes" release (v1.29/1.30), refined across subsequent 1.6x/1.7x releases, and is available on the current Kiali v2.x line — there is no "1.73+" cutoff before which multi-cluster graphs are entirely unavailable. Configure it with multiple cluster API server connections; the service graph shows inter-cluster edges with latency and error rate.',
