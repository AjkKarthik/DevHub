  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        'Ejection duration increases linearly with each ejection: duration = baseEjectionTime × (number of times this host has been ejected in a row) — first ejection = baseEjectionTime, second = 2×, third = 3×, etc. Pods that keep misbehaving stay ejected longer.',
    ...
    },
    {
      ...
      points: [
        ...
        'Abort injection: `fault.abort.percentage.value: 50; fault.abort.httpStatus: 503` — 50% of requests receive a 503. Test: does the calling service handle the error gracefully? Does the circuit breaker trip after enough failures?',
        ...
        'Fault injection and retries CANNOT coexist on the same VirtualService route — if a route configures both a `fault` block and `retries`/`timeout`, Istio silently disables the retry/timeout policy for that route. To actually test whether a retry policy recovers from a simulated failure, inject the fault on a SEPARATE, header-matched route (or a distinct VirtualService) so the real route\'s retries stay genuinely active.',
...
console.log(getResilienceConfig());`,
  ...
  qna: QnaItem[] = [
    ...
    },
    {
      ...
      a: '<code>minHealthPercent</code> (default <strong>0%</strong>, meaning disabled by default — not 50%) is an emergency brake: when the percentage of healthy hosts in the load balancer pool drops below this threshold, outlier detection is <em>disabled entirely</em> and the proxy load-balances across ALL hosts in the pool, healthy and unhealthy alike — not merely "no new ejections," but already-ejected hosts are restored to rotation too. This prevents outlier detection from draining the entire pool during a widespread failure. Example: with <code>maxEjectionPercent: 70</code> and <code>minHealthPercent: 30</code>, Istio will eject up to 70% of pods, but if healthy pods would drop below 30%, outlier detection switches off and the cluster falls back to sending traffic everywhere. The cluster degrades gracefully rather than being completely blacked out. Since the default is 0% (off), this safety valve must be explicitly configured to take effect.',
    },
