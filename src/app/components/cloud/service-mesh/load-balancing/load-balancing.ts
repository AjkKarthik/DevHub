...
console.log(getDestinationRule());`,
  ...
  { q: 'How does locality-weighted load balancing work in Istio?', a: 'Locality-weighted load balancing in Istio routes traffic preferentially to endpoints in the same region and zone as the calling workload to reduce latency and cross-zone data transfer costs. Configure it in a DestinationRule with the localityLbSetting block specifying distribution weights across regions and zones. Failover settings define what happens when local endpoints are unhealthy: traffic overflows to the next closest locality based on the priority ordering. This is especially useful for globally distributed services where you want to keep traffic local but have automatic failover to remote regions when local capacity is exhausted.' },
  ];
