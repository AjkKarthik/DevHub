...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Envoy Proxy Deep Dive', route: '/service-mesh', badge: 'Istio', description: 'Envoy architecture — listeners, routes, clusters, endpoints, filters, and the xDS API.', keyPoints: ['LDS → RDS → CDS → EDS discovery chain', 'HTTP filters: router, rate-limit, JWT, WASM', 'Access log format and sampling', 'Admin API: /stats, /config_dump, /clusters', 'Hot restart for zero-downtime config reload'], available: false },
  { title: 'Ambient Mesh', route: '/service-mesh', badge: 'Foundations', description: 'Istio ambient mode — ztunnel L4 layer without sidecars, waypoint proxies for L7, and reduced overhead.', keyPoints: ['No sidecar injection needed', 'ztunnel: node-level L4 mTLS transparently', 'Waypoint proxy: optional L7 per service', '60-90% memory reduction vs sidecar', 'Label: istio.io/dataplane-mode=ambient'], available: false },
  { title: 'Multi-cluster Mesh', route: '/service-mesh', badge: 'Foundations', description: 'Extend the service mesh across clusters — Istio multi-cluster models, east-west gateway, and trust.', keyPoints: ['Primary-remote vs multi-primary models', 'East-west gateway for cross-cluster traffic', 'Shared trust domain with common root CA', 'ServiceEntry for cross-cluster endpoints', 'Locality load balancing prefers local cluster'], available: false },
  { title: 'Consul Connect', route: '/service-mesh', badge: 'Reference', description: 'HashiCorp Consul as a service mesh — service discovery, Connect intentions, and multi-platform support.', keyPoints: ['Consul catalog: service registry + health checks', 'Connect: sidecar proxies with mTLS', 'Intentions: allow/deny service-to-service rules', 'Works outside Kubernetes (VMs, bare metal)', 'Consul Dataplane: lightweight mode for K8s'], available: false },
  { title: 'Service Mesh Cheat Sheet', route: '/service-mesh', badge: 'Reference', description: 'Istio CRDs, istioctl commands, mTLS modes, and traffic management YAML quick reference.', keyPoints: ['Key CRDs: VirtualService, DestinationRule, Gateway, AuthorizationPolicy', 'istioctl analyze; istioctl proxy-status; istioctl dashboard kiali', 'PeerAuthentication modes: DISABLE, PERMISSIVE, STRICT'], available: false },
  { title: 'Service Mesh Interview Prep', route: '/service-mesh', badge: 'Reference', description: '30+ service mesh interview questions — sidecar pattern, mTLS, traffic management, and observability.', keyPoints: ['What problem does a service mesh solve that a service itself cannot?', 'Explain the difference between a Gateway and a VirtualService', 'How does ambient mesh eliminate sidecar overhead?'], available: false },
];
