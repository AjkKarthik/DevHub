...
console.log(config.serviceRouter);`,
  ...
  { q: 'How does Consul service mesh compare to Istio?', a: 'Consul service mesh (Consul Connect) uses Envoy as the data plane proxy, similar to Istio, but uses a different control plane. Consul stores service catalog, health checks, and intentions (access policies) in its own distributed key-value store rather than relying on Kubernetes CRDs. Consul works across multiple platforms including VMs, Kubernetes, and bare metal in hybrid environments. Istio is more Kubernetes-native with richer L7 traffic management via VirtualService and DestinationRule. Consul is often preferred in multi-platform environments or when teams already use Consul for service discovery.' },
  { q: 'What are Consul service intentions and how do they differ from Istio AuthorizationPolicies?', a: 'Consul intentions define which services are allowed to communicate with each other. An intention specifying allow from api to db means the api service can connect to db but no other service can. Intentions are stored in Consul and enforced by Envoy sidecars. They operate at the identity level (service name) rather than on HTTP attributes. Istio AuthorizationPolicies are more flexible: they support matching on HTTP method, path, headers, JWT claims, and source IP in addition to source workload identity. For complex L7 authorization, Istio provides richer policy expressions.' },
  ];
