  ...
  theory: TheoryPoint[] = [
    {
      ...
      points: [
        ...
        'Consul\'s identity model combines Consul ACL tokens for API/agent-level access with per-service mTLS certificates whose URI SANs ARE genuinely SPIFFE-format (e.g. `spiffe://<cluster-id>.consul/ns/default/dc/dc1/svc/web`) — issued by Consul\'s own built-in or Vault-backed CA, not Istiod\'s. Consul and Istio are both SPIFFE-compatible in certificate FORMAT; they simply use different, mutually-untrusted CAs.',
    ...
    },
    {
      ...
      points: [
        ...
        'Consul Cluster Peering: modern replacement for WAN federation. Two independent Consul clusters (different trust domains) establish a peering connection. Services in each cluster can be exported/imported. Per Consul\'s own DNS reference, the exported services are queried using the PEER\'S OWN NAME as a distinct segment in the DNS name — e.g. `redis.service.<peer-name>.peer.consul` — not a generic fixed literal path.',
        'Exported services: in cluster peering, services must be explicitly exported (`ExportedServices` CRD) to be visible to peer clusters. This is more intentional (no accidental cross-cluster access) but requires more configuration than Istio\'s transparent multi-cluster discovery. The peer\'s own name is part of the DNS query (e.g. `redis.service.<peer-name>.peer.consul`), not a fixed literal path.',
...
console.log(config.serviceRouter);`,
  ...
  qna: QnaItem[] = [
    ...
    },
    {
      ...
      a: 'Consul\'s CA (built-in or Vault-backed) issues TLS certificates to each service proxy. The certificate lifecycle: <ol><li>consul-dataplane (or legacy consul-client) requests a certificate from the Consul Connect CA for its service identity</li><li>The CA issues a cert with a short TTL (default: 72 hours for leaf certs)</li><li>consul-dataplane automatically refreshes the cert somewhere in a 60%–90% window of its elapsed lifetime (not a single fixed percentage) — the exact refresh point is jittered within that range to avoid every proxy in the mesh re-requesting a cert at the same instant</li><li>The new cert is hot-swapped in Envoy via SDS (Secret Discovery Service) — no Envoy restart</li></ol>For Vault-backed CA: Consul sends CSRs to Vault PKI; Vault issues and signs certs. Rotation follows the same lifecycle. Configure `connect.caConfig.LeafCertTTL` in Consul to adjust TTL. The built-in CA root cert has a 10-year TTL by default — rotate proactively well before that.',
    },
    {
