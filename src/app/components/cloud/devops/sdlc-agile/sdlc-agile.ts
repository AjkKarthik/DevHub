...
// pessimistic = ceil(120/27) = 5`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A team runs Scrum for feature development but their production incidents also get thrown into the same sprint backlog, competing for the same fixed sprint capacity as planned feature work. What DevOps-specific problem does this create, and how does separating deployment/ops flow from planning flow address it?',
      a: 'Incidents and urgent ops work are inherently unpredictable-arrival, pull-based work — exactly the shape Kanban is designed for — while feature commitments are planned, push-based work locked in at sprint start. Forcing both into the same fixed-capacity sprint means every unplanned incident either blows the sprint\'s committed scope (breaking the team\'s ability to reliably forecast) or gets deprioritized behind planned feature work (letting production problems sit unaddressed until the next planning cycle). The standard DevOps fix is running the two as separate flows: feature development stays in Scrum\'s sprint cadence for planning predictability, while the deployment pipeline and incident/ops work run on a continuous Kanban-style pull system with WIP limits — so an incident can be pulled and worked immediately without derailing the sprint\'s feature commitments, and the two systems only intersect where an incident\'s follow-up work (a permanent fix) gets explicitly re-prioritized into a future sprint.' },
  ];
