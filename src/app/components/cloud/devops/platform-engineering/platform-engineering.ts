...
// legacy-api issues: ['Missing owner', 'Missing description', 'No tags', 'Production service has no runbook']`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A platform team ships a golden path for "create a new service" but six months later notices most new services are still being hand-rolled outside it. What does this usually indicate about the golden path itself, rather than about the engineers ignoring it?',
      a: 'Low voluntary adoption of a genuinely well-built golden path is a signal to look at the golden path\'s own design, not at engineer compliance — common root causes are: the scaffolded template is out of date with the platform\'s current best practices (so following it produces something engineers know they\'ll have to fix later anyway), the golden path covers only the happy path and breaks down for any team with slightly nonstandard requirements (forcing an early "fork" onto the manual route), or the platform team never did the user research to confirm the golden path actually solves engineers\' real pain points rather than the platform team\'s assumptions about their pain points. Since a golden path is explicitly meant to be the path of least resistance rather than mandatory, low adoption is direct product-market-fit signal (per the "platform as a product" principle) that something about the path itself needs fixing — chasing adoption through mandates instead treats a product problem as a compliance problem.',
    },
  ];
