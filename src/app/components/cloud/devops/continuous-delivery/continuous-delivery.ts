...
// score: 15 (change) + 25 (db) + 40 (friday) + 35 (no smoke) = 115, level: CRITICAL`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A team practicing Continuous Deployment (fully automatic, no manual gate) starts noticing that a growing share of "deployable" commits never actually get promoted quickly — engineers are quietly batching commits before merging to avoid triggering frequent auto-deploys. What does this signal, and how does it undermine the practice?',
      a: 'This is a sign the team does not actually trust their pipeline\'s automated checks to catch problems, even though the whole premise of Continuous Deployment is that a passing pipeline is sufficient evidence for production release — batching commits to reduce deploy frequency is engineers manually reintroducing the human gate that CD was designed to eliminate, just informally and inconsistently instead of through an explicit approval step. This defeats the core benefit (fast, small, low-risk releases) while keeping none of the benefit of an explicit gate (deliberate release timing, audit trail of who approved what) — the fix is either genuinely investing in the test/monitoring/rollback confidence needed to trust full automation, or being honest that the team actually wants Continuous Delivery with an explicit gate instead of pretending to practice Continuous Deployment.',
    },
  ];
