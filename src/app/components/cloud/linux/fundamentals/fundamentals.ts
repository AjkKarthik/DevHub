...
console.log(result.VERSION_ID); // 22.04`,
  ...
  quiz: QuizQuestion[] = [
    ...
    {
      q: 'A server\'s `uname -r` shows a kernel version, but the system has a newer kernel package installed via the package manager. Why can these disagree, and which one reflects reality until you act?',
      options: [
        'They can never disagree — uname -r always reflects the latest installed package',
        'uname -r reports the kernel currently RUNNING in memory, which was loaded at the last boot — installing a newer kernel package only stages it on disk for the NEXT boot, so the two stay out of sync until a reboot actually loads the new kernel',
        'The package manager automatically hot-swaps the running kernel without a reboot',
        'uname -r reads from the package manifest, not the running kernel',
      ],
      answer: 1,
      explanation: 'The Linux kernel is loaded once at boot time and stays resident in memory for the life of that boot — installing a new kernel package (via apt/yum/dnf) writes the new kernel image and modules to disk but does not replace the currently-running kernel. This is precisely why kernel security patches require a reboot to actually take effect, and why an automated patching pipeline that installs a new kernel but never reboots leaves the system running the OLD, vulnerable kernel indefinitely even though the package manager reports the new version as "installed" — checking `uname -r` after a patch cycle is the ground-truth way to confirm the new kernel is actually active, not just present on disk.',
    },
    {
