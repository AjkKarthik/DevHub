...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Environment Variables & Shell Config', route: '/linux', badge: 'Shell Scripting', description: 'export, .bashrc, .profile, .bash_profile, /etc/environment, and managing PATH reliably.', keyPoints: ['export VAR=value: available to child processes', '~/.bashrc: interactive shell; ~/.profile: login shell', 'source ~/.bashrc or . ~/.bashrc reload without new shell', '/etc/environment: system-wide, no export needed', 'printenv; env | grep PATTERN'], available: false },
  { title: 'Log Analysis', route: '/linux', badge: 'System Admin', description: 'Analyse logs with journalctl, tail, grep, awk, and logrotate — finding errors and monitoring patterns.', keyPoints: ['journalctl -u nginx -f follow service logs', 'journalctl --since "1 hour ago"', 'grep -E "ERROR|WARN" /var/log/syslog', 'awk "{print $1}" for field extraction', 'logrotate: rotate, compress, and delete old logs'], available: false },
  { title: 'Performance Tuning', route: '/linux', badge: 'System Admin', description: 'sysctl parameters, file descriptor limits, CPU governor, NUMA, and kernel boot parameters for performance.', keyPoints: ['sysctl vm.swappiness=10 reduce swap use', 'ulimit -n 65536 file descriptor limit', '/proc/sys/net/core/somaxconn: connection queue', 'CPU governor: performance vs powersave', 'numactl for NUMA-aware workloads'], available: false },
  { title: 'Vim & Text Editors', route: '/linux', badge: 'Foundations', description: 'Vim modes, navigation, editing commands, and survival skills for editing config files on any server.', keyPoints: ['Normal/Insert/Visual/Command modes', ':w save; :q quit; :wq save+quit; :q! force quit', 'i insert; a append; dd delete line; yy yank; p paste', '/pattern search; n next; :%s/old/new/g replace all', 'nano: simpler alternative — ctrl+o save, ctrl+x exit'], available: false },
