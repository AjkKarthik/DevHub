...
resource "null_resource" "notify" {
  ...
  { q: 'Why does HashiCorp recommend avoiding Terraform provisioners when possible?', options: ['Provisioners are too expensive and increase cloud costs', 'Provisioners run arbitrary scripts that cannot be modeled in state, making deployments unpredictable and breaking idempotency', 'Provisioners only work on Linux and not Windows', 'Provisioners require the Terraform Enterprise license'], answer: 1, explanation: 'Provisioners run scripts or commands on resources after creation, but their execution is not tracked in state. If a provisioner fails, Terraform marks the resource as tainted to be destroyed and recreated on next apply. If a provisioner succeeds but the result is later changed, Terraform has no way to detect or reconcile this drift. This breaks idempotency. Prefer cloud-native init mechanisms like EC2 user_data, cloud-init, or AWS SSM Run Command instead.' },
  { q: 'What is the difference between local-exec and remote-exec provisioners?', options: ['local-exec runs on the managed resource; remote-exec runs on the Terraform operator machine', 'local-exec runs a command on the machine running Terraform; remote-exec connects to the managed resource and runs commands there', 'They are identical but target different operating systems', 'remote-exec is deprecated; local-exec should always be used instead'], answer: 1, explanation: 'local-exec runs a command on the machine where Terraform is executing, such as your laptop or CI runner. Useful for triggering external scripts, calling APIs, or invoking other tools after resource creation. remote-exec connects to the provisioned resource via SSH or WinRM and runs commands on it directly. Both provisioners run only on resource creation by default, or with when = destroy set for teardown scripts.' },
  ];
  ...
  { q: 'When is it acceptable to use Terraform provisioners?', a: 'Provisioners are acceptable as a last resort when no native Terraform resource or data source exists for the operation, and when the operation is genuinely a one-time setup step. Example use cases: bootstrapping a Kubernetes cluster node with initial configuration before the cluster is ready to accept manifests, or registering a resource with an external system that has no Terraform provider. Always document why a provisioner is necessary with a comment. Explore alternatives first: cloud-init for VM configuration, Lambda functions triggered by resource events, or purpose-built resources in the provider if available.' },
  { q: 'How does the on_failure setting affect Terraform provisioners?', a: 'By default, on_failure = fail: if the provisioner script exits with a non-zero code, Terraform marks the resource as tainted and fails the apply. The tainted resource will be destroyed and recreated on the next apply. Setting on_failure = continue ignores the failure and continues with the rest of the apply. Use continue sparingly because it silently swallows errors. A tainted resource can be manually untainted with terraform untaint if you want to retry the provisioner without recreating the underlying resource. Always add timeouts to provisioners to prevent hanging applies caused by hung processes or network issues.' },
  ];
