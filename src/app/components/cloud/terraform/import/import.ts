...
resource "aws_s3_bucket_versioning" "data_lake" {
  ...
  { q: 'A team commits several import {} blocks to bring 20 existing resources under management, runs terraform apply successfully, and the resources are now in state. Should the import blocks be deleted from the configuration afterward?', options: ['No, they must remain forever or the resources will be un-imported', 'Yes — once an import block\'s apply has completed successfully, the resource is permanently recorded in state and the import block itself has served its purpose; leaving it in the config causes Terraform to treat it as a no-op on subsequent plans (since the resource is already in state) but it is safe (and cleaner) to remove it', 'Only if the -generate-config-out flag was NOT used', 'Deleting it will destroy the imported resource'], answer: 1, explanation: 'An import block\'s job is to move a resource from "exists in the real world, unknown to Terraform" into "tracked in state" — once that transition happens during a successful apply, the block has done its job. Terraform re-evaluates import blocks on every plan and simply no-ops if the target is already in state at the specified ID, so leaving old import blocks around is not harmful, but it does add clutter and can be confusing to future readers who wonder whether the resource is still "pending import." Best practice is treating import blocks as one-time migration scaffolding — remove them once the corresponding apply has landed, keeping only the resulting resource block as the permanent configuration.' },
  ];
