...
resource "aws_s3_bucket" "data" {
  ...
  { q: 'A team migrates from using only the default workspace to a multi-workspace setup (dev, staging, prod) but forgets that their production infrastructure was originally provisioned under the default workspace. What is the practical consequence of leaving prod on default instead of creating and migrating it to a "prod" workspace?', options: ['No consequence — the default workspace behaves identically to any named workspace in every respect', 'Nothing breaks functionally, but it creates an asymmetry that\'s easy to misread operationally: default has no explicit name showing up in `terraform workspace list` selection prompts the way "prod" would, making it easier for someone to accidentally run commands against production while believing they are in an unconfigured/no-workspace state', 'The default workspace automatically renames itself to match the environment', 'Terraform refuses to apply changes to the default workspace once other workspaces exist'], answer: 1, explanation: 'Functionally, default is just a workspace like any other (it stores state at the base backend path rather than an environment-prefixed path) — the real risk is human, not technical. Once a team has trained itself to check "which workspace am I in?" before running terraform commands against dev/staging/prod, a production environment silently sitting under the generically-named default workspace breaks that mental model: someone doing `terraform workspace select dev` to safely experiment might not realize that NOT selecting a workspace at all still means their command targets default — which is actually production. This is why teams that adopt workspaces deliberately often rename or explicitly recreate even their "first" environment under a named workspace, rather than leaving anything load-bearing on the implicit default.' },
  ];
