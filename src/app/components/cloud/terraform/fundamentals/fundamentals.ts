...
output "bucket_arn" {
  ...
  { q: 'A CI pipeline runs `terraform init` fresh on every run (no cached .terraform directory) and someone notices the pipeline is slow and occasionally installs a slightly different provider patch version than a teammate\'s local run got yesterday. What two things address these separately?', options: ['Both problems are solved automatically by upgrading Terraform itself', 'A committed .terraform.lock.hcl file pins exact provider versions for reproducibility across machines and runs; caching the .terraform/providers directory (or the plugin cache dir) between CI runs addresses the speed problem — they are independent fixes for independent problems', 'Running terraform init -upgrade on every run fixes both issues at once', 'Neither problem is fixable without switching to Terraform Cloud'], answer: 1, explanation: 'These are two distinct problems with two distinct fixes. Reproducibility: without a committed .terraform.lock.hcl, each `terraform init` can resolve to a different (newer, still-constraint-satisfying) provider patch version, so two runs on different days can silently use different provider code — committing the lock file pins the exact version everyone gets. Speed: re-downloading the same provider binaries on every CI run wastes time and bandwidth even with a correct lock file — setting the TF_PLUGIN_CACHE_DIR environment variable (or caching .terraform/providers as a CI cache artifact) lets init reuse already-downloaded provider binaries instead of re-fetching them every run. Conflating the two often leads to fixing one while leaving the other unaddressed.' },
  ];
