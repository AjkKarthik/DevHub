...
jobs:
  ...
  { q: 'What is the recommended approach for storing Terraform state in CI/CD pipelines?', options: ['Commit the state file to the repository alongside code', 'Use a remote backend like S3 with DynamoDB locking or Terraform Cloud', 'Store state only on the CI runner local filesystem', 'Regenerate state from scratch on every pipeline run'], answer: 1, explanation: 'In CI/CD, state must be stored in a remote backend so all pipeline runs share the same state. S3 with DynamoDB locking prevents concurrent modifications. Terraform Cloud provides built-in remote state with locking, run history, and policy enforcement. Never commit state to git because it contains sensitive outputs and causes merge conflicts in team workflows.' },
  { q: 'Why should terraform plan output be reviewed before terraform apply in CI/CD?', options: ['It is not necessary because CI environments are always safe', 'Plan output shows exactly what changes will be made, preventing unintended destructive operations', 'Plan must run first for Terraform to generate the state file', 'Plan output is needed only when using modules'], answer: 1, explanation: 'terraform plan shows the exact diff between desired and actual state: resources to add, change, or destroy. Reviewing the plan before apply prevents accidental deletion of production resources. In pipelines, save the plan with the -out flag and apply that exact plan file to ensure what was reviewed is exactly what gets applied, even if config changed between plan and apply steps.' },
  ];
  ...
  { q: 'How do you structure a Terraform CI/CD pipeline for a multi-environment setup?', a: 'A typical pipeline has stages: validate runs terraform fmt -check and terraform validate to catch syntax errors early. Plan runs terraform plan for each environment and posts the output as a PR comment for review. Apply is triggered on merge to main and runs against the saved plan file. Separate workspaces or directories per environment ensure changes are promoted sequentially from dev to staging to prod. Use OIDC-based authentication from GitHub Actions to cloud providers instead of long-lived credentials stored as CI secrets.' },
  { q: 'How do you detect and handle infrastructure drift in a CI/CD context?', a: 'Drift occurs when the actual infrastructure state differs from what Terraform state records, usually because someone made manual changes via the cloud console. Detect drift by running terraform plan on a schedule and alerting when the plan is non-empty outside of deployment windows. Remediate by either updating the Terraform config to match the manual change and running apply, or running apply to revert to the declared configuration. Enforce a no-manual-changes policy and use cloud SCPs or Azure Policies to restrict console access to prevent drift accumulation.' },
  ];
