...
output "network_summary" {
  ...
  { q: 'What is the purpose of output blocks in Terraform?', options: ['Outputs store configuration defaults for variables', 'Outputs expose values from a configuration for use by other configurations, modules, or for display after apply', 'Outputs define the order in which resources are created', 'Outputs are required for every resource to track its ID'], answer: 1, explanation: 'Output blocks expose values from a Terraform configuration: values are printed to the terminal after apply, available to parent modules as module.name.output_name, and accessible in remote state via the terraform_remote_state data source. Outputs are the public API of a module. Mark sensitive outputs with sensitive = true to prevent values from appearing in logs. Outputs are stored in state and persist between runs.' },
  { q: 'A root module output combines a sensitive resource attribute (e.g. a generated password) with a non-sensitive one (e.g. a resource name) using string interpolation, but the output block itself has no explicit `sensitive = true`. Is the combined output value shown in plain text?', options: ['Yes, since only the output block\'s own explicit setting controls this', 'No — sensitivity automatically propagates from any sensitive input used to compute a value, so an output built from even one sensitive source is treated as sensitive and redacted, regardless of whether the output block itself was explicitly marked', 'Only the sensitive half of the interpolated string is redacted, the rest shows normally', 'It depends on which value appears first in the string interpolation'], answer: 1, explanation: 'Sensitivity in Terraform is contagious through the dependency graph, not just a per-output-block flag: if ANY value feeding into an expression is marked sensitive (a sensitive variable, a resource attribute Terraform knows is sensitive, or another sensitive output), the resulting computed value automatically inherits that sensitivity and gets redacted in plan/apply output — you do not need to remember to add `sensitive = true` explicitly for every output that happens to touch sensitive data downstream. This automatic propagation is a safety net, but it also means an output can become unexpectedly redacted (breaking automation that parses terraform output) if it accidentally references something sensitive it did not need to.' },
  ];
  ...
  { q: 'How do you view Terraform output values after apply?', a: 'Run terraform output to print all non-sensitive output values. Specify an output name to show just that value. The -json flag outputs all values as JSON, which is useful for scripting and piping to other tools. The -raw flag prints the raw string value without surrounding quotes, useful in shell scripts when you want the value without formatting. To see sensitive values: the -json flag includes them in the response. Outputs are stored in state so you can query them without re-running apply. In CI/CD, capture output values with terraform output -raw and pass them to downstream deployment steps.' },
  { q: 'What is the difference between local values and output values in Terraform?', a: 'Local values defined in a locals block are intermediate values used only within the current module to reduce repetition and improve readability. They are never exposed outside the module and do not appear in state outputs. Output values defined in output blocks are explicitly exposed values that parent modules can reference, that appear in state for remote state data sources, and that are printed after apply. Think of locals as private helper variables and outputs as the module public interface. You can reference locals in outputs but outputs cannot be referenced back in locals.' },
  ];
