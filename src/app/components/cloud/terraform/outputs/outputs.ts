...
output "network_summary" {
  ...
  { q: 'How do you mark a Terraform output as sensitive?', options: ['Add a comment next to the output block', 'Set sensitive = true in the output block to prevent the value from being shown in plan and apply output', 'Prefix the output name with secret_ to trigger automatic masking', 'Sensitive outputs must be stored in a separate state file'], answer: 1, explanation: 'Setting sensitive = true in an output block prevents the value from being displayed in terraform plan and apply output, showing a placeholder instead. The value is still stored in state which may be readable. Sensitive values from variables and resources automatically propagate sensitivity to outputs that use them. When referencing a sensitive output in another module, the sensitivity is also propagated to prevent accidental exposure.' },
  ];
