...
# (0 resource changes — only state addresses updated)`,
  ...
  { q: 'What Terraform feature allows you to rename a resource without destroying and recreating it?', options: ['terraform rename command from the CLI', 'The moved block in HCL that maps old resource addresses to new ones', 'terraform state mv command that updates the state file', 'Resource names cannot be changed without destroy and recreate'], answer: 1, explanation: 'The moved block (Terraform 1.1+) declares a rename in code: specify from as the old address and to as the new address. This is the preferred approach because it is reviewable in PRs and reproducible across all environments. Alternatively, terraform state mv old-address new-address updates the state file directly from the CLI. Both prevent destroy and recreate. Use moved blocks in modules to support users upgrading from older versions with different resource names.' },
  { q: 'How do you safely refactor a Terraform configuration that is already managing production resources?', options: ['Delete all resources and recreate them with the new configuration', 'Make incremental changes, review plan output carefully for unexpected destroys, and use moved blocks for renames', 'Refactoring in production is not possible; use a new account instead', 'Run terraform destroy first, then recreate with the new config'], answer: 1, explanation: 'Safe refactoring steps: make small incremental changes per PR rather than large rewrites. Always run terraform plan and carefully review for unexpected destroy or replace operations before applying. Use moved blocks for any resource renames or module restructuring. Use terraform state mv for changes that cannot be expressed as moved blocks. Test in a non-production environment first. Add lifecycle prevent_destroy to critical resources as a safety net during refactoring.' },
  ];
  ...
  { q: 'How do you extract resources from a root module into a child module without destroying them?', a: 'Use moved blocks with module paths in the from and to fields. Specify the old root module resource address in from and the new path inside the module in to, for example from aws_instance.web to module.web.aws_instance.main. Steps: create the new module directory with the resource block at the new address, add the module block to the root module, add the moved block mapping old to new, run terraform plan to verify no destroy and create pairs appear, then apply. Remove the moved block in a follow-up PR after all environments have been updated.' },
  { q: 'What is the lifecycle meta-argument and when should you use prevent_destroy?', a: 'The lifecycle block controls Terraform resource behavior. create_before_destroy = true creates a replacement before destroying the old resource, useful for zero-downtime updates to immutable resources like ACM certificates or launch templates. prevent_destroy = true causes Terraform to error if any plan would destroy the resource, useful for databases, S3 buckets with data, and other stateful resources you never want accidentally deleted. ignore_changes allows ignoring specific attribute changes in future plans, useful for attributes managed externally like tags added by other tools. replace_triggered_by forces recreation when another referenced resource changes.' },
  ];
