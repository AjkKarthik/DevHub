...
jobs:
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A team enforces strict IAM policies that deny console-based Modify/Create/Delete on Terraform-managed resources, yet drift still appears in the next terraform plan. What is a common source of drift that IAM restrictions alone cannot prevent?', a: 'IAM restrictions block HUMAN console/CLI changes, but drift also comes from sources IAM cannot touch: another automated system (an autoscaler adjusting instance counts, a cloud-native auto-remediation tool, a separate CI/CD pipeline that also touches the same resources) making changes through its own legitimate service role; a cloud provider applying a default/managed change on its own (e.g. AWS auto-assigning a new default security group rule); or resources with attributes that naturally change outside Terraform\'s control (an auto-generated certificate ARN rotating, a load balancer\'s DNS name changing during a provider-side migration). IAM lockdown addresses the "rogue engineer changed something in the console" case well, but a complete drift-prevention strategy also needs to account for other automated actors touching the same infrastructure.' },
