...
# (Run this BEFORE moving — once removed from state, moved block is not needed for it)`,
  ...
  qna: QnaItem[] = [
    ...
    { q: 'After restoring an older version of state from S3 versioning following a corruption incident, terraform plan shows a large diff proposing to recreate resources that were created AFTER the restored state snapshot was taken. What is the safe way to reconcile this, versus just letting apply run?', a: 'Blindly running apply against a restored-but-stale state is dangerous — Terraform would try to "fix" the perceived drift by destroying and recreating resources that actually exist and are fine, just unknown to the older state snapshot; the safe reconciliation path is first identifying exactly which resources were created after the restored snapshot (checking cloud provider audit logs or CloudTrail for creation timestamps), then using `terraform import` (or import blocks) to re-add just those specific resources into the restored state before ever running apply, so the state accurately reflects everything that actually exists before Terraform is allowed to reconcile anything — treating the restored state as a starting point to patch up, not a final source of truth to apply immediately.' },
