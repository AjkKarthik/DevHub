...
output "open_ports" { value = keys(local.all_rules) }`,
  ...
  { q: 'A resource is created with for_each (not count), using a map keyed by environment name. Can you use the [*] splat expression to extract an attribute from every instance the way you would with a count-based resource?', options: ['Yes, splat works identically regardless of whether count or for_each was used', 'No — for_each-created resources are addressed by their map/set KEY (e.g. aws_instance.web["prod"]), not by a numeric index, so there is no ordered list for [*] to splat over; you need a for expression like [for k, v in aws_instance.web : v.id] instead', 'Splat only works on resources with exactly one instance', 'for_each resources must be converted to count before splat can be used'], answer: 1, explanation: 'The splat operator [*] is fundamentally a shorthand for iterating over an ordered, INDEXED collection — which is exactly what count produces (aws_instance.web[0], [1], [2]...). A for_each-created resource is instead a MAP keyed by arbitrary string or set-member keys (aws_instance.web["prod"], aws_instance.web["staging"]), which has no inherent numeric ordering for [*] to iterate over. Extracting attributes across all instances of a for_each resource requires an explicit for expression that can iterate over the map\'s key-value pairs, which is one of the concrete reasons the docs recommend deciding between count and for_each based partly on how you\'ll need to reference the resulting instances downstream.' },
