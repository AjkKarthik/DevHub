...
terraform {
  ...
  { q: 'A team uses partial backend configuration with a different backend.hcl per environment (dev.backend.hcl, prod.backend.hcl). A CI job runs `terraform init -backend-config=dev.backend.hcl`, then later in the same pipeline stage someone re-runs init with `-backend-config=prod.backend.hcl` without an intervening `terraform init -reconfigure`. What happens?', a: 'Terraform detects that the backend configuration has changed since the last init and, by default, prompts interactively asking whether to migrate existing state to the new backend — which is exactly the wrong behavior in an unattended CI job (it either hangs waiting for input or fails outright depending on how the pipeline handles stdin). Switching backends mid-pipeline requires either `-reconfigure` (skip migration, just point at the new backend fresh) or `-migrate-state` (explicitly migrate) passed non-interactively, and in practice most teams avoid this entirely by running dev and prod applies as fully separate pipeline stages or jobs, each starting from a clean working directory, rather than reusing one working directory across backend switches within a single job.' },
  ];
