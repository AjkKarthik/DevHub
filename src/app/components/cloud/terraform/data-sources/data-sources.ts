...
output "subnet_ids" { value = aws_subnet.public[*].id }`,
  ...
  { q: 'A data source queries an AWS AMI that gets deleted from AWS between `terraform plan` and `terraform apply` (a race condition in a CI pipeline with a delay between the two steps). What happens?', options: ['The apply silently uses a cached value from the plan file and succeeds', 'The apply re-queries the data source (since data sources are re-resolved at apply time too, not just cached from plan) and fails if the AMI no longer exists, since the filter now matches nothing', 'Terraform automatically falls back to the next-most-recent AMI', 'The resource referencing the AMI is destroyed automatically'], answer: 1, explanation: 'Unlike resources (whose planned changes are locked into the saved plan file and applied as-is), data sources are re-evaluated by querying the provider API again at apply time — the plan\'s data source result is not a guaranteed snapshot. If external state changes between plan and apply (the AMI gets deregistered, a filter that matched one result now matches zero or several), the apply step re-runs the query and can fail with a "no matching AMI found" error even though the plan succeeded moments earlier. This is a real source of CI flakiness for pipelines with any delay between plan and apply, and a case for either applying promptly after planning or pinning to more stable, less time-sensitive identifiers where possible.' },
  { q: 'A data source depends on a resource created earlier in the same apply (its filter uses an attribute from a resource Terraform is also creating in this run). Can Terraform resolve that data source during the plan phase?', options: ['Yes, always — Terraform pre-computes every resource attribute before planning any data source', 'No — if the data source depends on a resource that does not exist yet, Terraform cannot query the real API for it during plan and must defer that data source\'s evaluation to apply time, which can make dependent downstream values show as "known after apply"', 'The plan fails immediately with a circular dependency error', 'Terraform silently skips the data source and proceeds without its value'], answer: 1, explanation: 'A data source ultimately calls a real provider API to fetch live values — if it depends on a resource that Terraform has not created yet (because that resource is also part of this same apply), there is genuinely nothing to query yet, so Terraform cannot resolve the data source at plan time and must push its evaluation to apply time instead. This shows up in `terraform plan` output as values that are "(known after apply)" further down the dependency chain, and is a normal, expected consequence of mixing newly-created resources with data sources that depend on them within the same configuration — not a bug, but worth understanding so an unexpectedly large "known after apply" block in a plan doesn\'t cause alarm.' },
  ];
