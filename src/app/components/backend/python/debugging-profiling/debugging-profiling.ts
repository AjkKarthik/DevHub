...
print(f"Slow: {t_slow:.3f}s  Fast: {t_fast:.3f}s  Speedup: {speedup:.0f}x")`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does cProfile.Profile().enable() / .disable() measure that timeit does not?', options: ['Wall clock time only', 'Per-function call counts and cumulative/per-call CPU time across the entire call graph', 'Memory usage per function', 'Only the top-level function'], answer: 1, explanation: 'cProfile instruments every function call and records number of calls, total time, and time per call. This reveals the full call graph — showing where time is actually spent in nested functions. timeit measures only the total elapsed time of a snippet repeated N times. Use timeit for micro-benchmarks; cProfile for identifying bottlenecks in a real workload.' },
    { q: 'What is the difference between logging.exception() and logging.error()?', options: ['They are identical', 'logging.exception() logs at ERROR level AND appends the current exception traceback automatically', 'logging.exception() raises the exception again', 'logging.error() includes the traceback; exception() does not'], answer: 1, explanation: 'logging.exception(msg) calls logging.error(msg, exc_info=True) — it logs the message AND attaches the current exception traceback. Use it inside except blocks to log the full error context. logging.error() logs a message at ERROR level but does NOT include the traceback unless you pass exc_info=True explicitly.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the difference between Python\'s built-in cProfile and a sampling profiler like py-spy?', a: 'cProfile is a deterministic profiler — it instruments every function call, recording exact call counts and cumulative time for each, giving precise per-function statistics but with measurable overhead that can distort timing, especially for code with many small, frequent function calls. py-spy is a sampling profiler that periodically interrupts the running process (without any code modification or instrumentation) to capture a stack trace snapshot, producing statistically representative (not exact) timing data with near-zero overhead — making it safe to attach to a live production process without measurably affecting its performance.' },
    { q: 'Why is pdb.set_trace() (or the breakpoint() built-in) often more effective for debugging than scattering print() statements?', a: 'print() debugging requires guessing in advance what information might be useful, re-running the program after every change to add more prints, and manually tracing program flow from static output. breakpoint() drops you into an interactive debugger at that exact point in execution, letting you inspect ANY variable in scope, step through subsequent lines one at a time, evaluate arbitrary expressions, and even modify variables on the fly — all without needing to predict ahead of time exactly what you will need to inspect, since you can explore interactively once execution is paused.' },
    { q: 'What does memory_profiler (or tracemalloc) help diagnose that a CPU profiler like cProfile cannot?', a: 'cProfile measures TIME spent in each function, telling you nothing about MEMORY consumption — a function could be fast but allocate enormous amounts of memory that get garbage collected before the next profiling sample, hiding the issue entirely from a CPU profiler. tracemalloc (built into the standard library since 3.4) and memory_profiler track memory allocations line-by-line or per-snapshot, letting you identify which specific code paths are responsible for excessive memory growth or leaks — essential for diagnosing out-of-memory crashes or gradual memory growth in long-running services that a CPU profile would show as perfectly fine.' },
  ];
