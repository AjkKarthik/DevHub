...
# find_or_raise(repo, 99)       # KeyError`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does the new type statement (PEP 695, Python 3.12+) let you write instead of a module-level TypeVar assignment?', options: ['Nothing new — it is purely cosmetic sugar with no functional difference', 'type ListOrSet[T] = list[T] | set[T] declares a generic alias with the type parameter scoped to the alias, and class Stack[T]: works directly without Generic[T]', 'It removes the need for type hints entirely', 'It only works for function return types, not classes or aliases'], answer: 1, explanation: 'PEP 695 (Python 3.12+) lets you write type ListOrSet[T] = list[T] | set[T] instead of the older T = TypeVar("T"); ListOrSet = list[T] | set[T] — the type parameter T is scoped to the alias/class itself rather than a separately-declared module-level TypeVar object that has to be reused consistently across related generics. It also lets you write class Stack[T]: directly instead of class Stack(Generic[T]):.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How can you get typing.get_type_hints() to raise an error at import time if a type hint references an undefined name?', a: 'typing.get_type_hints(some_function_or_class) resolves all annotations (including string/deferred ones from __future__ import annotations) against the module\'s global namespace, and raises a NameError immediately if an annotation references a name that does not exist — this is how frameworks like Pydantic and dataclasses catch typos in forward-referenced type names at class-definition or first-use time rather than silently accepting a broken annotation that only fails if a type checker happens to be run.' },
    { q: 'What is a NewType, and how does it differ from a plain type alias like UserId = int?', a: 'NewType("UserId", int) creates a distinct type recognized by static type checkers — a UserId value can be used anywhere an int is expected (since it IS an int at runtime, with zero overhead), but a plain int cannot be silently passed where a UserId is expected without an explicit UserId(x) wrap, catching bugs like passing a raw product_id where a user_id was intended. A plain alias (UserId = int) is just another name for int with no such distinction — mypy treats UserId and int as fully interchangeable, providing no protection against mixing up semantically different integer IDs.' },
