...
def retry(max_attempts=3, base_delay=1.0, exceptions=(Exception,)):
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is a closure in Python, and what is the classic "late binding" pitfall with closures in loops?', a: 'A closure is a nested function that captures variables from its enclosing scope, retaining access to them even after the outer function has returned. The late binding pitfall: creating multiple closures inside a loop that reference the loop variable (e.g., a list of lambdas appending lambda: i inside for i in range(3)) all share the SAME variable cell, so by the time any closure is called, the loop variable holds its final value — all closures print 2, not 0, 1, 2 as might be expected. Fix by capturing the value as a default argument: lambda i=i: i, which binds the current value at definition time.' },
    { q: 'What is the difference between *args and **kwargs, and when would you use both in the same function signature?', a: '*args collects any extra positional arguments into a tuple, while **kwargs collects any extra keyword arguments into a dictionary — both let a function accept a variable, unknown-in-advance number of arguments. They are commonly combined in decorator wrapper functions and base-class methods designed for extension, where the wrapper needs to transparently forward whatever arguments the caller provides to an inner function without needing to know its exact signature: def wrapper(*args, **kwargs): return original_func(*args, **kwargs).' },
    { q: 'Why are mutable default arguments (e.g., def f(items=[])) considered a common Python bug source?', a: 'Default argument values are evaluated exactly once, at function definition time, not on every call — so a mutable default like a list or dict is created once and then shared and potentially mutated across every call that does not explicitly pass its own value, leading to surprising state leaking between unrelated calls. The standard fix is to use None as the default and create a fresh mutable object inside the function body: def f(items=None): items = items if items is not None else [].' },
  ];
