...
def retry(max_attempts=3, base_delay=1.0, exceptions=(Exception,)):
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What does a closure actually capture — the variable\'s value, or the variable itself?', a: 'A Python closure captures the variable itself (technically, a reference to the cell holding it), not a snapshot of its value at closure-creation time. This is why closures see later mutations of an enclosing variable: def make(): x = 1; def inner(): return x; x = 2 (still before inner is called); return inner — calling the returned inner() returns 2, not 1, because inner looks up x in the enclosing scope fresh each time it runs, and by then x has changed.' },
    { q: 'What order must *args, **kwargs, and keyword-only parameters appear in a function signature?', a: 'The required order is: positional parameters, then *args (or a bare * for keyword-only-without-varargs), then keyword-only parameters, then **kwargs — e.g. def fn(a, b, *args, c, d=1, **kwargs). Putting **kwargs before *args, or a regular positional parameter after *args without it being keyword-only, is a SyntaxError. This ordering exists because *args greedily absorbs all remaining positional arguments, so anything meant to be reachable only by keyword must come after it.' },
