...
print(sum_primes_below(100))   # 1060`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does a set comprehension {x for x in it} deduplicate that a list comprehension does not?', options: ['Nothing — they behave identically', 'Duplicate values are automatically collapsed to a single entry, and the result is unordered', 'Only string values are deduplicated', 'Set comprehensions raise an error on duplicate values'], answer: 1, explanation: 'A set comprehension builds a set, so any values that compare equal are collapsed to one entry — {x % 3 for x in range(10)} produces {0, 1, 2}, not 10 values. Unlike a list comprehension, the result has no guaranteed order and requires elements to be hashable.' },
    { q: 'Why can a generator only be iterated once, and what happens if you try to reuse it?', options: ['Generators can be iterated any number of times like a list', 'A generator is a single-use iterator — once exhausted (StopIteration raised), calling next() or looping over it again yields nothing', 'Reusing an exhausted generator restarts it from the beginning', 'It raises a TypeError on the second iteration attempt'], answer: 1, explanation: 'A generator object tracks its own execution position internally; once it runs to completion and raises StopIteration, that position cannot be reset. A second for loop over the same (already-exhausted) generator object silently produces zero iterations rather than erroring or restarting — to iterate again you must call the generator function a second time to get a fresh generator object.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you write a nested comprehension to flatten a list of lists, and how does it read left-to-right?', a: '[x for row in matrix for x in row] flattens a 2D list — the clauses read in the same order you would write equivalent nested for loops: for row in matrix: for x in row: collect x. It is easy to misread as the innermost loop coming first; mentally rewrite it as stacked for loops top-to-bottom in the order they appear to avoid mistakes. Adding a condition (if x > 0) always goes at the end, filtering the innermost yielded value.' },
