...
print(sum_primes_below(100))   # 1060`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the memory difference between a list comprehension and a generator expression?', options: ['They use identical memory', 'List comprehensions create the entire list in memory; generators yield one item at a time', 'Generators use more memory than lists', 'List comprehensions are lazy like generators'], answer: 1, explanation: '[x for x in it] materialises the entire list in memory immediately — for 1M items, that\'s 1M objects allocated. (x for x in it) creates a generator object that yields one item at a time — O(1) memory regardless of input size. Use generators for large data streams (reading files, processing huge APIs), lists when you need to iterate multiple times or access by index.' },
    { q: 'What does "yield from" do in a generator function?', options: ['Yields a list all at once', 'Delegates to a sub-generator, transparently forwarding values and exceptions', 'Converts a list into a generator', 'Raises StopIteration after the first yield'], answer: 1, explanation: 'yield from iterable forwards each item from the sub-iterable one at a time — equivalent to for item in iterable: yield item, but also correctly forwards .send() and .throw() into the sub-generator. It is the clean way to compose generators (e.g., flatten nested iterables) and is required to properly delegate to sub-generators in send/throw protocols.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the key difference between a list comprehension and a generator expression in terms of memory and execution?', a: 'A list comprehension ([x*2 for x in range(1000000)]) eagerly computes and stores every element in memory immediately, building the full list before returning. A generator expression ((x*2 for x in range(1000000)), with parentheses instead of brackets) is lazy — it produces a generator object that computes each value on demand as you iterate, never holding the full sequence in memory at once. Use generator expressions for large or unbounded sequences where you only need to iterate once, and list comprehensions when you need random access, multiple iterations, or list-specific methods (len, indexing, sorting).' },
    { q: 'What does the yield keyword do differently from return inside a function, and what makes a function a generator?', a: 'Any function containing a yield statement automatically becomes a generator function — calling it does not execute the function body immediately; instead it returns a generator object. Each time next() is called on that generator (implicitly via a for loop), execution resumes from where it last left off (right after the previous yield), runs until the next yield, and pauses again, preserving all local variable state between resumptions. return inside a generator (with no value, or with a value accessible via StopIteration.value) ends iteration, raising StopIteration, rather than returning a regular value to a normal caller.' },
    { q: 'When should you use a generator function instead of a class implementing the iterator protocol (__iter__/__next__) manually?', a: 'A generator function is far more concise for most iteration logic — Python automatically implements __iter__ and __next__ for you based on the yield statements, with state management (where you are in the iteration) handled implicitly by the function\'s suspended execution frame instead of manually tracked instance attributes. Implement the iterator protocol manually with a class only when you need more control — for example, supporting multiple independent iterators over the same data structure simultaneously (each with separate state), or when the iteration logic needs to integrate with other class behavior beyond just producing a sequence.' },
  ];
