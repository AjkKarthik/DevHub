...
def test_multiply(a, b, expected): assert multiply(a, b) == pytest.approx(expected)`
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Why are Python virtual environments (venv) essential, and what problem do they solve?', a: 'Without a virtual environment, all installed packages go into one shared, global (or per-user) Python installation — meaning two projects requiring different, incompatible versions of the same library cannot coexist, and accidentally installing or upgrading a package for one project can silently break another unrelated project relying on the older version. A virtual environment (created via python -m venv .venv) creates an isolated, project-specific directory with its own independent set of installed packages, so each project\'s dependencies are fully isolated from every other project and from the system Python installation.' },
