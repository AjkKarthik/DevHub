...
# gs.fit(X_train, y_train); print(gs.best_params_)`
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the difference between StandardScaler and MinMaxScaler, and when would you choose one over the other?', a: 'StandardScaler transforms features to zero mean and unit variance (values can be any real number, typically roughly -3 to 3) and is robust when the data roughly follows a normal-ish distribution — the default choice for linear models, SVMs, and neural networks. MinMaxScaler rescales values into a fixed range, usually [0, 1], preserving the shape of the original distribution exactly but being much more sensitive to outliers (a single extreme value compresses everything else toward 0). Prefer MinMaxScaler when you need bounded output (e.g. feeding into an algorithm that assumes a fixed range, or for image pixel data), and StandardScaler for most general-purpose modeling.' },
