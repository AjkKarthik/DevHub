...
def load_products(data: list[dict]) -> list[Product]:
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the fundamental difference between a dataclass and a Pydantic model in terms of validation?', a: 'A standard library @dataclass only generates boilerplate (__init__, __repr__, __eq__) based on type-hinted fields — it does NOT validate that values match their declared types at runtime; assigning a string to a field hinted as int succeeds silently. A Pydantic BaseModel actively validates and coerces input data against its field types at instantiation time, raising a ValidationError for mismatched or missing data — making Pydantic the better choice for parsing untrusted external input (API request bodies, config files), while dataclasses suit simple, already-trusted internal data containers.' },
    { q: 'Why does @dataclass(frozen=True) matter, and what does it actually prevent?', a: 'frozen=True makes dataclass instances immutable after creation — attempting to reassign any attribute (instance.field = new_value) raises a FrozenInstanceError instead of silently succeeding. This is useful for value-object-style data (coordinates, configuration snapshots) where unintended mutation after creation should be a hard error rather than a possible source of subtle bugs, and frozen dataclasses with all-hashable fields also become hashable themselves, making them usable as dict keys or set members, unlike regular mutable dataclasses.' },
