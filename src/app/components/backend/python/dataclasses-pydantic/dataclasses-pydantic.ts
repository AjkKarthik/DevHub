...
def load_products(data: list[dict]) -> list[Product]:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'How do you add a computed field to a Pydantic v2 model?', options: ['Use a @property', 'Use @computed_field from pydantic — it appears in model_dump() and schema unlike @property', 'Assign it in __init__', 'Use Field(computed=True)'], answer: 1, explanation: '@computed_field makes a property appear in model_dump() output and the JSON schema — useful for computed display values (full_name from first + last). Plain @property is still usable but is invisible to serialisation. In Pydantic v1, validators with @validator could output computed fields; v2\'s @computed_field is the clean replacement.' },
    { q: 'What is the difference between model.model_dump() and model.dict() in Pydantic v2?', options: ['They are identical', 'model_dump() is the v2 API; dict() is deprecated but still works in v2', 'dict() serialises to JSON; model_dump() to a Python dict', 'model_dump() includes nested models; dict() does not'], answer: 1, explanation: 'Pydantic v2 renamed dict() to model_dump(), json() to model_dump_json(), and schema() to model_json_schema(). The old names are deprecated in v2 and may be removed in v3. model_dump() returns a Python dict; model_dump_json() returns a JSON string. The mode="json" argument to model_dump() converts all values to JSON-compatible types.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Can you mix a dataclass and Pydantic in the same codebase, and when would that make sense?', a: 'Yes — a common pattern is using Pydantic models at the "edges" of a system (API request/response bodies, config loading) where untrusted or serialized data needs validation and coercion, and plain dataclasses for internal, already-validated data structures passed between functions within a trusted boundary, where the extra validation overhead on every construction is unnecessary. Pydantic also ships pydantic.dataclasses.dataclass, a drop-in decorator that adds Pydantic validation to a dataclass-style class if you want dataclass syntax with validation.' },
    { q: 'What happens if you try to add a new field to a frozen dataclass instance after creation, versus mutating a mutable field inside it?', a: 'frozen=True only prevents REASSIGNING or adding attributes directly on the instance (instance.x = 5 raises FrozenInstanceError) — it does NOT make the contents of mutable fields immutable. A frozen dataclass with a field items: list can still have its list mutated in place (instance.items.append(x) succeeds, since that mutates the list object, not the instance attribute itself). True deep immutability requires using immutable field types (tuple instead of list, frozenset instead of set) in addition to frozen=True.' },
    { q: 'How do Pydantic models integrate with FastAPI to provide automatic request validation and OpenAPI documentation?', a: 'FastAPI uses Pydantic models as type hints on path operation function parameters to automatically validate incoming JSON request bodies against the model\'s field types and constraints, returning a structured 422 error response with field-level details if validation fails — all without any manual validation code. The same Pydantic model definitions are also introspected to automatically generate the OpenAPI/Swagger schema and interactive documentation, meaning the model serves triple duty as a runtime validator, a documentation source, and (via response_model) a response serializer, keeping all three in sync from one source of truth.' },
  ];
