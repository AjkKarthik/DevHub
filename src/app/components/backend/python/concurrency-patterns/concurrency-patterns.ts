...
class CircuitBreaker:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between concurrent.futures.ThreadPoolExecutor and ProcessPoolExecutor?', options: ['They are identical', 'ThreadPoolExecutor uses OS threads (GIL limited, good for I/O); ProcessPoolExecutor uses OS processes (true parallelism, good for CPU)', 'ProcessPoolExecutor supports async; ThreadPoolExecutor does not', 'ThreadPoolExecutor creates one thread per task; ProcessPoolExecutor reuses workers'], answer: 1, explanation: 'ThreadPoolExecutor: shares memory and GIL across workers — threads run concurrently (switching on I/O) but not in parallel for CPU. Great for I/O-bound tasks. ProcessPoolExecutor: spawns separate Python processes with independent GILs — true parallel execution. Better for CPU-bound computation. Cost: process creation overhead, and all data must be picklable for inter-process communication.' },
    { q: 'What is an asyncio Event and how does it differ from a Condition?', options: ['They are identical', 'Event is a simple signal flag (set/wait/clear); Condition adds a predicate and allows waking a subset of waiters', 'Condition is deprecated; Event is the replacement', 'Event is for threads; Condition is for coroutines'], answer: 1, explanation: 'asyncio.Event is a single-bit flag: set() marks it, wait() blocks until set, clear() resets it. Simple "something happened" signals. asyncio.Condition wraps a lock and supports async with condition: await condition.wait_for(predicate), waking waiters only when a specific condition holds. Use Event for simple signals; Condition when waiters need to re-check a shared state after being woken.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'When should you choose multiprocessing over threading or asyncio for a Python concurrency problem?', a: 'Choose multiprocessing for CPU-bound work (heavy computation, data processing, image manipulation) where you need genuine parallel execution across multiple CPU cores — since each process has its own Python interpreter and GIL, multiprocessing is the only standard-library option that achieves true CPU parallelism. Choose threading or asyncio instead for I/O-bound work (network requests, file I/O, database queries) where the bottleneck is waiting, not computing — multiprocessing\'s overhead (separate memory spaces, inter-process communication serialization cost) makes it a poor fit for I/O-bound tasks where threading or asyncio achieve the same concurrency benefit far more cheaply.' },
    { q: 'What is a race condition, and why does Python\'s GIL not automatically prevent all race conditions in multi-threaded code?', a: 'A race condition occurs when the correctness of a program depends on the unpredictable timing/interleaving of concurrent operations accessing shared state. While the GIL ensures only one thread executes Python bytecode at any instant, many common operations (like x += 1) compile to MULTIPLE bytecode instructions (read x, add 1, write x) — the GIL can switch threads between these instructions, allowing two threads to interleave their read-modify-write sequences and lose an update. Protect shared mutable state with explicit locks (threading.Lock) regardless of the GIL\'s presence, since the GIL only guarantees atomicity at the single-bytecode-instruction level, not at the logical-operation level.' },
    { q: 'What is the producer-consumer pattern, and how does Python\'s queue.Queue (or asyncio.Queue) implement it safely?', a: 'The producer-consumer pattern decouples work generation (producers adding items to a shared queue) from work processing (consumers removing and processing items), allowing each side to operate at its own pace without direct coordination. queue.Queue (thread-safe) and asyncio.Queue (coroutine-safe) provide built-in, correctly synchronized put() and get() operations that handle the locking/blocking semantics internally — producers block on put() if the queue is full (when a maxsize is set, providing natural backpressure), and consumers block on get() if the queue is empty, without requiring manual lock management from application code.' },
  ];
