...
def regional_summary(df: pd.DataFrame) -> pd.DataFrame:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does np.broadcasting allow in NumPy?', options: ['Running NumPy operations across multiple CPUs', 'Arithmetic between arrays of different but compatible shapes without copying data', 'Sending arrays across a network', 'Converting between dtypes automatically'], answer: 1, explanation: 'Broadcasting stretches smaller arrays to match larger ones for element-wise operations. Example: arr (1000×3) + np.array([1,2,3]) adds [1,2,3] to every row without creating a full (1000×3) copy of the row array. Rules: dimensions are compared right-to-left; a dimension of 1 can broadcast to any size; mismatches raise ValueError.' },
