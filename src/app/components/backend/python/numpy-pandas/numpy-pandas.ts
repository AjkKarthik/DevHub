...
def regional_summary(df: pd.DataFrame) -> pd.DataFrame:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does df.pivot_table() do differently from df.groupby()?', options: ['They are functionally identical', 'pivot_table reshapes grouped results into a wide, spreadsheet-style table with group keys as row/column labels; groupby returns grouped data that must be manually reshaped', 'groupby only works on numeric columns', 'pivot_table cannot use an aggregation function'], answer: 1, explanation: 'df.groupby("month")["sales"].sum() gives a long-format Series indexed by month. df.pivot_table(values="sales", index="region", columns="month", aggfunc="sum") reshapes the same kind of aggregation into a wide table with regions as rows and months as columns — pivot_table is essentially groupby + unstack combined into one convenient call, ideal for spreadsheet-style cross-tabulation reports.' },
    { q: 'What is the purpose of pd.cut() vs pd.qcut()?', options: ['cut() creates categories; qcut() creates numeric bins', 'cut() bins by fixed value ranges; qcut() bins by quantile (equal-frequency bins)', 'They are identical', 'qcut() only works with datetime data'], answer: 1, explanation: 'pd.cut(series, bins=[0,18,65,np.inf], labels=["youth","adult","senior"]) creates bins by value boundaries. pd.qcut(series, q=4) creates quartile bins where each bin has roughly the same number of observations. Use cut for domain-meaningful ranges (age groups, price tiers); use qcut when you need equal-sized groups for statistical analysis.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Why are NumPy array operations significantly faster than equivalent pure-Python loops over lists?', a: 'NumPy arrays store homogeneous data in contiguous blocks of memory with a fixed, known element type, and vectorized operations (array_a + array_b) are implemented in compiled C code that operates on the entire array in a tight, optimized loop without the per-element overhead of Python\'s interpreter (type checking, dynamic dispatch, object boxing) that a Python for loop incurs on every iteration. This "vectorization" — expressing operations on whole arrays rather than element-by-element — is the core reason NumPy can be 10-100x faster than equivalent pure-Python loops for numerical work.' },
    { q: 'What is the difference between a pandas DataFrame view and a copy, and why does the SettingWithCopyWarning matter?', a: 'Some pandas operations (certain slicing operations) return a VIEW into the original DataFrame\'s underlying data, while others return a COPY — modifying a view can unexpectedly mutate the original DataFrame, while modifying a copy has no effect on the original, and pandas does not always make it obvious which one a given operation returned. The SettingWithCopyWarning is pandas\'s attempt to flag situations where you are assigning to what MIGHT be a view (ambiguous chained indexing like df[df.x > 0][\'y\'] = 1), since the resulting behavior is unreliable — the recommended fix is to use .loc[] for explicit, unambiguous indexed assignment, or .copy() when you intentionally want an independent copy.' },
    { q: 'When should you use pandas .apply() versus a vectorized operation, and why does .apply() often hurt performance?', a: '.apply() runs a Python function once per row or column, falling back to Python-level interpreted execution for each call — this reintroduces exactly the per-element interpreter overhead that vectorized NumPy/pandas operations are designed to avoid, often making .apply() 10-50x slower than an equivalent vectorized expression for simple operations. Use .apply() only when the operation genuinely cannot be expressed as a vectorized pandas/NumPy operation (complex conditional logic, calling an external API per row) — for anything expressible as arithmetic, comparisons, or built-in pandas/NumPy functions, prefer the vectorized form (df[\'col\'] * 2 instead of df[\'col\'].apply(lambda x: x * 2)).' },
  ];
