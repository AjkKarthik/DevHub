...
app.conf.beat_schedule = {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between a Celery chain and a Celery group?', options: ['They are identical primitives with different names', 'chain runs tasks sequentially, passing each result to the next; group runs tasks in parallel independently', 'group runs tasks sequentially; chain runs them in parallel', 'chain requires a result backend; group does not'], answer: 1, explanation: 'chain(t1.s(), t2.s(), t3.s()) runs tasks in sequence — t1\'s return value becomes t2\'s first argument, and so on. group(t1.s(), t2.s(), t3.s()) fires all tasks in parallel with no data dependency between them and no automatic aggregation. chord combines the two ideas: a group followed by a callback once the whole group finishes.' },
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is task_acks_late and how does it interact with idempotency?', a: 'By default, Celery acknowledges (removes from the queue) a task as soon as a worker STARTS it, not when it finishes — so if the worker crashes mid-task, the task is lost, not retried. Setting task_acks_late=True instead acknowledges only after the task completes (success or failure), so a crashed worker causes the broker to redeliver the task to another worker. This trades "at-most-once" for "at-least-once" delivery — which is safer against lost work, but only safe to enable if the task is idempotent, since it can now genuinely run twice for the same input.' },
