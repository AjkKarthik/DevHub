...
app.conf.beat_schedule = {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does the chord() primitive do in Celery?', options: ['Runs a list of tasks sequentially', 'Runs a group of tasks in parallel and calls a callback task when ALL are done', 'Sends tasks to multiple brokers', 'Chains tasks with shared context'], answer: 1, explanation: 'chord(group, callback) runs group tasks in parallel and calls callback with the list of results when all tasks complete. Example: chord([process.s(f) for f in files])(aggregate.s()) processes all files in parallel then aggregates results. Chord requires a result backend to track completions. Compare to chain (sequential) and group (parallel without callback).' },
    { q: 'How do you implement exponential backoff for Celery task retries?', options: ['Pass countdown=self.request.retries**2 to self.retry()', 'Use retry_backoff=True in the @app.task decorator', 'Write a custom retry loop in the task', 'Celery does not support backoff'], answer: 1, explanation: 'Adding retry_backoff=True to @app.task or @shared_task enables exponential backoff: each retry waits 2^retry_number seconds (1s, 2s, 4s...). Combine with max_retries=5 and retry_backoff_max=3600 (cap at 1 hour). You can also pass countdown=2**self.request.retries manually to self.retry(exc=exc) for finer control.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the role of a message broker (like Redis or RabbitMQ) in a Celery architecture, and why can Celery not work without one?', a: 'Celery itself is a task execution framework, not a message queue — it relies on a separate broker (RabbitMQ, Redis, Amazon SQS) to actually transport task messages from producers (your Django/Flask app calling task.delay()) to consumers (Celery worker processes). The broker handles message persistence, delivery guarantees, and queuing so that workers can be added, removed, or restarted independently of the application that enqueues tasks, decoupling task submission from task execution entirely.' },
    { q: 'Why should Celery tasks be designed to be idempotent, and what happens if they are not?', a: 'Celery, depending on configuration, can redeliver a task more than once — for example, if a worker crashes after starting a task but before acknowledging completion, or due to network issues with the broker, the same task message may be delivered and executed again. A non-idempotent task (e.g., one that charges a credit card or sends an email without a duplicate check) could cause real-world side effects to happen twice. Idempotent design — using a unique task ID to check-and-skip already-processed work, or making the operation naturally safe to repeat (an UPSERT instead of an INSERT) — protects against this redelivery scenario.' },
    { q: 'What is the difference between Celery task retries and Celery Beat scheduled tasks?', a: 'Task retries (configured via autoretry_for, max_retries, and retry_backoff on a task) handle a SINGLE task execution that failed, automatically re-queuing it for another attempt after a delay, typically used for transient failures like a temporarily unavailable external API. Celery Beat is an entirely separate scheduler process that periodically triggers NEW task executions on a defined schedule (cron-like, e.g., "run this task every day at midnight") — it has nothing to do with retrying failures, but rather with recurring, time-based task initiation similar to a distributed cron daemon integrated with the task queue.' },
  ];
