...
def summarise_results(counts: dict[str, int], top_n: int = 5) -> list[tuple[str, int]]:
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What happens if a daemon thread is in the middle of writing to a file when the main thread exits?', a: 'The write is abruptly cut off — daemon threads are killed the instant the main thread exits, with no chance to run cleanup code, finally blocks, or context manager __exit__ methods. This can leave a partially-written file, an unflushed buffer, or a corrupted output. This is precisely why daemon=True should only be used for threads whose work is safe to abandon at any point (polling loops, cache warmers), never for threads performing writes that need to complete atomically or need guaranteed cleanup.' },
    { q: 'Why does Python\'s threading module still provide real concurrency benefits for I/O-bound code despite the GIL?', a: 'The GIL is released automatically whenever a thread performs a blocking I/O operation (file reads, network calls, time.sleep) — while one thread is waiting on I/O, the GIL becomes available for another thread to acquire and run Python bytecode, meaning multiple I/O-bound threads can genuinely overlap their waiting periods even though only one thread executes actual Python code at any given instant. This makes threading effective for I/O-bound concurrency (downloading multiple URLs simultaneously, handling multiple slow database queries) even though it provides no benefit at all for CPU-bound work, which never releases the GIL during pure computation.' },
    { q: 'What is the overhead cost of using multiprocessing compared to threading, and why does it matter for choosing between them?', a: 'Each new process spawned via multiprocessing requires the OS to create an entirely separate memory space and (depending on the start method) potentially re-import and re-initialize the Python interpreter and application modules from scratch, which is significantly more expensive than creating a new thread within the existing process — meaning multiprocessing has a real "cost of entry" that is only worth paying when the parallel work itself is substantial enough (genuinely CPU-bound, running for a meaningful duration) to outweigh that fixed startup overhead. Spawning a new process for a tiny, fast task can actually be slower overall than just running it sequentially.' },
    { q: 'How do you safely share data between processes in Python\'s multiprocessing module, given that processes do not share memory by default?', a: 'Unlike threads (which share the same memory space within a process), separate processes have entirely independent memory — there is no implicit shared state, so passing data between processes requires explicit mechanisms: multiprocessing.Queue or Pipe for message-passing communication (data is serialized via pickle and sent across a inter-process channel), or multiprocessing.shared_memory / Value / Array for genuinely shared memory segments accessible by multiple processes without serialization overhead, typically used for large numeric arrays where copying via pickling would be too slow. Choosing the right mechanism depends on whether you need simple message passing or high-throughput shared numeric data.' },
  ];
