...
with open_temp_json({"key": "value"}) as path:
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Why does a decorator need to use functools.wraps, and what breaks if you omit it?', a: 'Without functools.wraps, a decorator\'s wrapper function replaces the original function\'s __name__, __doc__, and other metadata with the wrapper\'s own — so introspection tools, debuggers, and help() show misleading information (e.g., every decorated function appears to be named "wrapper" instead of its real name). @functools.wraps(func) applied to the inner wrapper function copies over this metadata from the original function, preserving correct introspection behavior, which is especially important for frameworks that rely on function names or docstrings (like some testing or routing libraries).' },
    ...
    { q: 'How does __exit__ in a context manager handle exceptions raised inside the with block, and what does returning True do?', a: '__exit__ receives three arguments describing any exception that occurred inside the with block: exc_type, exc_value, and traceback (all None if no exception occurred). If __exit__ returns a truthy value (explicitly True), Python suppresses the exception — it will not propagate further, as if it never happened. If __exit__ returns None or False (the default if you do not return anything), the exception propagates normally after cleanup runs. This lets a context manager implement custom exception-handling/suppression logic, such as a retry context or a context that converts specific exceptions into logged warnings.' },
  ];
