...
with open_temp_json({"key": "value"}) as path:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'How do you write a decorator that accepts its own arguments, like @retry(times=3)?', options: ['Decorators cannot take arguments other than the function', 'Add an extra outer function: the outer function takes the decorator arguments and returns the actual decorator, which takes and wraps fn', 'Use *args in the decorated function itself', 'Pass arguments through functools.wraps'], answer: 1, explanation: '@retry(times=3) requires three nested levels: retry(times) returns a decorator function, which takes fn and returns wrapper, which takes fn\'s own *args/**kwargs. So def retry(times): def decorator(fn): def wrapper(*a, **kw): ... ; return wrapper; return decorator; return decorator — calling retry(times=3) first returns the middle "decorator" function, which is then applied to fn.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How does contextlib.ExitStack help when you need to manage a dynamic (unknown at write-time) number of context managers?', a: 'ExitStack lets you enter any number of context managers programmatically (in a loop, conditionally) and guarantees all of them are exited in reverse order when the ExitStack itself exits, even if you don\'t know how many there will be until runtime: with ExitStack() as stack: files = [stack.enter_context(open(p)) for p in paths]. Without ExitStack, managing a variable number of nested with blocks requires awkward recursion or manual try/finally bookkeeping.' },
    { q: 'What is the difference between a context manager implemented with a class (__enter__/__exit__) versus the @contextmanager decorator?', a: 'A class-based context manager requires defining __enter__ (setup, returns the resource) and __exit__ (cleanup, receives exception info if one occurred) as separate methods — useful when the context manager needs to maintain state across multiple methods or be reused as a class instance elsewhere. The @contextmanager decorator from contextlib lets you write a context manager as a single generator function with a yield statement splitting setup (before yield) from teardown (after yield, ideally in a try/finally) — more concise for simple, linear setup/teardown logic without needing a full class.' },
    { q: 'What is the difference between contextlib.suppress(Exception) and a bare try/except/pass around the same code?', a: 'contextlib.suppress(ValueError) is a context manager purpose-built to suppress specific exception types, and its intent is immediately readable at a glance — "this block may raise ValueError and that is expected/fine." A try/except ValueError: pass achieves the same runtime effect but is more easily mistaken for accidentally-swallowed errors during code review, and doesn\'t compose as cleanly with other context managers (e.g. with suppress(FileNotFoundError), open(path) as f: reads cleanly as a single intent).' },
  ];
