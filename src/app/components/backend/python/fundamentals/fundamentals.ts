...
def fizzbuzz_compact(n: int) -> list[str]:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is Python\'s string slicing syntax s[start:stop:step] used for, and what does a negative step do?', options: ['Only positive steps are allowed', 's[start:stop:step] extracts a subsequence; a negative step (e.g. s[::-1]) reverses the sequence', 'Negative step raises a ValueError', 'Slicing only works on lists, not strings'], answer: 1, explanation: 's[start:stop:step] selects elements from start up to (not including) stop, advancing by step each time. s[::-1] is the idiomatic way to reverse a string or list (start and stop omitted, step=-1 walks backward through the whole sequence). s[::2] takes every second element.' },
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the difference between shallow copy and deep copy in Python, and when does the distinction matter?', a: 'A shallow copy (list(x), x.copy(), or copy.copy(x)) creates a new outer container but still references the SAME nested objects inside it — mutating a nested list inside a shallow copy also mutates it in the original. A deep copy (copy.deepcopy(x)) recursively copies every nested object too, so the copy is fully independent. The distinction only matters for containers holding mutable nested objects (lists of lists, dicts of lists); copying a flat list of immutable values (ints, strings) behaves the same either way.' },
