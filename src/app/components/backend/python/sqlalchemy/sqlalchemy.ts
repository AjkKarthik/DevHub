...
with SessionLocal() as s:
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is a SQLAlchemy connection pool, and why does exhausting it cause requests to hang rather than error immediately?', a: 'SQLAlchemy\'s engine maintains a pool of reusable database connections (default size 5, with an overflow limit) rather than opening a new TCP connection per query. When every pooled connection is checked out (e.g. under high concurrent load, or from connections leaked by code that never closes a session), new requests needing a connection BLOCK, waiting up to pool_timeout seconds for one to free up, rather than failing fast — which can look like the whole application hanging under load until you realize it is a pool-exhaustion issue, diagnosed by monitoring pool checked-out-connection counts.' },
