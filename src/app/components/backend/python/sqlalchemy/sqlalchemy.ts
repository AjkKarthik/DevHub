...
with SessionLocal() as s:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between session.add() and session.merge() in SQLAlchemy?', options: ['add() updates existing records; merge() inserts new ones', 'add() is for new objects only; merge() is an upsert — if the object\'s PK exists in the session or DB, it updates; otherwise inserts', 'They are identical', 'merge() is only for relationships'], answer: 1, explanation: 'session.add(obj) adds the object to the session — if it already exists in the identity map (by PK), SQLAlchemy tracks the same instance. session.merge(detached_obj) is for detached instances (e.g. from a previous session): if the PK exists in the identity map, it updates that instance; otherwise it fetches from DB and copies state. Use merge when reassociating detached objects from a closed session.' },
    { q: 'What does relationship() lazy="selectin" mean in SQLAlchemy 2.0?', options: ['Loads the relationship in a separate SELECT using IN clause — eagerly, per-object-batch', 'Loads the relationship using a JOIN', 'Never loads the relationship automatically', 'Loads the relationship lazily only for SELECT statements'], answer: 0, explanation: 'lazy="selectin" (selectinload) issues one SELECT per relationship for all parent objects at once: SELECT * FROM child WHERE parent_id IN (1,2,3,...). This avoids N+1 queries without a JOIN. Use selectinload for one-to-many relationships. Use joinedload for many-to-one (FK on the queried table) — JOIN is efficient when you always need the related object.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the difference between SQLAlchemy Core and the SQLAlchemy ORM, and when would you choose one over the other?', a: 'SQLAlchemy Core provides a Pythonic, database-agnostic way to construct and execute SQL expressions directly (select(), insert(), explicit table/column objects) without mapping rows to Python class instances — giving fine-grained control closer to raw SQL with full type safety and composability. The ORM layer builds on Core, mapping database tables to Python classes and rows to objects, enabling object-oriented data access (user.orders instead of a manual join query) at the cost of some abstraction overhead and "magic" that can obscure the actual generated SQL. Choose Core for performance-critical bulk operations or complex dynamic queries; choose the ORM for typical CRUD-heavy application code where object-oriented modeling improves readability.' },
    { q: 'Why does SQLAlchemy\'s lazy loading of relationships sometimes cause an N+1 query problem, similar to other ORMs?', a: 'By default, accessing a relationship attribute on a SQLAlchemy model instance (user.orders) that has not been explicitly eager-loaded triggers a SEPARATE database query at the moment of access — if you iterate over a list of users and access .orders for each one in a loop, you trigger one query per user (N queries) in addition to the original query that fetched the users (1 query), totaling N+1 queries instead of a single efficient join. Use eager loading strategies (joinedload(), selectinload()) specified at query time to fetch related data in the same or a batched query upfront, avoiding the per-instance lazy-load triggers entirely.' },
    { q: 'What is the purpose of a SQLAlchemy Session, and why should you generally use one session per request in a web application?', a: 'A Session manages a "unit of work" — it tracks all objects loaded or modified during its lifetime, batches pending changes, and commits or rolls back the ENTIRE set of changes atomically as a single transaction. Using one session per web request (rather than a single global, long-lived session, or a brand new session per individual query) ensures that all database operations within a single request participate in the same transaction (so a failure partway through correctly rolls back everything), while keeping sessions naturally scoped and short-lived to avoid stale cached object state accumulating across unrelated requests.' },
  ];
