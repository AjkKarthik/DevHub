...
async def check_urls(urls: list[str], max_concurrent: int = 10, timeout: float = 5.0) -> list[dict]:
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between asyncio.sleep(0) and yield in a coroutine?', options: ['They are identical', 'asyncio.sleep(0) yields control to the event loop, allowing other coroutines to run before resuming', 'yield resumes immediately; asyncio.sleep(0) waits 1 tick', 'asyncio.sleep(0) raises StopIteration'], answer: 1, explanation: 'await asyncio.sleep(0) suspends the coroutine for zero time, giving the event loop a chance to run other pending callbacks and coroutines before returning. It is used in busy loops to prevent starving the event loop. yield in a native coroutine (async def) is not valid syntax — coroutines use await, not yield.' },
    { q: 'How do you run synchronous blocking code without blocking the event loop?', options: ['Use threading.Thread directly', 'Use loop.run_in_executor(None, sync_function) to run it in a thread pool', 'Use asyncio.run() inside the coroutine', 'Blocking code cannot be used in asyncio'], answer: 1, explanation: 'loop.run_in_executor(None, func, *args) or asyncio.to_thread(func, *args) runs a synchronous function in a ThreadPoolExecutor, returning an awaitable. The event loop continues running other coroutines while the blocking call runs in a worker thread. Use this for file I/O, synchronous DB drivers, or CPU work (use ProcessPoolExecutor for CPU-bound tasks).' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the default size of the thread pool used by loop.run_in_executor(None, ...), and why does it matter?', a: 'When the executor argument is None, asyncio lazily creates a default ThreadPoolExecutor sized to min(32, os.cpu_count() + 4). This is a SHARED pool for the whole event loop — if you submit more concurrent blocking calls than the pool size, extra calls queue and wait for a free worker thread, silently reducing your actual concurrency below what your code assumes. For workloads with many simultaneous blocking calls (e.g. hundreds of concurrent DB queries via a sync driver), pass an explicitly-sized ThreadPoolExecutor instead of relying on the default.' },
    ...
    { q: 'Why does calling a blocking, synchronous function (like time.sleep or a synchronous requests.get) inside an async function defeat the purpose of asyncio?', a: 'asyncio achieves concurrency through cooperative multitasking on a single thread — coroutines voluntarily yield control back to the event loop at await points, allowing other coroutines to run while one is waiting on I/O. A blocking synchronous call has no await point and does not yield control at all; it freezes the entire event loop for its duration, preventing ANY other coroutine from making progress, effectively making the whole async application behave as if it were single-threaded and sequential for that duration. Use asyncio.to_thread() (3.9+) or run_in_executor to offload genuinely blocking calls to a thread pool.' },
    { q: 'What is the difference between asyncio.gather() and asyncio.as_completed() for running multiple coroutines concurrently?', a: 'asyncio.gather() schedules all coroutines to run concurrently and returns their results as a list in the SAME ORDER they were passed in, only after ALL of them complete (or raises immediately if one fails, by default). asyncio.as_completed() also runs them concurrently but yields each result as soon as it becomes available, in COMPLETION order (not the original order) — useful when you want to process results as they arrive rather than waiting for the slowest one to determine when you see the fastest one\'s result.' },
    { q: 'Why can you not simply call an async function directly and expect it to run, the way you would call a regular function?', a: 'Calling an async def function does not execute its body at all — it immediately returns a coroutine object representing the deferred computation, without running a single line of the function. You must either await it from within another async function and an active event loop, or use asyncio.run(coro()) at the top level of a synchronous program to create an event loop, run the coroutine to completion, and return its result — simply calling my_async_func() and discarding the result (without await or asyncio.run) silently does nothing and often triggers a "coroutine was never awaited" warning.' },
  ];
