...
var dict = employees
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'orders.SelectMany(o => o.Lines) flattens all order lines across all orders. What does SelectMany\'s overload with a result-selector, `orders.SelectMany(o => o.Lines, (order, line) => new { order.Id, line.Sku })`, let you do that the single-argument version cannot?',
      a: 'The two-argument overload lets you project a result that combines BOTH the outer element (the order) and each inner element (a line) together, whereas the single-argument overload only gives you access to the inner elements after flattening — the outer order is lost once flattened. Without the result-selector overload, recovering "which order did this line come from" after a plain SelectMany requires either keeping the order reference on the line object itself (if the domain model already has that back-reference) or falling back to a nested loop / query-syntax `from order in orders from line in order.Lines select new { order.Id, line.Sku }` — the result-selector overload is exactly the method-syntax equivalent of that query-syntax pattern, letting you flatten while still tagging each flattened element with data from its parent.',
    },
    {
