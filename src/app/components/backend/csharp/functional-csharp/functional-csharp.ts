...
// Fail: Name is required.  ← stops at validation, never checks duplicate`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A developer uses Map with a lambda that internally calls another method returning Result<U>, accidentally producing Result<Result<U>> instead of using Bind. What concretely goes wrong for callers of this chain, beyond it just being "the wrong pattern"?',
      a: 'The immediate consumer-facing problem is that checking IsSuccess or calling Match/Switch on the outer Result<Result<U>> only tells you whether the OUTER Map operation itself succeeded (which, since Map\'s transform is assumed infallible, is almost always true) — it says nothing about whether the INNER Result<U> represents success or failure, so a caller who naively checks `.IsSuccess` and moves on can end up treating a genuinely failed inner operation as a success, silently swallowing the actual error. Unwrapping the nested Result correctly requires an extra explicit step (`.Value.IsSuccess` or similar) that callers are easy to forget precisely because a single Result<T> chain everywhere else in the codebase only ever needs one level of unwrapping — this double-wrapping bug is exactly the failure mode Bind\'s flattening exists to prevent structurally, rather than relying on every caller remembering to unwrap twice.',
    },
    {
