      ...
      right: `public class DataReader : IDisposable
{
  ...
  qna: QnaItem[] = [
    ...
    },
    {
      ...
      a: 'Both call <code>Dispose()</code> automatically even if an exception is thrown, but they differ in scope:<br><br><strong>Using statement</strong> (C# 1+): <code>using (var x = new Foo()) { ... }</code> — disposed at the explicit closing brace. Fine-grained control.<br><br><strong>Using declaration</strong> (C# 8+): <code>using var x = new Foo();</code> — disposed at the end of the <em>enclosing</em> scope. Reduces nesting when you have multiple disposable resources. Multiple using declarations are disposed in <em>reverse</em> declaration order — just like nested using blocks.<br><br>Use the declaration form for cleaner code. Fall back to the statement form when you need the resource disposed before the method ends (e.g., flush a writer, then read what you just wrote in the same method).',
    },
    {
