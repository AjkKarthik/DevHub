...
public async Task ProcessAsync(string item, CancellationToken ct)
{
  ...
  qna: QnaItem[] = [
    ...
    },
    {
      ...
      a: '<code>volatile</code> provides two guarantees: (1) <strong>Visibility</strong> — reads and writes always go to main memory, never a CPU register or L1/L2 cache. A change made by one thread is immediately observable by all other threads. (2) <strong>Ordering</strong> — reads are acquire-fences and writes are release-fences, preventing certain compiler and CPU reorderings. What it does NOT guarantee: atomicity of compound operations. <code>volatile int x; x++;</code> is still a race condition because increment is three operations. Use <code>Interlocked.Increment</code> for atomic arithmetic, or <code>lock</code> for multi-field atomic updates.',
    },
    {
