...
public async Task<int> GetCountAsync()  // Task is safely re-awaitable
{
  ...
  qna: QnaItem[] = [
    {
      q: 'A unit test calls an async void method and then asserts on some state the method was supposed to mutate. The assertion sometimes passes and sometimes fails, non-deterministically. Why, and what does this reveal about testing async void code?',
      a: 'Because async void returns no Task, the test has no way to await the method\'s completion — it calls the method, which kicks off the async operation and returns control to the caller almost immediately (at the first await point inside it), and the test\'s assertion runs immediately after without any guarantee the async work has actually finished. The test\'s pass/fail then depends on a race between the assertion running and the async void method\'s continuation completing, which is exactly the kind of flaky, timing-dependent test behavior that async void\'s lack of an awaitable handle causes. This is a concrete, practical reason (beyond exception handling) that async void is considered untestable and should be reserved strictly for event handlers the framework requires to be void — any method you intend to call from your own code, including tests, needs to return Task so callers can actually wait for it.',
    },
    {
      q: 'Why does the SAME .Result blocking pattern that deadlocks in ASP.NET Framework generally NOT deadlock in an ASP.NET Core / console application?',
      a: 'The deadlock specifically requires a captured SynchronizationContext that only allows one thread to run request-handling code at a time — that is a property of the classic ASP.NET Framework request pipeline (and WPF/WinForms UI threads), not of async/await itself. ASP.NET Core deliberately does not install a SynchronizationContext by default, and neither do plain console apps — so an `await` continuation inside a blocked-on `.Result` call is free to resume on any available thread pool thread rather than needing the specific occupied thread back, which means the classic deadlock pattern usually does not reproduce there. This does NOT make `.Result`/`.Wait()` safe in ASP.NET Core — it still wastes a thread pool thread blocking synchronously and can cause thread-pool starvation under load, a different but still real problem — it just changes the failure mode from "instant deadlock" to "gradual throughput degradation under concurrency," which is easy to miss in local testing.',
    },
    {
