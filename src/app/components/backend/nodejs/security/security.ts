...
app.listen(3000);`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does the helmet middleware do in an Express application?', options: ['Adds authentication middleware', 'Sets secure HTTP response headers (CSP, HSTS, X-Frame-Options, etc.) to harden the app against common web attacks', 'Encrypts the response body', 'Validates SSL certificates'], answer: 1, explanation: 'helmet() sets ~11 security-related HTTP headers. Key ones: Content-Security-Policy (prevents XSS), Strict-Transport-Security (force HTTPS), X-Frame-Options: DENY (prevent clickjacking), X-Content-Type-Options: nosniff (prevent MIME sniffing). One line of code prevents several OWASP Top 10 attack categories.' },
