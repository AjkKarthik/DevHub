...
console.log(results); // [{status:'fulfilled',value:2}, ...]`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'You switch a batch operation from Promise.all() to Promise.allSettled() to avoid one failure aborting the whole batch — what new responsibility does your code now have that it didn\'t before?', options: ['None — allSettled is a drop-in replacement', 'The caller must manually inspect each result\'s status and handle rejected entries itself, since allSettled never throws', 'allSettled requires wrapping every promise in try/catch first', 'allSettled automatically retries rejected promises'], answer: 1, explanation: 'Promise.all\'s rejection was doing implicit error handling — one throw and the whole batch failed loudly. Promise.allSettled never rejects, so silently ignoring the returned array (or only reading .value without checking status) means failures vanish with no error and no log. Switching to allSettled shifts error handling from "automatic via rejection" to "manual: filter results by status === \'rejected\' and decide what to do with each."' },
  ...
  qna: QnaItem[] = [
    ...
    { q: 'If Promise.all() rejects on the first failing promise, do the OTHER promises in the batch stop running too?', a: 'No — Promise.all() only stops waiting; it does not cancel anything. The promises you passed in are already-running operations (fetch calls, DB queries, etc.) that continue executing to completion in the background regardless of what Promise.all() does with its own returned promise. This matters for cleanup: if one of those "abandoned" operations has side effects (a write to a database, a charge to a payment API), it still happens even though your code already moved on to the catch block — plain Promises have no built-in cancellation, so preventing wasted work requires an explicit mechanism like AbortController.' },
    { q: 'Why does forgetting to await an async function inside a loop sometimes cause unexpected behavior?', a: 'If you call an async function inside a for loop without await (or without collecting and awaiting the returned promises), each call fires immediately without waiting for the previous one to finish, launching all operations concurrently rather than sequentially — this can overwhelm a rate-limited API, exhaust database connection pools, or produce race conditions if operations depend on shared state. Use a for...of loop with await for guaranteed sequential execution, or Promise.all() with .map() when concurrent execution is actually desired and safe.' },
    { q: 'What is an unhandled promise rejection in Node.js, and why does it now crash the process by default?', a: 'An unhandled promise rejection occurs when a Promise rejects and no .catch() or try/catch around an await ever handles that rejection. Since Node.js 15, unhandled rejections crash the process by default (previously they only logged a warning) because Node.js maintainers determined that silently swallowing rejected promises was a common source of hidden bugs — the new behavior surfaces these as loud, fail-fast errors rather than allowing applications to silently continue in a potentially inconsistent state.' },
  ];
