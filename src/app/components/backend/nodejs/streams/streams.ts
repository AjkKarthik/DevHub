...
console.log(\`Lines: \${counter.lineCount}\`);`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is backpressure in Node.js streams?', options: ['Memory pressure from too many buffers', 'Signal from a writable stream that it cannot accept more data — the readable should pause until the writable drains', 'CPU throttling during stream processing', 'Network congestion control'], answer: 1, explanation: 'When a Readable produces data faster than a Writable can consume, data buffers in memory. The Writable signals backpressure by returning false from write() — the Readable should pause until the drain event fires. Using pipe() handles this automatically. Manual streams must respect write() return value to avoid unbounded memory growth.' },
    { q: 'What is stream.pipeline() and why is it preferred over pipe()?', options: ['pipeline() is faster than pipe()', 'pipeline() propagates errors and destroys all streams on failure — pipe() leaves streams open on error', 'pipeline() supports objectMode; pipe() does not', 'They are identical'], answer: 1, explanation: 'readableStream.pipe(writableStream) does not propagate errors — if readableStream errors, writableStream is not destroyed and vice versa, causing resource leaks. stream.pipeline(src, transform, dest, callback) propagates errors to all streams in the chain and destroys them on failure, calling the callback with the error.' },
  ];
