...
console.log(\`Lines: \${counter.lineCount}\`);`
  ...
  qna: QnaItem[] = [
    ...
    { q: 'If a Transform stream in the middle of a pipeline is slow (e.g. it does CPU-heavy work per chunk), does backpressure propagate all the way back to the original readable source, or only to the stream immediately before it?', a: 'It propagates all the way back through the whole chain, not just one hop. When the slow Transform\'s internal buffer fills, its write() calls start returning false to whatever is writing into it; if that\'s another stream connected via pipe()/pipeline(), that stream pauses reading from ITS source in response, and so on backward through every stage until the original readable source is paused too. This is what makes streams composable for arbitrarily long pipelines — you don\'t need to manually wire backpressure signaling between each pair, pipe()/pipeline() propagates the slowdown transitively through the entire chain automatically.' },
