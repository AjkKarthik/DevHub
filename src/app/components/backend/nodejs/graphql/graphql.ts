...
await startStandaloneServer(server, {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What problem does DataLoader solve in GraphQL?', options: ['Schema validation errors', 'The N+1 query problem — batches individual resolver DB calls into one query per request', 'Rate limiting per resolver', 'Caching schema definitions'], answer: 1, explanation: 'Without DataLoader, resolving a list of 100 posts with each post\'s author makes 100 separate DB queries for authors. DataLoader collects all author IDs from the event tick and makes one SELECT WHERE id IN (...) query. It also caches within the request, so the same author queried by multiple resolvers is only fetched once.' },
