...
await startStandaloneServer(server, {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'A resolver calls `await userLoader.load(id)` inside a loop with an `await` between each call, instead of collecting all IDs first and loading them together. Does DataLoader still batch these into one query?', options: ['Yes, always — DataLoader batches every call regardless of timing', 'No — awaiting between calls lets each load() resolve on its own event-loop tick before the next one is queued, defeating the batching window', 'DataLoader batches based on a fixed time window (e.g. 10ms), so this pattern is fine', 'Batching only depends on the number of calls, not their timing'], answer: 1, explanation: 'DataLoader batches together only the .load() calls made within the SAME tick of the event loop, before it flushes the queued keys as one batch. Awaiting each load() individually inside a loop means each call gets its own microtask turn to resolve before the next iteration even queues its key — so instead of one batched query, you\'re back to one query per call, silently reintroducing the N+1 problem DataLoader was supposed to fix. The fix is to fire off all the load() calls (or Promise.all them) without awaiting in between, so they land in the same batching window.' },
    { q: 'What is the difference between a Query and a Mutation in GraphQL?', options: ['Queries return lists; mutations return single objects', 'Queries are read-only (no side effects); mutations are for writes (create, update, delete)', 'Mutations require authentication; queries do not', 'They are identical — the distinction is only semantic'], answer: 1, explanation: 'Queries should be idempotent reads — calling them multiple times produces the same result. Mutations signal intent to modify data — create, update, delete. This distinction allows clients (and caching layers) to know that mutations cannot be cached. GraphQL does not enforce this technically — the convention is yours to uphold.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'If DataLoader caches results per request, can two different resolvers in the SAME query end up seeing stale data relative to each other — for example, one resolver reading a user before a mutation-like side effect and another reading it after?', a: 'Within a single GraphQL query execution, yes this is a real risk: DataLoader\'s per-request cache means the first .load(userId) call caches whatever it fetched, and every subsequent .load(userId) call anywhere else in that same query — even in a resolver that logically runs "after" some data-changing operation — returns the cached value rather than re-querying. This is rarely a problem for pure queries (nothing should be mutating mid-query), but becomes a real gotcha in resolvers that trigger side effects (e.g. a resolver that lazily creates a record) — the loader cache can hand back pre-side-effect data to a sibling resolver. The fix is calling loader.clear(key) or loader.clearAll() after any resolver that mutates data the loader has already cached.' },
