...
start().catch(err => { console.error(err); process.exit(1); });`
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A multi-stage Dockerfile copies `node_modules` from the build stage into the final stage instead of running `npm ci --omit=dev` fresh in the final stage. What goes wrong?', a: 'Copying node_modules across stages carries over devDependencies (test frameworks, bundlers, TypeScript) into the production image, defeating the whole point of the multi-stage split — the final image ends up just as bloated and just as exposed to build-tool vulnerabilities as a single-stage build would be. It can also silently ship native modules compiled against the build stage\'s architecture/libc if the build and final base images differ (e.g. a Debian-based builder feeding an Alpine final stage), causing runtime crashes from ABI mismatches that a fresh npm ci in the final stage — using the final stage\'s own Node/libc — would avoid entirely.' },
  ];
