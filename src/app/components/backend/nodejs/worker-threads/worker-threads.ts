...
if (isMainThread) {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What data can be transferred between the main thread and worker threads using transferList?', options: ['All JavaScript objects', 'Only typed arrays and ArrayBuffers', 'Transferable objects (ArrayBuffer, MessagePort, ImageBitmap) — transferred without copying, ownership moves to the receiver', 'Only primitive values (string, number, boolean)'], answer: 2, explanation: 'postMessage with a transferList moves ownership of transferable objects to the receiver — zero copy. After transfer, the original reference becomes detached (TypedArray.length becomes 0). For large ArrayBuffers (image processing, audio data), this is critical — copying a 100MB buffer would be expensive. SharedArrayBuffer is shared without transfer — multiple threads see the same memory.' },
    { q: 'When is it NOT worth using worker threads in Node.js?', options: ['For CPU-bound tasks taking over 100ms', 'For short tasks under ~1ms — thread setup overhead exceeds the work time', 'For image processing', 'For encryption'], answer: 1, explanation: 'Worker thread creation has ~2-10ms overhead (thread startup + V8 context initialisation). For a task that takes 0.5ms, creating a worker thread is 4–20× slower than running it on the main thread. Use workers for tasks that take hundreds of milliseconds or more. A pool of pre-warmed workers eliminates per-task startup cost.' },
  ];
