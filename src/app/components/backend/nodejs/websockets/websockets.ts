...
io.on('connection', (socket) => {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'A multi-server Socket.io deployment has the Redis adapter configured, but the load balancer is NOT using sticky sessions. What breaks?', options: ['Nothing — the Redis adapter makes sticky sessions unnecessary', 'A client using HTTP long-polling fallback can have consecutive requests land on different servers mid-handshake, breaking the connection, since polling requires session affinity that the Redis adapter does not provide', 'Redis pub/sub itself requires sticky sessions to function', 'Sticky sessions are only relevant for raw WebSocket, never for Socket.io'], answer: 1, explanation: 'The Redis adapter solves cross-server BROADCASTING (event A on server 1 reaching a client on server 2) — it does not solve connection affinity. Socket.io\'s HTTP long-polling transport (used as a fallback when WebSocket is unavailable) sends a sequence of HTTP requests that must all hit the same server process to maintain the logical connection; without sticky sessions, the load balancer can route those requests to different servers and break the session. Sticky sessions and the Redis adapter solve two different problems and are both needed together.' },
