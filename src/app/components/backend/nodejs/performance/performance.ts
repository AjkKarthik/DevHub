...
class EventLoopMonitor extends EventEmitter {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does v8.writeHeapSnapshot() help you diagnose?', options: ['CPU flame graph bottlenecks', 'Memory leaks — captures all live JS objects in a heap snapshot for analysis in Chrome DevTools', 'Event loop blockage', 'Network latency'], answer: 1, explanation: 'A heap snapshot captures the entire JS heap as a JSON file. Open in Chrome DevTools → Memory → Load snapshot to see all allocated objects, their sizes, and what is holding references. Compare two snapshots taken at different times (snapshot 1 → do operations → snapshot 2) to find objects growing between snapshots — the definition of a memory leak.' },
    { q: 'What is the purpose of the --max-old-space-size Node.js flag?', options: ['Sets the maximum number of open file handles', 'Sets the V8 old generation heap limit in MB — default ~1.4GB; increase for memory-intensive applications', 'Limits the number of old connections', 'Sets the maximum request body size'], answer: 1, explanation: 'V8 garbage collects the young generation frequently and the old generation less often. The old generation holds long-lived objects. Default ~1.4GB. For memory-intensive Node.js services, increase with node --max-old-space-size=4096 app.js. Without this, V8 throws "JavaScript heap out of memory" when the old generation fills up, even if the machine has free RAM.' },
  ];
