...
describe('Tasks API', () => {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the main benefit of Supertest over starting a real HTTP server yourself?', options: ['Supertest is faster to install', 'Supertest still binds a real TCP port under the hood, but auto-assigns an ephemeral one (port 0) instead of you hardcoding and managing one yourself', 'Supertest supports more HTTP methods', 'Supertest runs tests in parallel'], answer: 1, explanation: 'Supertest does not bypass the network stack — when given an Express app (a plain request handler function), it wraps it via http.createServer(app) and binds it to an OS-assigned ephemeral port with .listen(0) on the first request. The real benefit is that you never manage or hardcode that port yourself, eliminating manual port bookkeeping and conflicts when test files run in parallel.' },
    { q: 'What does jest.spyOn(obj, "method") do?', options: ['Creates a copy of the method', 'Wraps the original method to track calls while keeping its original implementation (unless mockReturnValue is added)', 'Prevents the method from being called', 'Replaces the method with undefined'], answer: 1, explanation: 'jest.spyOn(userService, "findOne") wraps the method with a Jest mock that records calls, arguments, and return values — but still calls the original implementation. Add .mockResolvedValue(data) to override the return value. Restore with jest.restoreAllMocks() in afterEach. Use spyOn when you want to track calls without changing behavior, or mock only for specific tests.' },
    { q: 'What is the difference between unit tests and integration tests in Node.js context?', options: ['Unit tests are faster; integration tests are slower — no other difference', 'Unit tests isolate one function/class with all dependencies mocked; integration tests exercise multiple layers with real dependencies (real DB or in-memory)', 'Integration tests run in production; unit tests run in CI only', 'Unit tests use Jest; integration tests use Mocha'], answer: 1, explanation: 'Unit tests: test a single function/service in isolation, mock all I/O (DB, HTTP). Fast, no external setup. Integration tests: test routes end-to-end through Express/Fastify → controller → service → real DB (PostgreSQL in Docker, or sqlite in-memory). Catch interface mismatches that mocks hide. Both are needed: units for coverage, integration for confidence.' },
  ];
