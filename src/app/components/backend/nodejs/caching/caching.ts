...
export const cache = new CacheService(redis);`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does stale-while-revalidate cache-control directive do?', options: ['Prevents the browser from caching the response', 'Serves stale content immediately while fetching a fresh copy in the background', 'Forces a 304 Not Modified response', 'Clears all cached entries'], answer: 1, explanation: 'stale-while-revalidate=N tells the browser/CDN: if the cached entry is expired but within N seconds past expiry, serve the stale version immediately and refresh in the background. The next request gets the fresh copy. This eliminates cache-miss latency spikes while keeping data reasonably fresh.' },
    { q: 'If two application server instances both try to acquire a stampede-prevention lock in Redis at the exact same moment, how does Redis guarantee only one of them wins?', options: ['Redis processes requests in parallel so both could win simultaneously', 'Redis is single-threaded — commands like SET key value NX are executed atomically one at a time, so even if two SET NX commands arrive at nearly the same instant, Redis processes them sequentially and only the first one actually sets the key (NX = only set if not already present), causing the second to fail and know it lost the race', 'Redis uses a separate locking server (ZooKeeper) to arbitrate between competing clients', 'Both instances always win and the conflict is resolved by the application afterward'], answer: 1, explanation: 'Redis\'s single-threaded command execution model means there is never any true simultaneity at the point commands are actually processed — even if two SET key value NX commands are sent within microseconds of each other, Redis executes them one after another in its single command-processing thread, and the NX (only-if-not-exists) condition means the second command simply fails silently once the first has already set the key. This atomicity guarantee is exactly why a simple SET NX in Redis is a reliable distributed lock primitive without needing any additional coordination protocol.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What happens if a write-through cache update succeeds but the corresponding database write fails partway through the same operation?', a: 'This is write-through\'s core consistency risk: if the cache update and the DB write are not performed as a single atomic operation, a partial failure (cache updated, DB write failed, or vice versa) leaves the two data stores disagreeing about the true current value — the cache now confidently serves a value the database never actually persisted. Robust write-through implementations write to the DB FIRST and only update the cache after confirming the DB write succeeded (so a DB failure never corrupts the cache), accepting that a crash between the two steps can still leave a stale cache entry that must be handled by a reasonable TTL as a backstop.' },
    ...
    { q: 'What happens to a stampede-prevention lock if the process holding it crashes before releasing (deleting) the lock key?', a: 'Without a safeguard, a crashed lock-holder would leave the lock key set forever, permanently blocking every future request from ever regenerating that cache entry — a much worse outcome than the stampede it was meant to prevent. The standard fix is to always set the lock key with a short TTL (e.g. SET lock:key value NX EX 5) rather than relying on the holder to explicitly delete it — if the holder crashes mid-regeneration, the lock simply expires on its own after a few seconds and the next request is free to acquire it and try again, bounding the worst-case impact of a crash to that TTL window rather than an indefinite outage.' },
  ];
