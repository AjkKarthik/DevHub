...
export const cache = new CacheService(redis);`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is cache stampede (thundering herd) and how do you prevent it?', options: ['A race condition where multiple writes corrupt the cache simultaneously', 'When many requests hit an expired cache simultaneously, overloading the DB with concurrent refreshes', 'A Redis out-of-memory event', 'A DNS caching failure'], answer: 1, explanation: 'When a cached item expires, all concurrent requests miss the cache and hit the DB simultaneously. Fix: (1) Lock-based: the first miss acquires a distributed lock and refreshes; others wait. (2) Probabilistic early expiry: occasionally refresh before TTL expires based on compute time. (3) Background refresh: refresh before expiry, never let cache go cold.' },
  ];
