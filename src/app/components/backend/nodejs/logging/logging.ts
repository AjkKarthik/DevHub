...
app.get('/test', (req, res) => {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'If your structured logs are already JSON, why might a query like `WHERE duration > 500` still fail to find slow requests?', a: 'Structured logging only guarantees the log LINE is valid JSON — it says nothing about the TYPE of each field. A common bug is logging duration as a string (e.g. `\`${ms}ms\`` or a value that got stringified somewhere in the pipeline) instead of a number; most log platforms then index that field as text, and numeric comparison queries silently match nothing. The fix is enforcing a schema at the logger level (e.g. Pino serializers, or a shared logging wrapper) so fields like duration, statusCode, and byte counts are always emitted as actual JSON numbers, not numeric-looking strings.' },
    { q: 'A correlation ID is generated at the request entry point — what happens to log correlation for work that outlives the original HTTP request, like a job enqueued from that request and processed minutes later by a worker?', a: 'The correlation ID must be explicitly written into the job payload (or message headers) when it is enqueued, and the worker must read it back out and bind it to its own logger (e.g. via a new child logger) when processing the job — it does NOT propagate automatically just because the job "came from" that request, since by the time the worker picks it up the original request\'s context (and any AsyncLocalStorage store tied to it) is long gone. Skipping this step is a common gap: HTTP-to-HTTP correlation works out of the box with middleware, but HTTP-to-queue-to-worker correlation requires deliberately threading the ID through the message itself.' },
    { q: 'What happens to log ordering when Pino offloads serialization/transport to a separate worker thread — can logs appear out of order relative to when they were emitted?', a: 'Within a single pino instance, log records are still queued and written in the order they were emitted — offloading to a worker thread does not reorder them, since Pino streams entries to the transport in emission order even though the actual I/O happens asynchronously off the main thread. What CAN appear interleaved unexpectedly is logs from genuinely concurrent operations (multiple in-flight requests each logging) — but that\'s a property of concurrent execution itself, not something Pino\'s worker-thread transport introduces; each individual log call\'s position in the overall stream is preserved relative to other calls from the same logger instance.' },
  ];
