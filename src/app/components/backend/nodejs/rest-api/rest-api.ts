...
router.delete('/:id', (req, res, next) => { try { svc.deletePost(req.params.id); res.status(204).send(); } catch(e){next(e);} });`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'A POST to /orders queues the order for async processing rather than creating it synchronously — which status code fits, and what should the response body contain?', options: ['201 Created, with the final order object', '200 OK, with an empty body', '202 Accepted, with a status URL or job ID the client can poll for the eventual result', '204 No Content, since nothing exists yet'], answer: 2, explanation: '202 Accepted communicates "request accepted, processing not yet complete" — distinct from 201 (resource already exists and is returned) and 200. The response body conventionally includes a Location or status-check URL / job ID so the client knows where to poll for completion, since there is no finished resource to return yet.' },
