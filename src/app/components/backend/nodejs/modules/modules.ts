...
console.log(\`Cleared \${clearProjectModules()} modules\`);`
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does "type": "module" in package.json change?', options: ['Enables TypeScript compilation', 'All .js files in the package are treated as ESM — require() is disabled, import/export syntax required', 'Enables top-level await only', 'Makes the package publishable to npm'], answer: 1, explanation: 'Without "type":"module", .js files use CommonJS (require/module.exports). With "type":"module", all .js files become ESM (import/export, top-level await, strict mode by default). You can override individual files with .cjs or .mjs extensions. This change affects whether __dirname/__filename are available (not in ESM — use import.meta.url instead).' },
  ];
