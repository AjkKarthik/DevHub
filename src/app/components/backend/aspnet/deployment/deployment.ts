...
app.Run();`,
  ...
  qna: QnaItem[] = [
    { q: 'A team publishes with PublishAot=true and the build succeeds with only a handful of trimmer warnings they decided to ignore. In production, one specific endpoint throws a runtime exception referencing a type that was trimmed away, even though the build had no errors. What does this reveal about trusting a clean AOT build?', a: 'A successful AOT build only proves the compiler could statically resolve everything it analyzed — trimmer WARNINGS (not errors) specifically flag code paths the trimmer could not fully verify as safe, most commonly reflection-based code (an attribute-driven serializer, a DI container resolving a type by string name, an ORM building queries dynamically) where the trimmer cannot prove which types are actually needed at runtime and may remove ones that are. Ignoring trimmer warnings is effectively deferring the actual verification from build-time to runtime — the build succeeding is not equivalent to "the app works," and thorough AOT adoption requires either resolving every trimmer warning or exercising every code path in a real AOT-published binary (not just the JIT dev build) before trusting it for production traffic.' },
