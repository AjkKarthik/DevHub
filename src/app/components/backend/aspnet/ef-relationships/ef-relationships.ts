    ...
    async (int postId, int tagId, AppDbContext db, CancellationToken ct) =>
{
  ...
  qna: QnaItem[] = [
    ...
    },
    {
      ...
      a: 'AsSplitQuery() splits a query with multiple collection Includes into separate SQL SELECTs. Without it, joining multiple one-to-many collections causes cartesian explosion — if an order has 10 items and each has 3 tags, the JOIN produces 30 rows per order. Use AsSplitQuery() when you Include multiple collections. For a single collection Include, a JOIN is fine.',
    },
    {
