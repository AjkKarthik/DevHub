    ...
    : BackgroundService
{
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A team starts a new microservice with CreateSlimBuilder() for the AOT/cold-start benefits, but three months in adds a library that depends on the default HttpClient message handler chain CreateSlimBuilder() omits. What is the actual fix, and is switching back to CreateBuilder() the only option?', a: 'Switching to CreateBuilder() is not the only option and usually not the best one, since it would discard the AOT startup/footprint benefits that motivated the original choice — CreateSlimBuilder\'s omissions are individually addable back via explicit service registration (e.g. calling AddHttpClient() manually restores IHttpClientFactory infrastructure) rather than being an all-or-nothing package deal. The slim builder is a MINIMAL starting point specifically so teams can opt back into exactly the pieces they need rather than paying for everything by default — the right response to "I need one thing CreateSlimBuilder omitted" is usually adding that one registration explicitly, not abandoning the slim builder\'s smaller footprint and faster cold starts for the whole app.' },
