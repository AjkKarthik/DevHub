...
public class JobMetrics
{
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A background job resolves 10,000 Transient IDisposable instances from the root IServiceProvider over its lifetime (one per item processed, never inside a scope). What specifically fails first — do you get an immediate crash, or does something more gradual happen?',
      a: 'Nothing crashes immediately — this is a slow memory leak, not an acute failure: each of the 10,000 instances is held alive by the root container\'s internal disposal-tracking list (so it can dispose them at shutdown, which for a long-running service may be days or weeks away), meaning none of their memory is reclaimable by the GC in the meantime even though the code that resolved them has long since finished using them. The practical failure mode is gradual memory growth that eventually triggers OOM kills, degraded performance from GC pressure as the heap grows, or exhaustion of any unmanaged resources those disposables hold (file handles, DB connections) — the kind of issue that is easy to miss in short-lived local testing and only surfaces after the service has been running in production for an extended period, which is exactly why it is a particularly dangerous class of bug to introduce.',
    },
    {
