...
app.MapPost("/products/deactivate-old", async (AppDbContext db, CancellationToken ct) =>
{
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A query result is serialized directly to JSON for an API response after being loaded with AsNoTrackingWithIdentityResolution() instead of plain AsNoTracking(). If the query includes the same entity reachable from two different navigation paths, what could go wrong specifically in the JSON output, given identity resolution wires both paths to the SAME object instance?',
      a: 'Standard JSON serializers (including System.Text.Json by default) do not understand object-graph identity the way .NET references do — when the same object instance is reachable from two different properties in the graph, a naive serializer either duplicates the full object twice in the JSON output (defeating the purpose of deduplication for the API consumer, who still sees two independent-looking copies) or, worse, throws a runtime exception if the graph now contains a CYCLE that identity resolution created (e.g. Order → Customer → Orders → same Order instance, forming a reference loop invisible with AsNoTracking\'s duplicated-instance behavior but very much present with WithIdentityResolution\'s shared-instance behavior). This is a concrete reason to be deliberate about which one to use for anything destined for JSON serialization — either configure the serializer\'s reference handling (ReferenceHandler.Preserve or IgnoreCycles) or map to DTOs that break the cyclic graph before serializing.',
    },
    {
