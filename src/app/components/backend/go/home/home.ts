...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Go Generics',               route: '/go', badge: 'Foundations', available: false,
    description: 'Generic functions and types in Go 1.18+ — type parameters, constraints, and when to use them.',
    keyPoints: ['func Map[T, U any](s []T, f func(T) U) []U — generic transform', 'type constraint: interface with ~T for underlying types', 'comparable: built-in constraint for == / != comparisons', 'Type inference: Go deduces type params from call site', 'Use sparingly — interfaces are often clearer'] },
  { title: 'JSON & Encoding',           route: '/go', badge: 'HTTP & APIs', available: false,
    description: 'JSON marshalling/unmarshalling, struct tags, custom MarshalJSON, and encoding/xml.',
    keyPoints: ['json:",omitempty" omits zero-value fields', 'json:"-" excludes field from JSON completely', 'Custom Marshaler: implement json.Marshaler interface', 'json.Decoder for streaming large JSON (not json.Unmarshal)', 'encoding/xml: same tag pattern for XML serialisation'] },
  { title: 'Go CLI Tools',              route: '/go', badge: 'Tooling', available: false,
    description: 'Build CLI applications with cobra and flag — subcommands, flags, persistent flags, and tab completion.',
    keyPoints: ['cobra: de-facto CLI framework (kubectl, hugo use it)', 'cmd.PersistentFlags() — flags inherited by subcommands', 'cobra-cli generate: scaffold command files', 'flag package: stdlib flags, simpler for single-command tools', 'viper: config file + env vars + flags in one'] },
  { title: 'Go Performance & Profiling', route: '/go', badge: 'Tooling', available: false,
    description: 'pprof CPU and heap profiling, escape analysis, sync.Pool, and avoiding allocations.',
    keyPoints: ['go tool pprof: interactive profile analysis', 'go test -cpuprofile cpu.out -memprofile mem.out', 'sync.Pool: reuse allocations in hot paths', 'go build -gcflags="-m": escape analysis output', 'Benchmark with b.ReportAllocs() to count allocations'] },
  { title: 'Go Build & Cross-compilation', route: '/go', badge: 'Tooling', available: false,
    description: 'Build flags, cross-compile for Linux/ARM, CGo trade-offs, and reproducible Go builds.',
    keyPoints: ['GOOS=linux GOARCH=amd64 go build: cross-compile', 'CGO_ENABLED=0: static binary, no glibc dependency', 'go build -ldflags="-s -w": strip debug info for smaller binary', 'Build tags: //go:build linux && amd64', 'go build -trimpath: reproducible builds'] },
  { title: 'Gin Web Framework',           route: '/go', badge: 'HTTP & APIs', available: false,
    description: 'The most popular Go web framework — fast routing, middleware chain, param binding, and error handling.',
    keyPoints: ['gin.New() vs gin.Default(): Default adds Logger + Recovery middleware', 'Route params: c.Param("id"); query: c.Query("q"); body: c.ShouldBindJSON(&dto)', 'Middleware: r.Use(AuthMiddleware()) applies globally; r.Group() scopes middleware', 'c.JSON(200, gin.H{"key": val}): JSON response with status code', 'ShouldBindJSON vs BindJSON: ShouldBind does not abort on error — handle manually'] },
