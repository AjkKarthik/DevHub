...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Symbols & Iterators',         route: '/javascript', badge: 'Foundations', available: false,
    description: 'Symbol primitive, well-known symbols, and the iterator protocol — how for...of and spread really work.',
    keyPoints: ['Symbol(): unique primitive, never equal to anything else', 'Symbol.iterator: makes any object iterable (for...of, spread, destructure)', 'Well-known symbols: Symbol.toPrimitive, Symbol.hasInstance, Symbol.asyncIterator', 'Generator functions auto-implement the iterator protocol', 'Private-like properties on shared objects without name collision'] },
  { title: 'Generators',                 route: '/javascript', badge: 'Async', available: false,
    description: 'function*, yield, two-way communication, infinite sequences, and async generators.',
    keyPoints: ['function*: returns a generator — pauses at each yield', 'next(value) passes a value back in to the yield expression', 'Infinite sequence: function* counter() { let n=0; while(true) yield n++; }', 'yield*: delegate to another iterable (compose generators)', 'async function* for async iteration — for await...of a stream'] },
  { title: 'Proxy & Reflect API',        route: '/javascript', badge: 'Patterns', available: false,
    description: 'Intercept object operations with Proxy traps — powers Vue 3 reactivity, validation, and logging.',
    keyPoints: ['new Proxy(target, handler): intercept get/set/has/apply/construct', 'Vue 3 reactivity is built on Proxy — zero configuration tracking', 'Reflect: mirrors Proxy traps — always call Reflect inside trap for correct default', 'Validation proxy: throw on invalid property assignment', 'Revocable proxies: proxy.revoke() removes all access'] },
  { title: 'WeakMap, WeakSet & WeakRef', route: '/javascript', badge: 'Patterns', available: false,
    description: 'Weak references that don\'t prevent garbage collection — private data, caches, and memory-sensitive patterns.',
    keyPoints: ['WeakMap: keys must be objects; entries GC\'d when key is unreachable', 'WeakSet: tracks object existence without preventing GC', 'WeakRef: hold a reference that may be collected; use deref() to check', 'FinalizationRegistry: callback when object is GC\'d — for cleanup', 'Use WeakMap for private component data without memory leaks'] },
