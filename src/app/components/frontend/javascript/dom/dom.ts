...
createVirtualList(document.querySelector('#list'), items);`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A function reads element.offsetHeight, then calls a THIRD-PARTY library function you don\'t control, then reads offsetHeight again. Could this innocent-looking code still cause layout thrashing even though you never explicitly wrote a DOM style in between your own two reads?',
      a: 'Yes — layout thrashing depends on whether ANY write to the DOM happened between the two reads, not on whether YOUR code wrote it; if the third-party function does anything that invalidates layout internally (sets a style, changes a class, inserts/removes an element, even reads certain properties that force a flush in some browsers), the second offsetHeight read still triggers a forced synchronous reflow, even though the write was hidden inside an opaque dependency. This is a real debugging trap: layout thrashing audits that only look at your own code\'s read/write ordering can miss thrashing caused by black-box library calls sandwiched between reads — profiling with the browser\'s Performance panel (looking for "Forced reflow" warnings) catches this regardless of where the offending write originates, which plain code review often cannot.',
    },
  ];
