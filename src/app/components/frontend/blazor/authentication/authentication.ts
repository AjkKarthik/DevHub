...
<nav>
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the difference between AuthenticationStateProvider and the AuthorizeView component?',
      a: 'AuthenticationStateProvider is the underlying service that supplies the current user\'s authentication state (claims, identity) to the rest of the app — you typically implement a custom one for non-standard auth flows. AuthorizeView is a declarative UI component that consumes that state to conditionally render content based on authorization (showing different markup for authenticated vs anonymous users, or based on roles/policies) without writing imperative C# checks in every component.' },
    { q: 'Why does Blazor WebAssembly authentication require different handling than Blazor Server authentication?',
      a: 'Blazor Server runs on the server with a persistent SignalR circuit, so it can directly access ASP.NET Core\'s cookie-based authentication and HttpContext like a traditional server app. Blazor WASM runs entirely in the browser with no server-side session — it must use token-based authentication (typically OIDC/OAuth2 via the Microsoft.AspNetCore.Components.WebAssembly.Authentication package) storing tokens client-side and attaching them to outgoing API calls, since there is no server-side session to rely on between requests.' },
  ];
