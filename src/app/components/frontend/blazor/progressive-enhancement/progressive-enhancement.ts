...
<!-- No data-enhance — full reload ensures cookie middleware runs -->`
  ...
  mistakes: CommonMistake[] = [
    ...
    },
    {
      ...
      explanation: 'Enhanced forms intercept the response and swap page content. A 302 redirect response after login needs a full browser navigation to set cookies and update the address bar correctly.'
    },
    {
...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'Which form type should NOT have data-enhance?', options: ['Search forms', 'Filter forms', 'Login/logout forms with redirect', 'Newsletter signup'], answer: 2, explanation: 'Login and logout flows involve server redirects and cookie operations that require a full browser navigation. data-enhance intercepts the redirect, breaking the auth flow.' },
    ...
    { q: 'What does <enhance> on a <form> or <a> element do in .NET 8 Blazor?', options: ['Adds CSS transitions', 'Enables enhanced navigation/form submission — JS intercepts the request and updates only the changed page region', 'Loads the page progressively', 'Enables streaming rendering'], answer: 1, explanation: 'data-enhance (the attribute form) opts individual elements into Blazor\'s enhanced navigation and forms. Instead of a full page reload, Blazor fetches the response and morphs only changed DOM regions. It is opt-in at the element level, giving fine-grained control over which links and forms use enhanced behaviour.' },
  ];
