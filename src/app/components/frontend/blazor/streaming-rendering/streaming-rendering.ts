...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'How do you opt out of enhanced navigation for a specific link in Blazor?', options: ['Use <a href="/page" data-no-enhance>', 'Use <a href="/page" data-enhance-nav="false">', 'Use NavigationManager.NavigateTo("/page", forceLoad: true)', 'Use <NavLink enhanced="false">'], answer: 2, explanation: 'NavigationManager.NavigateTo(url, forceLoad: true) triggers a full browser navigation, bypassing enhanced navigation. For links in markup, add data-enhance-nav="false" to the anchor element. Both are needed when navigating to pages that require a full server round-trip (external auth redirects, file downloads, etc.).' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A page has two components with slow data dependencies: one takes 200ms, the other takes 3 seconds, and only the slow one has [StreamRendering] applied. What does the user actually see, and in what order?',
      a: 'The fast (200ms) component, having no [StreamRendering] attribute, is part of the "initial complete render" — but since it\'s on the same page as a streaming component, the overall response still streams: the initial flush includes the fast component\'s already-resolved content plus a placeholder for the slow component\'s streamed region (the page as a whole waits for the fast component since it has no placeholder mechanism of its own, but doesn\'t wait for the slow one). Once the 3-second dependency resolves, Blazor sends a follow-up chunk that patches just that placeholder region in the already-delivered DOM — so practically, the user sees the fast component\'s real content plus a loading indicator for the slow one almost immediately, then the slow component\'s content pops in ~3 seconds later without a full page reload or re-render of the fast component.' },
    { q: 'What is the relationship between streaming rendering and the @attribute [StreamRendering] directive?',
      a: 'Streaming rendering is opt-in per component — adding @attribute [StreamRendering(true)] (or [StreamRendering] in newer syntax) at the top of a .razor file tells Blazor that this specific component supports being rendered in multiple streamed updates rather than requiring a single complete render. Without this attribute, the component renders normally (waiting for all its async work before producing output), meaning you selectively enable streaming only for components with genuinely slow data dependencies where progressive display provides real user-facing benefit.' },
  ];
