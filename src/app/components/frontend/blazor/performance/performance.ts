...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'Why does implementing IEqualityComparer on a component parameter reduce renders?', options: ['It speeds up JSON serialization', 'Blazor uses it to detect whether a parameter actually changed before calling SetParametersAsync', 'It bypasses the diffing algorithm', 'It is required for generic components'], answer: 1, explanation: 'By default, Blazor calls SetParametersAsync (and re-renders) whenever the parent re-renders, even if the parameter value hasn\'t changed. Implementing IEqualityComparer or checking parameter equality in ShouldRender lets you skip the render when nothing meaningful changed — especially valuable for expensive list items.' },
  ];
