...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'Why does implementing IEqualityComparer on a component parameter reduce renders?', options: ['It speeds up JSON serialization', 'Blazor uses it to detect whether a parameter actually changed before calling SetParametersAsync', 'It bypasses the diffing algorithm', 'It is required for generic components'], answer: 1, explanation: 'Blazor already skips SetParametersAsync by default when EVERY parameter is a primitive/immutable value (numbers, strings, bool, enums, Guid, EventCallback) that provably hasn\'t changed — but for any reference-type parameter (a custom class, record, or collection), it always re-invokes SetParametersAsync regardless of value equality, since it can\'t safely assume the object hasn\'t been mutated. A manual equality check (in ShouldRender or SetParametersAsync) is what lets you skip the render for THAT case — especially valuable for expensive list items bound to record or object parameters.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Why does excessive use of @bind on every keystroke (oninput) hurt performance in Blazor Server specifically?',
      a: 'In Blazor Server, every bound update is sent over the SignalR connection to the server, processed, and a re-render diff is sent back — unlike Blazor WASM where everything happens locally in the browser. Binding with @bind:event="oninput" on a text field triggers this round-trip on every single keystroke, which for a fast typist or a slow/high-latency network connection can introduce visible lag or flood the server with rapid updates; debouncing the input (updating local state immediately but only triggering the bound C# update after a pause) mitigates this.' },
    { q: 'A list rendered with @key="item.Id" contains items with duplicate Id values (a data bug upstream). What happens to Blazor\'s diffing, and why is this worse than simply omitting @key entirely?',
      a: 'Blazor\'s diffing assumes @key values are unique identities within a render — when two elements share the same key, the differ cannot reliably distinguish which physical DOM node corresponds to which logical item, and its behavior in that case is undefined from the app\'s perspective: it may reuse/move the wrong node, attach the wrong component state to the wrong visible row, or throw at runtime depending on the exact diff path taken. This is worse than having no @key at all (which at least falls back to consistent, if less efficient, positional diffing) because a broken key assumption can silently misattribute state between rows — e.g. an input\'s typed text or a checked checkbox appearing to "jump" to a different row after a re-render. The takeaway: @key is only safe to add once the underlying data\'s identity values are actually guaranteed unique.' },
  ];
