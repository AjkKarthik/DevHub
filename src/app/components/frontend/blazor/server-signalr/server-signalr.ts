...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between IHubContext<T> and Hub<T> in SignalR?', options: ['They are identical', 'Hub<T> is used inside a hub class; IHubContext<T> is injected outside a hub (services, controllers, background tasks) to send messages', 'IHubContext is for server-to-server; Hub is for client-to-server', 'Hub<T> supports strongly-typed clients; IHubContext does not'], answer: 1, explanation: 'A Hub is an ephemeral object created per incoming connection message — it has context (caller, groups). IHubContext<T> is a singleton service that lets you send messages to clients from anywhere outside an active hub invocation: background services, timer callbacks, REST controllers, etc.' },
  ];
