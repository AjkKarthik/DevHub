...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'When does Blazor call Dispose() (or DisposeAsync()) on a component?', options: ['After every render', 'When the component is removed from the render tree (navigation away or conditional rendering)', 'When StateHasChanged is called', 'At the end of each circuit lifetime'], answer: 1, explanation: 'Blazor calls Dispose/DisposeAsync when the component is removed from the render tree — typically on navigation or when an @if condition removes it. Implement IDisposable or IAsyncDisposable to clean up timers, subscriptions, CancellationTokenSources, and DotNetObjectReferences.' },
  ];
