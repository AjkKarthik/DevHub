...
@code {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What should you set on Virtualize items to prevent DOM remounting on list reorder?', options: ['id attribute', '@key="item.Id"', 'style="stable"', 'No special attribute needed'], answer: 1, explanation: '@key="item.Id" gives each Virtualize item a stable identity. Without it, when the list is filtered or reordered, Blazor destroys and recreates DOM nodes rather than moving them — causing input state loss (scroll position, focused element) and unnecessary repaints.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'An ItemsProvider callback that fetches from a slow remote API is invoked repeatedly as the user scrolls rapidly through a large virtualized list. What problem does this create, and how does the ItemsProviderRequest\'s CancellationToken address it?',
      a: 'Rapid scrolling triggers a new ItemsProvider call for each newly-visible window before the previous call\'s data has necessarily arrived — without cancellation, a slow, now-stale request can resolve AFTER a more recent one and overwrite the correctly-fetched current view with outdated data, or simply waste bandwidth and server load fetching data the user has already scrolled past. The CancellationToken passed in ItemsProviderRequest is triggered by Virtualize when a newer request supersedes an in-flight one, so a well-behaved provider should pass that token through to its underlying HTTP call (or database query) so the stale request is aborted rather than completing and racing the fresher one.' },
  ];
