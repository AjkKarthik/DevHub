...
@code {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A component calls NavigateTo with forceLoad: true inside an event handler that has NOT awaited anything yet. Does the current handler code after the NavigateTo call still finish executing?',
      a: 'Yes, briefly — NavigateTo itself does not synchronously halt execution or throw; it schedules the navigation/page reload and returns control to the caller, so any remaining synchronous code in the same event handler still runs until the method returns (or until the browser actually tears down the page, which is asynchronous from the app\'s perspective). This is a common source of subtle bugs: code written after NavigateTo(url, forceLoad: true) assuming "we\'ve left this page now" can still execute — for example, a subsequent StateHasChanged() or state mutation on a component that is about to be torn down — so cleanup logic that must run before navigation should execute BEFORE calling NavigateTo, not after.' },
  ];
