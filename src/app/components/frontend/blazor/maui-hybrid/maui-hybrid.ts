...
<p>OS Version: @DeviceInfo.OsVersion</p>`
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A BlazorWebView-hosted app calls a native MAUI API (like the camera) from a Razor component, then the same component library is reused in a plain web Blazor project. What breaks, and how does the shared-RCL-with-interfaces pattern prevent a hard compile failure?',
      a: 'The MAUI-specific camera API type simply does not exist in a web project\'s reference set, so any component directly calling it fails to compile outside MAUI — the fix (already covered by wrapping platform APIs behind interfaces in the shared RCL) is that the Razor component only ever calls an injected interface like ICameraService, and each host project registers its own implementation: the MAUI project registers a real implementation backed by the native camera API, while the web project registers a stub or browser-based implementation (or throws NotSupportedException) behind the same interface — so the component code compiles and runs unmodified in both, with the platform-specific behavior resolved entirely through DI at each host\'s startup rather than through conditional compilation in the component itself.' },
