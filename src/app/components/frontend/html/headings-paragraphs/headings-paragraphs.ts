...
`
  ...
  qna: QnaItem[] = [
    ...
    { q: 'A page has `<strong><strong>Danger</strong></strong>` — the author nested two <strong> tags around the same text expecting it to be announced with EXTRA emphasis by a screen reader. Does nesting semantic tags like this actually increase the announced importance?', a: 'No — nesting <strong> or <em> tags does not compound or intensify how assistive technology announces the text; screen readers treat the semantic role as binary (this text has strong importance / this text does not), not as a scale that stacks with repetition, so `<strong><strong>Danger</strong></strong>` is announced identically to a single `<strong>Danger</strong>`. This is a common misunderstanding that treats semantic HTML like CSS font-weight (where you might imagine "more bold" from stacking), but semantic elements convey a category of meaning, not an intensity value — if truly greater urgency needs to be communicated, that requires different content or an ARIA live region with assertive politeness, not repeated nesting of the same semantic tag.' },
