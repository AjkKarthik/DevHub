...
export class HtmlInputTypes {
  ...
  qna: QnaItem[] = [
    { q: 'A password-change form sets autocomplete="off" on the "new password" field to prevent the browser from suggesting the user\'s OLD saved password. Does this reliably work across modern browsers?', a: 'No, not reliably — most modern browsers (Chrome, Firefox, Edge) deliberately IGNORE autocomplete="off" specifically for username/password fields as a UX decision, reasoning that password managers are a security benefit users should not be able to accidentally disable via a form author\'s attribute. The actually-supported way to signal "this is a new password, not the account\'s current one" is autocomplete="new-password" (a specific value the password-manager UI understands and treats differently, offering to generate a new password instead of autofilling the old one), not autocomplete="off". This is a common source of confusion: developers reach for "off" expecting it to suppress autofill, when for password fields specifically the browser has already decided to override that value.' },
