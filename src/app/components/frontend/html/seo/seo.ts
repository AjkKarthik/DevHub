...
<body>
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: "What is the difference between noindex and disallow in robots.txt?", options: ["They are identical", "noindex (meta tag/header) tells indexers not to index the page; disallow (robots.txt) tells crawlers not to visit it at all", "disallow prevents all bots; noindex allows crawling", "robots.txt is only for images"], answer: 1, explanation: "robots.txt Disallow prevents crawlers from fetching a URL — Google will not crawl it (but may still index it if other pages link to it). noindex in a meta tag or HTTP header tells crawlers: you can crawl it but do not put it in search results. To fully remove a page: use both." }
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: "What is the impact of Core Web Vitals on SEO?", a: "Google uses Core Web Vitals (LCP, CLS, INP) as a ranking signal via the Page Experience update. Good scores are a tie-breaker — great content still ranks above a fast but thin page. However, meeting 'Good' thresholds (LCP &lt; 2.5s, CLS &lt; 0.1, INP &lt; 200ms) prevents a ranking penalty. Measure with PageSpeed Insights, Search Console's Core Web Vitals report, and Lighthouse." },
    { q: "How does JavaScript-rendered content affect SEO?", a: "Googlebot can execute JavaScript, but crawling and indexing JS-rendered content is slower (Google must first crawl the URL, then schedule JavaScript rendering in a second wave). Prefer server-side rendering (SSR) or static generation for SEO-critical content — it is in the HTML on first request. Use SSG/SSR for landing pages, blog posts, product pages. Client-side-only rendering is fine for dashboards and auth-only areas." }
  ];
