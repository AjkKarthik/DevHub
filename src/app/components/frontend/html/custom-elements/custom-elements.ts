...
customElements.define('user-card', UserCard);`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'A team chooses "closed" Shadow DOM mode specifically to prevent third-party scripts on the page from tampering with their component\'s internal DOM. Does closed mode actually achieve this goal against a determined attacker script?',
      a: 'Not reliably — closed mode blocks the STANDARD access path (element.shadowRoot returns null), but it is not a hard security boundary: a script can still reach the shadow root by monkey-patching Element.prototype.attachShadow BEFORE the component\'s own code runs, capturing the shadow root reference at creation time for every element on the page, or by using browser devtools which can inspect closed shadow roots regardless. Closed mode is better understood as an API-ergonomics signal ("this internal structure is not meant to be part of your public contract, please don\'t rely on it") rather than a genuine security or tamper-protection mechanism — actual security-sensitive data should never be exposed via the DOM at all, shadow or otherwise, since the DOM is fundamentally a shared, inspectable structure once a script has execution context on the page.',
    },
    {
