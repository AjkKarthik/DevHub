...
export class HtmlStorageApis {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'How is data shared between browser tabs using Web Storage?', options: ['sessionStorage is shared between all tabs for the same origin', 'localStorage is shared between all tabs; sessionStorage is isolated per tab', 'Neither is shared between tabs', 'Both are shared between tabs'], answer: 1, explanation: 'localStorage is shared across all tabs and windows from the same origin — a change in one tab is visible in others (triggering the "storage" event). sessionStorage is isolated per tab, even for the same URL. Opening a new tab to the same page creates a NEW sessionStorage — it does not inherit the opener\'s session data.' }
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Can localStorage store JavaScript objects directly?', a: 'No — localStorage only stores strings. To store objects, serialize with JSON.stringify() and deserialize with JSON.parse(): <code>localStorage.setItem("user", JSON.stringify({ name: "Alice" }))</code> and <code>JSON.parse(localStorage.getItem("user"))</code>. Forgetting this leads to "[object Object]" being stored, which is a common bug. Always wrap JSON.parse in try/catch since stored data could be corrupted.' },
    { q: 'What is the Storage event and when does it fire?', a: 'The "storage" event fires on the window object whenever localStorage is modified — but only in OTHER tabs/windows that share the same origin, not the tab that made the change. This enables cross-tab communication: <code>window.addEventListener("storage", e => console.log(e.key, e.newValue))</code>. The event includes key, oldValue, newValue, url, and storageArea. sessionStorage does not trigger storage events between tabs.' },
    { q: 'When should you use IndexedDB instead of localStorage?', a: 'Use IndexedDB when: (1) you need to store more than ~5–10 MB of data, (2) data is structured and you need query/index support, (3) you need to store binary data (Blobs, ArrayBuffers) like cached images or files, (4) you need offline-first capabilities with complex data, (5) you are in a Service Worker (localStorage is not available in workers). IndexedDB has a complex callback/Promise API — libraries like Dexie.js make it far easier to use.' },
    { q: 'What happens to Web Storage data in private/incognito mode?', a: 'In private/incognito mode, browsers still support localStorage and sessionStorage within that session, but clear ALL storage when the private session ends. localStorage in a private tab does not persist beyond the session and is not shared with the non-private localStorage. Additionally, Safari and Firefox may limit storage quota in private mode. Always handle storage operations defensively (try/catch QuotaExceededError).' }
  ];
