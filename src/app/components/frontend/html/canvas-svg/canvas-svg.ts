...
<body>
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'Which SVG element is used to apply reusable visual effects to multiple elements?', options: ['<use>', '<defs>', '<symbol>', '<filter>'], answer: 1, explanation: '<defs> defines reusable elements (gradients, filters, clip paths) that are not rendered directly. Other elements reference them with url(#id). <filter> goes inside <defs> to define blur, colour effects, etc.' }
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you make a Canvas drawing accessible to screen reader users?', a: 'Canvas is a pixel grid with no inherent semantics. To make it accessible: (1) provide a meaningful text alternative inside the canvas element (fallback content for browsers that do not support canvas), (2) add aria-label or aria-labelledby to the canvas element, (3) maintain a parallel DOM (hidden offscreen list or ARIA live region) that describes the data. For charts, prefer SVG — its DOM is inspectable by assistive technology directly.' },
    { q: 'When should you choose SVG over Canvas for data visualisation?', a: 'Choose SVG when: (1) you need accessibility (SVG elements have roles and text content), (2) the user must interact with individual elements (hover, click on a specific bar), (3) data set is small-to-medium (SVG struggles above ~10k elements), or (4) you need crisp vector output at any zoom. Choose Canvas when: rendering many thousands of objects per frame, doing pixel-level manipulation (image processing, games, WebGL alternative).' }
  ];
