...
// Always wrap JSON.parse in try/catch + retry logic`,
  ...
  { q: 'Why might a researcher deliberately choose a base model over an instruction-tuned model for a specific project?', a: 'Instruction tuning and RLHF optimize a model to be a helpful, safe conversational assistant — this reshapes its output distribution toward "assistant-like" responses, which can actually hurt tasks that need the model\'s raw, unfiltered text-completion behavior: studying what the pretraining corpus encoded, generating diverse creative continuations without the assistant\'s characteristic hedging/refusal patterns, or as a from-scratch starting point for a custom fine-tune with a very different target behavior than a general assistant (e.g. a narrow domain-specific classifier) where the instruction-tuned model\'s existing habits would need to be "un-learned" first.' },
