...
function buildWeatherToolSchema(): ToolSchema {
  ...
  { q: 'What is the ReAct prompting pattern?', options: ['Restart and Correct', 'Reasoning and Acting: alternate reasoning traces with tool actions', 'Retrieve and Consolidate', 'Refine and Complete'], answer: 1, explanation: 'ReAct (Yao et al.): prompts the model to produce Thought -> Action -> Observation cycles. Reasoning traces improve action selection; observations from tool use inform the next reasoning step. Foundation for most agent frameworks.' },
  ...
  { q: 'What is the difference between single-agent and multi-agent architectures?', a: 'Single-agent: one LLM handles all reasoning and tool use — simpler, easier to debug. Multi-agent: multiple specialized agents collaborate; an orchestrator routes subtasks (researcher agent, coder agent, reviewer agent). Multi-agent handles parallelism and specialization but adds coordination complexity. Use multi-agent when tasks exceed a single agent context window or benefit from parallel specialized work.' },
