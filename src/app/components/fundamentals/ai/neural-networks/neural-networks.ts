...
function reluBackward(z: number[], dOut: number[]): number[] {
  ...
  { q: 'How does backpropagation calculate gradients for a neural network?', a: 'Backprop applies the chain rule in two passes: (1) Forward pass: compute activations layer by layer, store intermediate values; (2) Backward pass: starting from the loss, compute gradient w.r.t. each layer\'s weights by multiplying upstream gradient by local Jacobian. For a layer y = W*x + b: dL/dW = dL/dy * x^T; dL/dx = W^T * dL/dy. Automatic differentiation (PyTorch autograd) builds a computation graph and differentiates it automatically.' },
