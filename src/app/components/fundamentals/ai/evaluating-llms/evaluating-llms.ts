...
# Or: human evaluation on 10% sample to calibrate the LLM judge`,
  ...
  { q: 'What task was BLEU originally designed for, and why does that origin explain its weaknesses on chatbot evaluation?', options: ['BLEU was designed for image captioning', 'BLEU was designed for machine translation, where there is usually one narrow correct phrasing — a fit that breaks down for open-ended chat where many differently-worded answers can all be equally correct', 'BLEU was designed for code generation and works best there', 'BLEU has no particular original use case'], answer: 1, explanation: 'BLEU was built to score machine translation output, a task where a source sentence has a small number of acceptable reference translations that share most of the same words and phrase structure — n-gram overlap is a reasonable proxy for quality there. Open-ended chatbot responses have no such narrow target: two equally good answers to "explain photosynthesis" can use almost entirely different words, so BLEU\'s word-overlap assumption, valid for translation, actively penalizes good chatbot answers that just happen to phrase things differently from a single reference.' },
  { q: 'What is LLM-as-a-Judge evaluation?', options: ['Using an LLM to judge code quality only', 'Using a powerful LLM (GPT-4, Claude) to score another model\'s responses on rubrics like helpfulness and accuracy', 'Fine-tuning an LLM for evaluation', 'A human judge rating LLM outputs'], answer: 1, explanation: 'LLM-as-judge: use a powerful LLM to evaluate responses on rubrics (correctness, helpfulness, harmlessness). Scales better than human eval, correlates well with human preferences. Risks: position bias (favors first response), self-enhancement bias (GPT-4 favors GPT-4 outputs).' },
  { q: 'What is the purpose of a benchmark like MMLU or HumanEval?', options: ['To train LLMs', 'To measure specific capabilities (knowledge, coding) across standardized tasks for model comparison', 'To measure inference speed', 'To test LLM safety only'], answer: 1, explanation: 'MMLU: 57 academic subjects, multiple choice — measures world knowledge. HumanEval: coding tasks with unit test verification. HellaSwag: commonsense reasoning. MATH: math problem solving. Benchmarks allow objective comparison but can be gamed by training on benchmark data (contamination).' },
  ];
  ...
  { q: 'How do you build a domain-specific evaluation set for your LLM application?', a: 'Steps: (1) Sample 100-200 real user queries from production logs or anticipated use cases; (2) Include diverse inputs: common cases, edge cases, adversarial inputs; (3) Add golden answers labeled by domain experts; (4) Define rubrics: correctness, completeness, format adherence; (5) Mix automated checks (exact match, regex, JSON schema validation) with model-graded criteria. Refresh the eval set quarterly as usage patterns evolve.' },
  { q: 'What is the difference between perplexity and downstream task accuracy for evaluating language models?', a: 'Perplexity: measures how well the model predicts a text corpus — lower = better. Good for comparing base models trained on the same distribution. Limitation: low perplexity does not guarantee good task performance. Downstream accuracy: measures performance on specific tasks (classification, QA, summarization). Preferred for evaluation of instruction-tuned/RLHF models. Use both: perplexity for pretraining, task accuracy for fine-tuned models.' },
  { q: 'What is red-teaming for LLMs and how is it different from standard evaluation?', a: 'Red-teaming: adversarial testing to find failure modes — jailbreaks, prompt injections, harmful outputs, hallucinations under specific conditions. Standard eval: measures average-case performance on a curated benchmark. Red-teaming: worst-case exploration, often manual. Process: define threat model (what harms are possible), create adversarial prompts, document failures, measure attack success rate. Required for safety-critical deployments.' },
  { q: 'How do you detect hallucination in LLM outputs?', a: 'Techniques: (1) Factual grounding: compare generated facts against retrieved sources (RAG pipeline — does the answer cite the source?); (2) Consistency check: ask the same question multiple times with different phrasings — inconsistent answers indicate uncertainty; (3) Entailment model: check if the generated claim is entailed by source documents; (4) Knowledge-based: compare against a fact database (Wikidata). Self-consistency (majority vote across multiple samples) reduces hallucination rate.' },
  ];
