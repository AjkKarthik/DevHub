...
X_pca = pca.fit_transform(X_scaled)`,
  ...
  { q: 'What are the two hyperparameters DBSCAN requires, and what happens if epsilon is set too small?', options: ['n_clusters and random_state — the same as K-Means', 'epsilon (neighborhood radius) and minPts (minimum points to form a dense region) — too small an epsilon classifies almost every point as noise', 'Only minPts is required; epsilon is inferred automatically', 'learning_rate and max_iter'], answer: 1, explanation: 'DBSCAN needs epsilon (the radius defining a point\'s neighborhood) and minPts (how many neighbors within that radius make a point a "core" point). If epsilon is too small, most points won\'t have enough neighbors within range to be core points, so DBSCAN labels nearly everything as noise (-1) instead of forming real clusters — a k-distance plot is commonly used to pick a reasonable epsilon.' },
