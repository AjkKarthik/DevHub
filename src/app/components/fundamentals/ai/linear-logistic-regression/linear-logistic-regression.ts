...
function gradientStep(
  ...
  { q: 'What does regularization (L1/L2) do to linear model coefficients?', options: ['Increases all coefficients', 'Penalizes large coefficients, shrinking them toward zero to reduce overfitting', 'Removes correlated features automatically', 'Increases model complexity'], answer: 1, explanation: 'L2 (Ridge): adds sum(w^2) to the loss — shrinks all coefficients proportionally. L1 (Lasso): adds sum(|w|) — drives some coefficients exactly to zero (sparse model, feature selection). ElasticNet combines both. Higher regularization strength -> stronger shrinkage.' },
  ...
  { q: 'When does multicollinearity cause problems in linear regression?', options: ['Always, regardless of model purpose', 'When coefficient interpretation matters — correlated features cause unstable coefficient estimates', 'Only in logistic regression', 'Only when n < p (more features than samples)'], answer: 1, explanation: 'Multicollinearity: when predictors are highly correlated, their individual coefficients are poorly estimated (large variance, unstable across samples) but the overall predictions remain accurate. Problem for interpretation; less so for prediction. Solution: regularization (Ridge), PCA preprocessing, or dropping redundant features.' },
  ];
  ...
  { q: 'How do you interpret the coefficients of a logistic regression model?', a: 'Each coefficient w_i represents the change in log-odds per unit increase in feature x_i. Log-odds to probability: p = sigmoid(w^T x + b). Odds ratio: exp(w_i) = multiplicative change in odds when x_i increases by 1. Example: w_i = 0.5 means each unit increase in x_i multiplies the odds by exp(0.5) = 1.65. For standardized features, coefficients are comparable in magnitude. Interpretation assumes other features are held constant.' },
  { q: 'What is gradient descent and how does it train linear models?', a: 'Gradient descent: iteratively update weights in the direction of the negative gradient of the loss. w = w - lr * gradient. For MSE loss: gradient = -2/n * X^T (y - Xw). Variants: batch GD (all data per step), SGD (one sample), mini-batch GD (small batch). Linear regression has a closed-form solution (normal equation) for small datasets. For large n: SGD (scikit-learn SGDClassifier/SGDRegressor) is preferred. Learning rate: too large = diverge; too small = slow convergence.' },
  { q: 'How do you handle imbalanced classes in logistic regression?', a: 'Techniques: (1) class_weight=balanced in scikit-learn: re-weights samples inversely proportional to class frequency; (2) Adjust classification threshold (default 0.5 — lower threshold to increase recall for minority class); (3) Oversampling minority class (SMOTE) or undersampling majority; (4) Use precision-recall AUC or F1 instead of accuracy as the evaluation metric. For severe imbalance (1:100+), consider anomaly detection approaches.' },
  ];
