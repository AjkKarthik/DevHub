...
console.log(cosineSimilarity(a, b).toFixed(6)); // same value`,
  ...
  { q: 'How do you choose between different vector databases (Pinecone, Weaviate, pgvector)?', a: 'pgvector: PostgreSQL extension — ideal if you already use Postgres, want SQL query capabilities alongside vector search, and have moderate scale (<1M vectors). No extra infrastructure. Pinecone: managed, serverless, scales to billions of vectors, no infrastructure management, fast. Best for pure vector search at scale. Weaviate/Qdrant: open-source, self-hosted or cloud, support hybrid search (BM25 + vector), filtering, and multi-tenancy. Choose based on existing infrastructure, scale, and need for hybrid search.' },
