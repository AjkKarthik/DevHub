...
// Reduction: 0.78% of original`,
  ...
  { q: 'What is the difference between full fine-tuning and parameter-efficient fine-tuning (PEFT)?', options: ['Full fine-tuning uses fewer parameters', 'Full fine-tuning updates all model weights; PEFT (LoRA, prefix tuning) updates a small subset', 'PEFT is only for image models', 'Full fine-tuning does not change the model architecture'], answer: 1, explanation: 'Full fine-tuning updates all billions of parameters — expensive (GPU memory = model size + gradients + optimizer states). PEFT methods (LoRA: train low-rank adapters; prefix tuning: train soft prompts) update <1% of parameters with minimal performance loss.' },
  { q: 'What is LoRA (Low-Rank Adaptation) and how does it reduce training cost?', options: ['A learning rate scheduler', 'Adds trainable low-rank matrices to existing weight matrices; only these adapters are trained', 'A data augmentation technique', 'A tokenization method'], answer: 1, explanation: 'LoRA decomposes weight updates as W = W0 + BA where B and A are low-rank matrices (rank r << min(d_in, d_out)). Only B and A are trained. A 7B model update from 14GB to <100MB of adapter weights. Adapters can be merged into base weights at inference for zero overhead.' },
  { q: 'When should you fine-tune an LLM vs use prompt engineering or RAG?', options: ['Always fine-tune for best performance', 'Fine-tune when prompt engineering and RAG cannot meet quality/latency requirements and you have >500 high-quality examples', 'Fine-tune first, then add RAG if needed', 'Fine-tune only for coding tasks'], answer: 1, explanation: 'Decision order: (1) Prompt engineering (free, instant); (2) RAG (no training, handles factual/knowledge tasks); (3) Fine-tuning (expensive, takes time, needs quality data, best for style/format adaptation, not factual knowledge). Fine-tune when output format/tone is critical, not to add knowledge.' },
  ];
  ...
  { q: 'What data quality requirements matter most for LLM fine-tuning?', a: 'Quality over quantity: 500 high-quality examples outperform 50,000 noisy ones. Requirements: (1) Diverse inputs covering the full distribution of target use cases; (2) Consistent formatting matching the inference prompt format exactly; (3) Accurate, expert-labeled outputs; (4) Filtered for harmful content, PII, and duplicates. Use a data flywheel: collect real production examples, label the hardest ones, retrain. Aim for 50-500 examples per task category.' },
  { q: 'What is catastrophic forgetting in fine-tuning and how do you mitigate it?', a: 'Catastrophic forgetting: fine-tuning on a new task causes the model to lose capabilities from pretraining. Mitigation: (1) LoRA/PEFT — adapters do not modify base weights, preserving general capabilities; (2) Replay: mix some original pretraining data into the fine-tuning set; (3) Low learning rate: smaller updates preserve more of the base model; (4) Early stopping on a held-out general benchmark to catch regression. Full fine-tuning with high LR on small datasets has the highest forgetting risk.' },
  { q: 'How do you evaluate the quality of a fine-tuned model?', a: 'Evaluation strategy: (1) Held-out set from the same distribution as training data (direct task quality); (2) General capability benchmarks (MMLU, HellaSwag) to catch regression; (3) Human preference evaluation (head-to-head vs base model on target use cases); (4) Safety/alignment evals (ToxiGen, BBQ for bias). Compare to baseline (base model + best prompt) to confirm fine-tuning actually helped. Track prompt format sensitivity — fine-tuned models can be brittle to format changes.' },
  { q: 'What is RLHF and how does it differ from supervised fine-tuning?', a: 'SFT (Supervised Fine-Tuning): train on human-labeled input-output pairs. RLHF (Reinforcement Learning from Human Feedback): (1) SFT on demonstrations; (2) Reward model trained on human preference pairs (A vs B); (3) RL optimization (PPO) to maximize reward model score while staying close to SFT model (KL penalty). RLHF produces more helpful and aligned outputs but is complex and expensive. DPO (Direct Preference Optimization) is a simpler RLHF alternative that directly trains on preference pairs without a separate reward model.' },
  ];
