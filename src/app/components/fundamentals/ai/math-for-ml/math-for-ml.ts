...
const loss = -(y * Math.log(prob + eps) + (1-y) * Math.log(1-prob + eps));`,
  ...
  { q: 'What does the Hessian (second derivative) of a loss function tell you that the gradient alone does not?', options: ['Nothing extra — it is redundant with the gradient', 'The curvature of the loss surface at that point, which determines how confidently and how far you can safely step', 'The learning rate to use', 'Only whether the function is continuous'], answer: 1, explanation: 'The gradient gives direction and steepness at a single point, but says nothing about how quickly that steepness changes nearby. The Hessian (matrix of second derivatives) describes curvature — high curvature means the loss changes direction sharply, so a large step could overshoot the minimum, while low curvature means it is safer to take bigger steps. Second-order optimization methods (Newton\'s method) use the Hessian directly, though it is expensive to compute for large neural networks, which is why first-order methods (SGD, Adam) dominate in practice.' },
  { q: 'Why does the chain rule cause gradients to vanish in very deep networks?', options: ['It does not — vanishing gradients are unrelated to the chain rule', 'The chain rule multiplies many local derivatives together layer by layer; if each is consistently less than 1 (e.g. sigmoid\'s max derivative is 0.25), the product shrinks exponentially with depth toward zero', 'The chain rule only applies to the first layer', 'Gradients vanish because of floating-point rounding, not the chain rule itself'], answer: 1, explanation: 'Backpropagation\'s repeated chain-rule multiplication means the gradient reaching an early layer is the PRODUCT of every local derivative between that layer and the loss. If those local derivatives are each small fractions (as with saturating activations like sigmoid/tanh), multiplying dozens of them together shrinks the gradient exponentially — by the time it reaches early layers, it is effectively zero, so those layers barely update. This is precisely why ReLU (whose derivative is 1 for positive inputs, not a small fraction) and residual/skip connections were introduced to keep gradients from vanishing in deep networks.' },
