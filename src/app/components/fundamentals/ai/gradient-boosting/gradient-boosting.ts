...
preds = scaler.inverse_transform(model.predict(X_test).reshape(-1,1))`,
  ...
  { q: 'What is the relationship between learning rate and the number of trees needed in gradient boosting?', options: ['They are unrelated — n_estimators is always fixed at 100', 'A smaller learning rate requires MORE trees to reach the same fit quality, but tends to generalize better; a larger learning rate needs fewer trees but risks overfitting faster', 'A smaller learning rate always needs fewer trees', 'Learning rate only affects training speed, not the final model quality'], answer: 1, explanation: 'Each tree in gradient boosting contributes learning_rate × its_prediction to the ensemble. A small learning rate (e.g. 0.01) means each tree corrects only a little of the remaining error, so many more trees are needed to fully fit the data — but this gradual correction tends to generalize better than a few large corrective steps from a high learning rate, which is why learning_rate and n_estimators (often paired with early stopping) are tuned together, not independently.' },
  ];
