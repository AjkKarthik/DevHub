...
Final answer (number only):\`;
  ...
  { q: 'Why can chain-of-thought prompting sometimes make a model\'s answer WORSE on simple factual-recall questions?', options: ['CoT never makes answers worse', 'Forcing reasoning steps on a question that requires none can lead the model to talk itself into second-guessing a correct immediate recall, or to rationalize an incorrect answer with plausible-sounding but wrong reasoning', 'CoT always increases API latency past the timeout', 'CoT disables the model\'s access to its training data'], answer: 1, explanation: 'CoT helps most when a task genuinely benefits from decomposition (math, multi-step logic) — for a simple factual lookup ("what year did X happen"), forcing the model to "think step by step" can prompt it to generate plausible-sounding intermediate reasoning that talks it away from a correct direct-recall answer, or to construct a confident-sounding justification for a wrong guess. This is why CoT should be applied selectively based on task type, not blanket-added to every prompt.' },
