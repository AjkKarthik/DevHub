...
export class AiResponsibleAi {
  ...
  { q: 'Does the EU AI Act apply to a US-based company that never has a physical presence in the EU?', options: ['No — it only applies to companies headquartered or incorporated in the EU', 'Yes — the Act has extraterritorial reach: it applies to any provider whose AI system\'s output is used within the EU, regardless of where the provider is based', 'Only if the company has more than 500 employees', 'Only for AI systems classified as high-risk'], answer: 1, explanation: 'Like the GDPR before it, the EU AI Act applies based on where the AI system\'s OUTPUT is used, not where the provider is located — a US company with no EU offices or servers is still subject to the Act if its AI product\'s output reaches users in the EU. This extraterritorial scope means global AI providers generally must comply with the Act\'s requirements for any EU-facing product, rather than treating it as a regional concern only relevant to EU-based companies.' }
  ];
