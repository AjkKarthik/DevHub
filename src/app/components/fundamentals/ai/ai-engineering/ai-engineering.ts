...
function checkRateLimit(userId: string, cost: number): boolean {
  ...
  { q: 'What is an "indirect" prompt injection, and how does it differ from a user typing a malicious instruction directly?', a: 'Direct prompt injection is a user typing "ignore previous instructions" straight into the chat input. Indirect prompt injection hides the malicious instruction inside content the LLM reads as part of its task — a webpage the model summarizes, a PDF attached to an email-processing agent, or a retrieved document in a RAG pipeline — so the attacker never directly interacts with the application at all, and the injected content looks to the model exactly like legitimate task data. Indirect injection is harder to defend against because you cannot simply validate user-typed input; you must treat ALL retrieved/fetched content as untrusted too.' },
