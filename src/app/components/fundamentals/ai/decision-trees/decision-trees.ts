...
function bestThreshold(features: number[], labels: number[]): number {
  ...
  { q: 'What splitting criterion does a classification decision tree use?', options: ['Mean squared error', 'Gini impurity or information gain (entropy)', 'Euclidean distance', 'Correlation coefficient'], answer: 1, explanation: 'Gini impurity = 1 - sum(p_i^2): probability that a random element is misclassified. Information gain = entropy(parent) - weighted entropy(children). CART uses Gini by default; ID3/C4.5 use entropy. Lower impurity/higher gain = better split.' },
  { q: 'What is the bias-variance tradeoff for decision trees?', options: ['Deep trees have high bias; shallow trees have high variance', 'Shallow trees have high bias; deep trees have high variance', 'Trees always have balanced bias and variance', 'Tree depth does not affect bias or variance'], answer: 1, explanation: 'A deep (unpruned) tree has low bias (memorizes training data) but high variance (sensitive to training data noise, overfits). A shallow (pruned) tree has high bias (underfits) but low variance. Ensemble methods (Random Forest, Gradient Boosting) reduce variance by averaging many trees.' },
  { q: 'What is the key difference between Random Forest and a single Decision Tree?', options: ['Random Forest uses deeper trees', 'Random Forest trains many trees on bootstrap samples with random feature subsets, then averages predictions', 'Random Forest is always slower', 'Random Forest requires labeled data'], answer: 1, explanation: 'Random Forest: bagging (bootstrap samples) + random feature subsets at each split. Reduces overfitting by averaging many diverse trees. Single tree: prone to overfitting. RF is more robust but less interpretable. Feature importance is averaged across all trees.' },
  ];
