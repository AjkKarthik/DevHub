...
// x = x + Attention(LayerNorm(x))  — more stable, used by GPT-2, LLaMA, etc.`,
  ...
  { q: 'How does multi-head attention differ from single-head attention?', a: 'Multi-head attention runs h parallel attention operations (heads), each with its own Q, K, V projection matrices. Outputs are concatenated and projected. Each head can attend to different types of relationships simultaneously (one head tracks syntactic structure, another semantic similarity, another coreference). This allows the model to jointly attend to information from different representation subspaces. h=8 or 16 is typical; each head has dimension d_model/h.' },
