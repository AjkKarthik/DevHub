...
let count = 0; function dfs(n) { ... if(++count === k) result = n.val; ... }`,
  ...
  { q: 'Does inserting values in sorted order into a BST guarantee correctness, and what goes wrong?', a: 'Correctness yes — every insertion still respects the BST property. But performance collapses: inserting 1,2,3,4,5 in order produces a tree that is really a linked list (each node only has a right child), turning O(log n) search/insert into O(n). This is precisely why production ordered-map implementations (Red-Black trees, AVL trees) exist — they rebalance via rotations specifically to prevent this degenerate shape regardless of insertion order.' },
