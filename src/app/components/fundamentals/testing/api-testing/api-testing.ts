...
`describe('Products API', () => {
  ...
  { q: 'How do you test authentication in API tests?', a: 'Generate a test JWT in beforeEach: const token = jwt.sign({ userId: 1 }, testSecret); attach to requests: wait request(app).get(\'/api/me\').set(\'Authorization\', \Bearer \\). For OAuth, use a mock auth server or stub the token verification middleware. Test unauthorized access by omitting the token and expecting 401.' },
  { q: 'How do you test paginated API endpoints?', a: 'Test boundary cases: page 1 (returns first N items, nextPage link exists), last page (no nextPage), empty page (returns 200 with empty array, not 404). Verify total count header. Test invalid params (page=-1, limit=0) return 400. For cursor-based pagination, capture cursor from response and use in next request.' },
