...
test('charge saves receipt on success', () => {
  ...
  { q: 'What are the five types of test doubles?', options: ['Mock, Stub, Fake, Spy, Dummy', 'Mock, Stub, Real, Proxy, Virtual', 'Fake, Real, Partial, Full, Empty', 'Stub, Mock, Clone, Mirror, Shadow'], answer: 0, explanation: 'Gerard Meszaros classification: Dummy (unused placeholders), Stub (canned responses), Fake (working simplified implementation), Spy (records calls on top of real implementation), Mock (pre-programmed with expectations).' },
  { q: 'What distinguishes a Fake from a Stub?', options: ['Fakes are class-based, stubs are functions', 'Fakes have working implementations (e.g. in-memory DB); stubs return fixed values without logic', 'Stubs verify interactions, fakes do not', 'They are identical'], answer: 1, explanation: 'A fake has a real (simplified) implementation — e.g. an in-memory database that passes integration tests. A stub just returns fixed data without business logic. Fakes are useful when you need stateful behaviour across multiple calls.' },
  { q: 'When should you use a Spy vs a Mock?', options: ['Use Spy for async, Mock for sync', 'Use Spy when you want the real method to run but also track calls; use Mock when you want to replace behavior entirely', 'Use Mock for verification, Spy for returns', 'They serve the same purpose'], answer: 1, explanation: 'Spy wraps the real implementation and records interactions. Mock replaces the implementation entirely. Use spies for non-destructive observation; mocks when the real implementation has side effects you want to avoid.' },
  ];
  ...
  { q: 'Why is it a code smell if a single test needs 3+ Dummy objects to construct the system under test?', a: 'Needing many unused Dummy parameters to satisfy a constructor is usually a signal that the class under test has too many dependencies (violating single responsibility) or that its constructor requires collaborators that a specific behaviour genuinely does not need — a better fix is often extracting the tested behaviour into a smaller class with fewer dependencies, or using optional/default parameters, rather than continuing to pad every test with more Dummies to satisfy an overly large constructor.' },
  { q: 'How do you avoid over-mocking in tests?', a: 'Over-mocking symptoms: tests pass but production code is broken; tests are hard to maintain; every test has 10+ mock setups. Strategies: (1) Mock at the system boundary (HTTP, DB, filesystem), not within your domain logic; (2) Use real objects where fast and side-effect-free; (3) Apply the rule: if you would not subclass it, do not mock it; (4) Prefer fakes over mocks for complex collaborators.' },
  { q: 'What is partial mocking and when is it a code smell?', a: 'Partial mocking mocks some methods of a real class while leaving others real (jest.spyOn + mockImplementation on specific methods). A code smell when: the class does too many things (needs partial override to test one); coupling to implementation makes testing the method in isolation impossible. Prefer extracting responsibilities into separate classes that can be fully mocked or used real.' },
  ];
