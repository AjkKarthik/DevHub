...
test('profile page visual regression', async ({ page }) => {
  ...
  { q: 'What tool does Storybook use for visual regression testing?', options: ['Cypress visual plugin', 'Chromatic (visual testing cloud for Storybook stories)', 'Percy standalone', 'Playwright screenshots only'], answer: 1, explanation: 'Chromatic is purpose-built for Storybook — it captures screenshots of every story across browsers and shows diffs in a UI review workflow. Integrates with GitHub/GitLab PRs for visual change review.' },
  { q: 'What is the main challenge with pixel-by-pixel screenshot comparison?', options: ['Screenshots are too large', 'Minor rendering differences (antialiasing, subpixel rendering) cause false positives', 'Screenshots do not capture animations', 'Only Chrome is supported'], answer: 1, explanation: 'Pixel-perfect comparison is fragile — subpixel rendering, antialiasing, and font rendering vary between OS/browser versions, causing false positives. Tools like Percy and Chromatic use perceptual diffing algorithms with configurable thresholds.' },
  ];
