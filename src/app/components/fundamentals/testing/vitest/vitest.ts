...
// test: { globals: true } — then you can omit the imports above`,
  ...
  { q: 'How does Vitest workspace (vitest.workspace.ts) differ from running multiple separate test configs?', options: ['No functional difference from separate configs', 'It lets one Vitest run apply different configurations (environment, setup files) to different projects in a monorepo, sharing one process and one unified report', 'It only changes coverage report formatting', 'Workspaces only work in browser mode'], answer: 1, explanation: 'A vitest.workspace.ts file defines multiple named projects (e.g. environment: "jsdom" for a frontend package, environment: "node" for a backend package) that all run under one vitest invocation with a single merged report and shared watch mode — useful in monorepos needing different test environments without separate CI jobs per package.' },
