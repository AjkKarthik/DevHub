...
test('shows error when API fails', async () => {
  ...
  { q: 'What problem does Mock Service Worker (MSW) solve?', options: ['Test runner configuration', 'Intercepts HTTP requests at the network level so the app code is tested without real API calls', 'Generates API mocks from OpenAPI specs', 'Provides a test database'], answer: 1, explanation: 'MSW intercepts network requests using a Service Worker (browser) or Node HTTP interceptor. Your app code makes real fetch/axios calls; MSW intercepts at the network layer — no changes to application code for mocking.' },
  { q: 'How do you set up MSW for Jest/Node.js tests?', options: ['Use setupWorker()', 'Import setupServer(), define handlers, call server.listen() in beforeAll and server.close() in afterAll', 'Register a service worker in jest.setup.ts', 'Use msw.interceptFetch() in each test'], answer: 1, explanation: 'In Node.js/Jest: const server = setupServer(...handlers); beforeAll(() => server.listen()); afterEach(() => server.resetHandlers()); afterAll(() => server.close()).' },
  { q: 'How do you add a one-time handler override in MSW?', options: ['Recreate the server for each test', 'server.use() with runtime handlers that override defaults for that test only', 'Pass handlers in each describe block', 'Restart the service worker'], answer: 1, explanation: 'server.use(http.get(\'/api/users\', () => HttpResponse.json([], { status: 200 }))) overrides the default handler for the rest of the test. server.resetHandlers() in afterEach removes overrides. Used for testing error states.' },
  ];
  ...
  { q: 'How do you test error responses with MSW?', a: 'Add a handler that returns an error status: http.get(\'/api/users\', () => new HttpResponse(null, { status: 500 })). Or use HttpResponse.json({ message: \'Not found\' }, { status: 404 }). For network errors: http.get(\'/api\', () => HttpResponse.error()) — simulates a network failure. Use server.use() in individual tests to override the default happy-path handler.' },
  { q: 'What is the difference between MSW v1 and v2 handler syntax?', a: 'MSW v2 uses the Request/Response Web API directly. v1: rest.get(\'/api\', (req, res, ctx) => res(ctx.json(data))). v2: http.get(\'/api\', () => HttpResponse.json(data)). v2 is closer to standard fetch/Response APIs, supports streaming, and is tree-shakeable. Always use v2 for new projects — v1 syntax is deprecated.' },
  { q: 'How do you use MSW to test file upload or form data requests?', a: 'In the handler, parse the request body: http.post(\'/upload\', async ({ request }) => { const formData = await request.formData(); const file = formData.get(\'file\'); return HttpResponse.json({ name: file.name, size: file.size }) }). Assert the response: xpect(await response.json()).toMatchObject({ name: \'test.pdf\' }). Works with both FormData and multipart.' },
  ];
