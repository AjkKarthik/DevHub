...
test('completing a todo marks it done', async ({ page }) => {
  ...
  { q: 'What is the Playwright Page Object Model used for?', options: ['Managing test reporters', 'Encapsulating page interactions behind a class to reduce duplication and improve maintainability', 'Running tests in parallel', 'Intercepting network requests'], answer: 1, explanation: 'Page Object Model (POM) wraps page-specific actions in a class (e.g. LoginPage with login(), isLoggedIn()). Tests use the class instead of raw selectors — changes to UI require updating only the POM class.' },
