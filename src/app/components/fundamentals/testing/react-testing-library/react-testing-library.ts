...
test('hides results when input is cleared', async () => {
  ...
  { q: 'How do you test a component that makes API calls?', options: ['Use a real API in tests', 'Mock fetch/axios or use MSW to intercept and return test data', 'Skip async components', 'Test only synchronous components'], answer: 1, explanation: 'Use jest.mock() for axios/fetch, or MSW (Mock Service Worker) to intercept HTTP at the network level. Wrap assertions in waitFor() to handle async rendering: wait waitFor(() => expect(screen.getByText(\'Data loaded\')).toBeInTheDocument()).' },
  ];
