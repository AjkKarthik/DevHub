...
test('hides results when input is cleared', async () => {
  ...
  { q: 'What is the key philosophy of React Testing Library?', options: ['Test component implementation details', 'Test what users see and interact with, not internal component state', 'Test component lifecycle methods', 'Test Redux reducers in isolation'], answer: 1, explanation: 'RTL follows the guiding principle: tests should resemble how users use the software. Query by accessible roles, labels, and text — not by class names, IDs, or component internals.' },
  { q: 'Which RTL query is preferred for finding a button?', options: ['getByTestId(\'submit-btn\')', 'getByRole(\'button\', { name: /submit/i })', 'getByClassName(\'btn\')', 'querySelector(\'.submit\')'], answer: 1, explanation: 'getByRole(\'button\', { name: /submit/i }) mirrors how assistive technologies find elements — the most accessible and resilient query. getByTestId is a fallback for elements that have no accessible role or label.' },
  { q: 'How do you test a component that makes API calls?', options: ['Use a real API in tests', 'Mock fetch/axios or use MSW to intercept and return test data', 'Skip async components', 'Test only synchronous components'], answer: 1, explanation: 'Use jest.mock() for axios/fetch, or MSW (Mock Service Worker) to intercept HTTP at the network level. Wrap assertions in waitFor() to handle async rendering: await waitFor(() => expect(screen.getByText(\'Data loaded\')).toBeInTheDocument()).' },
  ];
  ...
  { q: 'How do you test user events in React Testing Library?', a: 'Use userEvent (preferred) over fireEvent. Setup: const user = userEvent.setup(). Then: await user.click(button) or await user.type(input, Hello World). userEvent simulates real browser behavior including focus, keydown, keyup, and input events in the correct sequence. fireEvent only dispatches a single DOM event.' },
  { q: 'How do you test React context in RTL?', a: 'Wrap the rendered component in the context provider: render with ThemeContext.Provider wrapping the component. Or create a custom render helper that wraps all tests with required providers and use this throughout tests. This avoids repeating provider setup in every test.' },
  { q: 'What is the screen object in RTL and why use it?', a: 'screen is a global-like object with all query methods bound to document.body. Preferred over destructuring from render: screen.getByRole() vs const { getByRole } = render(...). Benefits: no need to update destructuring when adding queries; easier to read in test failures since screen shows what is in the document.' },
  ];
