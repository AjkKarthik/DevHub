...
`describe('Users page', () => {
  ...
  { q: 'What makes Cypress different from Selenium for E2E testing?', options: ['Cypress is faster because it uses HTTP', 'Cypress runs inside the browser with direct DOM access, no WebDriver', 'Cypress only supports Chrome', 'Cypress requires a separate test server'], answer: 1, explanation: 'Cypress runs in the same event loop as the application — direct DOM manipulation, XHR interception, and time control without WebDriver network round-trips. This makes it faster and more reliable than Selenium.' },
  { q: 'How do you intercept network requests in Cypress?', options: ['cy.mock()', 'cy.intercept() with route matchers and optional response fixtures', 'cy.stub() on fetch', 'cy.server() and cy.route()'], answer: 1, explanation: 'cy.intercept(method, url, response) intercepts network requests. Use { fixture: \'users.json\' } to return fixture data, or a function to dynamically modify responses. Aliases: cy.intercept(...).as(\'getUsers\') then cy.wait(\'@getUsers\').' },
  { q: 'What is the cy.wait() command used for?', options: ['Pausing for a fixed time', 'Waiting for aliased network requests or DOM conditions to resolve', 'Waiting for animations', 'Polling for element visibility'], answer: 1, explanation: 'cy.wait(\'@alias\') waits for an intercepted network request to complete before continuing. Prefer cy.wait(\'@alias\') over cy.wait(N) — event-based waits are more reliable than arbitrary timeouts.' },
  ];
