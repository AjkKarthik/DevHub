...
`public class BankAccountTests
{
  ...
  { q: 'What is the difference between [Fact] and [Theory] in xUnit?', options: ['[Fact] runs once; [Theory] runs multiple times with different data via [InlineData]', '[Theory] is for async tests', '[Fact] is for integration, [Theory] for unit', 'They are identical'], answer: 0, explanation: '[Fact]: a single test case with no parameters. [Theory] + [InlineData(...)]: parameterised tests — same test body runs with each set of inputs. Reduces duplication for similar test cases with different inputs.' },
