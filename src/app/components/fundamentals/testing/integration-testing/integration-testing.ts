...
describe('POST /items', () => {
  ...
  { q: 'What is Testcontainers used for in integration testing?', options: ['UI test orchestration', 'Spinning up real Docker containers (databases, services) for integration tests that clean up after', 'Performance load testing', 'API contract verification'], answer: 1, explanation: 'Testcontainers provides on-demand real services (PostgreSQL, Redis, Kafka) as Docker containers. Each test run gets a clean instance — tests are reproducible and isolated without shared state.' },
