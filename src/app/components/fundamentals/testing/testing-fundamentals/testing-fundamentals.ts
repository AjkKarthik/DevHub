...
test('multiply with negative numbers', () => {
  ...
  qna: QnaItem[] = [
    { q: 'What is the "testing trophy" model, and how does it differ from the classic testing pyramid?', a: 'The testing trophy (popularized by Kent C. Dodds) reshapes the classic pyramid for modern frontend/full-stack apps: it keeps a small base of unit tests, but makes INTEGRATION tests the largest layer (not unit tests), with a thin layer of E2E tests on top and a very thin sliver of static analysis (type checking, linting) at the very bottom. The rationale is that for typical UI-heavy applications, integration tests (rendering a component with its real child components and a mocked network layer) give the best confidence-to-cost ratio, whereas the classic pyramid\'s heavy unit-test emphasis assumes business-logic-heavy backend systems.' },
