...
test('clamp is idempotent', () => {
  ...
  { q: 'What is shrinking in property-based testing?', options: ['Reducing test coverage', 'Automatically reducing a failing input to the smallest failing case', 'Compressing test output', 'Minimising assertion count'], answer: 1, explanation: 'When a random input causes a failure, the framework automatically shrinks it to the minimal failing example. A random 1000-element array failure shrinks to the smallest array that still fails — making debugging much easier.' },
  { q: 'Which JavaScript library is used for property-based testing?', options: ['Faker.js', 'fast-check', 'Chance.js', 'Bogus'], answer: 1, explanation: 'fast-check is the most popular TypeScript/JavaScript property-based testing library. It provides arbitraries (data generators) and integrates with Jest/Vitest: c.assert(fc.property(fc.integer(), n => n + 0 === n)).' },
  ];
