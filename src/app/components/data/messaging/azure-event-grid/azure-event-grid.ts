  ...
  readonly theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        'Retention is tier-capped, not a flat 1–90 day dial: Basic is fixed at 1 day, Standard maxes out at 7 days, and only Premium/Dedicated reach the full 90-day ceiling. On Standard/Premium/Dedicated the DEFAULT is only 1 hour — reaching the tier\'s maximum requires explicitly configuring it. Consumers read at their own pace like Kafka, within whatever window is actually set.',
...
async function startBatchProcessor() {
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What is an Event Grid domain?', options: ['A namespace for event routing rules', 'A multi-tenant endpoint managing many topics under one URL', 'An archived event store', 'A retry configuration object'], answer: 1, explanation: 'An Event Grid domain manages up to 100,000 topics under a single endpoint — far beyond the 100 custom topics allowed per subscription — ideal for multi-tenant apps where each tenant gets a dedicated topic without separate endpoints.' },
  ];

  readonly qna: QnaItem[] = [
    { q: 'Is Event Hubs really Kafka-compatible?', a: 'Yes, but only on Standard tier and above — the Kafka endpoint (port 9093) is not available on Basic tier namespaces at all (a common cause of TopicAuthorizationException). On a supported tier, existing Kafka producers/consumers can point to Event Hubs by changing only the bootstrap server and adding SASL authentication — no code changes needed.' },
    { q: 'What is the difference between Event Grid and Event Hubs retention?', a: 'Event Grid is a push system — it delivers events to endpoints immediately and does not retain events. Event Hubs retains events for a tier-capped window: 1 day fixed on Basic, up to 7 days on Standard, up to 90 days on Premium/Dedicated. Consumers can replay any event within whichever window applies.' },
