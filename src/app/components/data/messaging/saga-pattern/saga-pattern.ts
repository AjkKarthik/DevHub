...
startSaga('ORD-001');`,
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What is a compensating transaction in a saga?', options: ['A transaction that speeds up the saga', 'An action that undoes the effects of a previous successful step when a later step fails', 'A rollback in a distributed database', 'A retry mechanism for failed steps'], answer: 1, explanation: 'When a saga step fails, compensating transactions are executed in reverse order to undo previously completed steps. Example: OrderPlaced → PaymentCharged → InventoryReserved (fails) → RefundPayment compensation.' },
  ];
