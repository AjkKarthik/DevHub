...
async function processOrder(msg: { offset: string; value: Buffer | null }) {
  ...
  readonly qna: QnaItem[] = [
    { q: 'How do I process messages exactly once end-to-end?', a: 'Use idempotent producers (idempotent:true) and transactional producers for cross-topic atomicity. Consumer-side: make processing idempotent — check a database before acting. True end-to-end exactly-once requires Kafka Streams transactions or careful offset management with idempotent sinks.' },
