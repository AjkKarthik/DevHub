...
async function setup() {
  ...
  readonly qna: QnaItem[] = [
    ...
    { q: 'How do you bind a queue to multiple exchanges?', a: 'A queue can have multiple bindings — to the same or different exchanges. Each binding has its own routing key/pattern. Use <code>queue.bind(exchange, routingKey)</code> for each. This allows a queue to receive messages from different sources (e.g., both high-priority.orders and normal.orders topics on different exchanges).' },
  ];
