...
async function setup() {
  ...
  readonly qna: QnaItem[] = [
    ...
    { q: 'How do you implement a publish-subscribe pattern with RabbitMQ?', a: 'Use a <strong>fanout exchange</strong>: each subscriber creates its own queue and binds it to the exchange. Publisher sends to the exchange with any routing key (ignored by fanout). All bound queues receive a copy. For selective subscription, use a <strong>topic exchange</strong> with pattern bindings so subscribers only receive relevant event types.' },
    { q: 'What is a headers exchange and when is it useful?', a: 'Headers exchange routes based on message header attributes instead of routing keys. Binding specifies key-value pairs; the x-match argument controls matching: <strong>all</strong> (all headers must match) or <strong>any</strong> (any header match). Use for complex routing rules that cannot be expressed in routing key strings — e.g., routing by content-type and region.' },
    { q: 'Can two exchanges be bound to each other in RabbitMQ, and what is that used for?', a: 'Yes — exchange-to-exchange binding routes messages from one exchange into another as if it were a queue, letting you build multi-level routing topologies. A common use: a fanout exchange broadcasts to several downstream topic exchanges, each applying its own domain-specific routing-key filtering before delivering to actual queues, separating the concern of "who gets a copy of this event category" (fanout layer) from "which specific queue within that category" (topic layer) rather than binding every queue directly to one giant exchange.' },
  ];
