...
// Or use SNS subscription dead-letter queue in the subscription config`,
      explanation: 'For AWS-managed endpoints (SQS, Lambda) — the case shown here — SNS retries for up to 23 days (100,015 total attempts) before dropping undeliverable messages. HTTP/S endpoints get a far shorter default (3 retries, 20 seconds apart, roughly a minute total) unless a custom delivery policy is configured, and even then the total retry window caps at 3,600 seconds. Configure a DLQ on the subscription to capture dropped events regardless of endpoint type.'
    },
    {
...
// ORD-3 → premium queue + eu queue (both filters match)`,
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What is the maximum number of targets a single EventBridge rule can route a matched event to?', options: ['1 — one rule, one target', '5 targets per rule', 'Unlimited', 'It matches the number of SNS subscriptions'], answer: 1, explanation: 'A single EventBridge rule can route a matched event to up to 5 targets (Lambda functions, SQS queues, Step Functions, etc.) simultaneously — each target receives its own copy of the event. Needing to fan out to more than 5 targets requires either multiple rules matching the same pattern or routing through an intermediate SNS topic.' },
  ];
  ...
  readonly qna: QnaItem[] = [
    ...
    { q: 'Can SNS guarantee ordering?', a: 'Standard SNS topics do not guarantee ordering. FIFO SNS topics (ending in .fifo) support strict ordering and exactly-once delivery, but only work with FIFO SQS subscriptions and Lambda targets. The default throughput was raised 10x in late 2023 to 3,000 messages/second per topic (automatically, for existing and new topics alike) — not the 300 msg/s figure still commonly quoted from before that change.' },
    { q: 'How do you ensure ordered delivery with SNS?', a: 'Standard SNS topics do not guarantee ordering. For ordered delivery use <strong>SNS FIFO topics</strong> paired with SQS FIFO queues — order is maintained per MessageGroupId. Default FIFO throughput is 3,000 msg/s per topic, and enabling High Throughput mode (FifoThroughputScope=MessageGroup) raises that further, at the cost of scoping deduplication to the message group instead of the whole topic. FIFO topics support only SQS FIFO and Lambda subscribers.' },
    { q: 'Can you migrate an existing SNS-based fan-out architecture to EventBridge without downtime, and what is the general approach?', a: 'Yes — a common zero-downtime approach is the "strangler" pattern: create the EventBridge bus and rules alongside the existing SNS topic, have the SNS topic\'s existing publisher ALSO publish to EventBridge (dual-write) for a transition period, migrate consumers one at a time to subscribe via EventBridge rules instead of SNS subscriptions, verify each migrated consumer behaves identically, and only decommission the SNS topic once every consumer has moved off it. Attempting a hard cutover in one step risks dropped events for any consumer not yet ready on the new path.' },
