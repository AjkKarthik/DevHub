...
async function scheduleAndCancel(orderId: string) {
  ...
  readonly qna: QnaItem[] = [
    ...
    { q: 'What is the difference between Service Bus queues and topics?', a: 'Queues: point-to-point — one message consumed by one receiver. Topics: publish-subscribe — a message is copied to N subscriptions, each getting its own copy. Subscriptions support SQL filter expressions (e.g. Label = \'orders\') to receive only matching messages. Use queues for work distribution, topics for fan-out.' },
