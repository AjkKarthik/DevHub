...
async function startCounter() {
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What is a KStream vs KTable in Kafka Streams?', options: ['KStream is bounded, KTable is unbounded', 'KStream represents an unbounded event log; KTable represents the latest state per key (changelog)', 'They are interchangeable', 'KTable is for windowed aggregations only'], answer: 1, explanation: 'KStream: each record is an independent event (append-only log). KTable: each record updates state for a key — only the latest value per key is retained (changelog semantics). KTable materialises as a state store.' },
