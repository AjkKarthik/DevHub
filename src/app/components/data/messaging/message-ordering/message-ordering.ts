...
// Expected output: seq=0: created, seq=1: paid, seq=2: shipped`,
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'How does SQS FIFO guarantee ordering?', options: ['By timestamp sorting on consume', 'MessageGroupId routes messages to FIFO groups; within a group delivery is strictly ordered', 'By limiting to one producer', 'Through broker-side locks'], answer: 1, explanation: 'SQS FIFO uses MessageGroupId to create ordered groups. Within a group, messages are delivered in strict send order. Multiple groups can be processed in parallel, but each group is sequential.' },
  ];
