...
run();`,
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'How does the RabbitMQ retry pattern with backoff work?', options: ['RabbitMQ natively retries with configurable backoff', 'Messages are nacked to a delay queue with TTL, then re-published to the work queue after delay', 'Consumer waits before calling ACK', 'A retry plugin is required'], answer: 1, explanation: 'RabbitMQ lacks native delayed retry. Pattern: nack message → dead-letter to a delay queue (per-message TTL = delay duration) → DLX routes expired message back to original queue. Create multiple delay queues for exponential backoff.' },
    { q: 'What is the RabbitMQ priority queue pattern?', options: ['Exchange-level priority routing', 'Queues with x-max-priority argument; higher-priority messages delivered first when consumers are available', 'Consumer-side priority sorting', 'Separate queues per priority with manual consumer assignment'], answer: 1, explanation: 'Declare a queue with x-max-priority (1-255). Higher-priority messages are delivered before lower-priority ones when a consumer is available. Caution: high-priority messages can starve low-priority ones.' },
  ];
