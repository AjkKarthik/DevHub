...
monitorConnectors('http://connect:8083');`,
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between a source connector and sink connector in Kafka Connect?', options: ['Source reads from Kafka, sink writes to Kafka', 'Source reads from external systems and writes to Kafka; sink reads from Kafka and writes to external systems', 'They are identical but installed differently', 'Source is for files, sink is for databases'], answer: 1, explanation: 'Source connectors pull data from external systems (databases, files, APIs) into Kafka topics. Sink connectors push data from Kafka topics to external systems (Elasticsearch, JDBC, S3).' },
  ...
  readonly qna: QnaItem[] = [
    ...
    { q: 'How does Kafka Connect handle schema evolution?', a: 'Connect integrates with <strong>Schema Registry</strong> using Avro/Protobuf/JSON Schema converters. Schemas are registered and versioned. Producers serialize with a schema ID; consumers deserialize using the registry. Schema compatibility modes (BACKWARD, FORWARD, FULL) enforce evolution rules. SMTs (Single Message Transforms) can modify records inline before write.' },
    ...
    { q: 'How does Kafka Connect manage offset tracking for source connectors?', a: 'Source connectors report source offsets (e.g., database LSN, file position, API cursor) to a Kafka internal topic (<code>connect-offsets</code>). On restart, the connector reads its last committed offset and resumes from there. This enables exactly-once source semantics when paired with idempotent producers. Sink connectors use standard consumer group offsets.' },
  ];
