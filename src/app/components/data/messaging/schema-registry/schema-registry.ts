...
const encoded = await registry.encode(id, payload);`,
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What is schema backward compatibility?', options: ['New schemas can read data written with old schemas', 'Old schemas can read data written with new schemas', 'Schema is unchanged', 'Any schema change is allowed'], answer: 0, explanation: 'BACKWARD compatibility: new schema can read old data. Add optional fields with defaults. Delete required fields. Producers can upgrade gradually. In FORWARD: old schema reads new data — producers can add fields that old consumers ignore.' },
    { q: 'How does Confluent Schema Registry wire format work?', options: ['Schemas are embedded in every message', 'A 5-byte header: magic byte 0x00 + 4-byte schema ID; payload is Avro/Protobuf/JSON Schema encoded', 'Schema is stored in Kafka headers', 'Schemas are stored per-partition'], answer: 1, explanation: 'Messages start with magic byte 0 and a 4-byte schema ID. Consumers fetch the schema from the registry by ID and deserialize. The registry caches schemas — schema lookup adds minimal overhead.' },
  ];
  ...
  readonly qna: QnaItem[] = [
    ...
    { q: 'How do you evolve an Avro schema in a Kafka pipeline?', a: 'To evolve: (1) Register a new schema version with the registry; (2) Update producers to use the new schema; (3) Consumers can use old or new schema — the registry handles compatibility. With BACKWARD compatibility, new schema can read old messages. Always add fields with defaults; never rename or change field types. Test with <code>mvn schema-registry:test-compatibility</code>.' },
    { q: 'What is the difference between Avro, Protobuf, and JSON Schema in Schema Registry?', a: '<strong>Avro</strong>: binary, schema-based, compact, most common for Kafka. <strong>Protobuf</strong>: binary, field numbers for evolution, supports gRPC. <strong>JSON Schema</strong>: human-readable, larger, good for debugging. Avro is fastest/smallest. Use JSON Schema for legacy systems or when human-readability matters. All three are supported by Confluent Schema Registry.' },
    { q: 'How does schema registry subject naming strategy affect schema sharing?', a: '<strong>TopicNameStrategy</strong> (default): one schema per topic (key/value). <strong>RecordNameStrategy</strong>: schema shared across topics by record type — reuse schema for the same event type on different topics. <strong>TopicRecordNameStrategy</strong>: combination. Change with <code>value.subject.name.strategy</code> in producer config. RecordNameStrategy enables polymorphic topics.' },
  ];
