...
WHERE published_at IS NOT NULL
  ...
  readonly quiz: QuizQuestion[] = [
    ...
    { q: 'What problem does the Transactional Outbox pattern solve?', options: ['Message duplication in Kafka', 'Dual-write inconsistency between database commit and message broker publish', 'Consumer lag in high-throughput systems', 'Schema evolution in Avro'], answer: 1, explanation: 'Dual-write problem: if you write to DB then publish to broker, a crash between them leaves the DB updated but no message sent (or vice versa). The Outbox pattern writes message intent to DB atomically with the business data, then relays it to the broker.' },
