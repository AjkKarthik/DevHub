...
useMutation(DELETE_POST, {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do I pass file uploads in GraphQL mutations?', a: 'The multipart request spec (graphql-multipart-request-spec) defines the wire format and the `Upload` scalar. Apollo Server removed BUILT-IN support for it in v3 (2021) — the multipart/form-data content type is a CSRF loophole, so Apollo now recommends signed URLs (upload directly to S3/cloud storage, bypassing the GraphQL server) instead. `graphql-upload` can still be wired in manually for legacy apps, but only with CSRF prevention explicitly enabled.' },
