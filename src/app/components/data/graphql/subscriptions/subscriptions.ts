...
const resolvers = {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Can subscriptions be used in GraphQL federation?', a: 'It\'s complex, and it is a paid feature: federated subscriptions require a self-hosted Apollo Router on a GraphOS ENTERPRISE plan (not just any Federation v2+ supergraph), with your subgraph composition specifically built against Federation 2.4+ (earlier v2.x releases do not support subscription operations at all). Each subgraph can publish subscription events that the router aggregates.' },
