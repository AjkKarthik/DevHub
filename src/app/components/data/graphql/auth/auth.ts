  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        'Rules are NOT memoized by default — `rule()`\'s real default cache mode is `no_cache`, so the same rule re-runs its check function every time it is called. Caching is opt-in via `rule(name, { cache: \'contextual\' })` (once per request, ignores args) or `{ cache: \'strict\' }` (once per unique parent+args combination).',
        ...
        'graphql-shield and its companion graphql-middleware have had no release in 3+ years — both are effectively unmaintained. @envelop/graphql-middleware (useGraphQLMiddleware) is the actively maintained modern replacement, wrapping the exact same shield()/rule() API.'
      ]
    },
    {
...
function authDirectiveTransformer(schema: any) {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the difference between @auth directive and graphql-shield?', a: '@auth is a custom directive you implement via schema transformer — declarative, visible in SDL. graphql-shield is a middleware library with composable rules (and/or/not), caching, and a permission map. Use @auth for simple role checks; graphql-shield for complex permission logic — but note graphql-shield and graphql-middleware have had no release in 3+ years and are effectively unmaintained. For new projects, prefer @envelop/graphql-middleware (useGraphQLMiddleware), an actively maintained wrapper around the identical shield()/rule() API.' },
    ...
    { q: 'Can a subscription re-check authorization mid-stream if a user\'s permissions change while they are still subscribed?', a: 'By default, no — the auth check typically happens once at subscription setup time (in the onConnect/context function), and the resulting context persists for the lifetime of that WebSocket connection, so a user whose role is revoked after subscribing keeps receiving events until they reconnect. For subscriptions where permission changes must take effect immediately, the resolver\'s async generator needs to re-validate authorization on each emitted event (not just at connection time) or the server needs to proactively terminate/re-authenticate active subscriptions when a user\'s permissions change — neither happens automatically, so this must be deliberately designed for security-sensitive subscription data.' }
  ];
