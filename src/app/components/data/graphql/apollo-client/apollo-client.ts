...
function PostSearch() {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do I abort an in-flight query?', a: 'Pass an AbortController\'s signal through context.fetchOptions -- Apollo\'s HttpLink forwards it to the underlying fetch() call: client.query({ query, context: { fetchOptions: { signal: abortController.signal } } }), then call abortController.abort(). fetchPolicy has nothing to do with aborting, and ObservableQuery (from watchQuery()) has no cancel() method -- to stop a watchQuery, unsubscribe from its returned subscription instead. Note: Apollo\'s own request deduplication can make a re-run of the identical query+variables after an abort still resolve to the aborted in-flight request.' },
