...
// → Hot spot: all inserts go to newest chunk ✗`,
  ...
  qna: QnaItem[] = [
    ...
    { q: 'If replication lag on a secondary suddenly spikes from under 1 second to several minutes, what are the most likely root causes to check first, in order?', a: 'Check in this order: (1) Secondary resource contention — is the secondary\'s disk I/O or CPU saturated by competing queries (analytics reads routed there, backups running) that are starving oplog application of resources? (2) Write volume spike on the primary — did the incoming write rate suddenly exceed what the secondary can apply, even with healthy resources? (3) Network issues between primary and secondary — packet loss or latency in cross-AZ/cross-region replication links. (4) A long-running operation on the secondary itself blocking oplog application (rare on MongoDB 4.4+ due to parallel oplog application, but still possible with certain index builds). Diagnosing in this order — starting with the secondary\'s own resource usage — resolves the majority of real-world lag spikes fastest, since secondary overload is the most common cause.' },
    ...
    { q: 'How does zone sharding work for geographic data distribution in MongoDB?', a: 'Zone sharding: assigns specific shard key value ranges to specific shards (zones). Used for geographic data isolation — route EU user data to EU shards, US data to US shards. Setup: sh.addShardToZone("shard0001", "EU"): assigns shard0001 to zone EU (sh.addShardTag() is the older, legacy alias for the same command). sh.updateZoneKeyRange("db.users", { region: "EU" }, { region: "EU~" }, "EU"): assigns the EU region range to the EU zone (sh.addTagRange() is the legacy alias) — the trailing "~" (tilde) creates an upper bound that sorts lexicographically after every ordinary EU-prefixed string, since ~ sorts higher than any letter or digit; simply concatenating an empty string onto "EU" produces "EU" again, not a real upper bound at all. Repeat for US zone on different shards. Chunk placement: the balancer migrates chunks to satisfy zone assignments. Once configured, new EU documents automatically land on EU-zoned shards. Use cases: GDPR compliance (EU data must stay in EU). Data sovereignty requirements. Reduced latency (route requests to geographically close shards). Combining with replica sets: each shard is a replica set. EU shards can be replica sets hosted in EU data centers. Zone sharding does not prevent cross-shard queries if the shard key is omitted from the query — scatter-gather still queries all shards.' },
