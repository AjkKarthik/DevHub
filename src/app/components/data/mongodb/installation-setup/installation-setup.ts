  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        'Connect from another container: use the service name as hostname — <code>mongodb://admin:secret@mongo:27017/myapp?authSource=admin</code> where <code>mongo</code> is the Docker Compose service name, not localhost. The <code>authSource=admin</code> is required here: it is not optional decoration — since the admin user was created in the <code>admin</code> database (per <code>MONGO_INITDB_ROOT_*</code> above), omitting it makes the driver default <code>authSource</code> to <code>myapp</code> instead, where that user does not exist, and the connection fails to authenticate.',
...
start();`,
  ...
  qna: QnaItem[] = [
    ...
    },
    {
      ...
      a: 'MongoDB starts normally and accepts connections — and rather than needing the admin user created BEFORE enabling authorization, MongoDB has a dedicated "localhost exception" for exactly this case: with security.authorization: enabled and zero users in the system, a connection from localhost only is still allowed to create the FIRST user (typically with the userAdmin or userAdminAnyDatabase role in the admin database) — no restart required at all. The exception closes itself automatically the instant that first user is created; every connection after that, including from localhost, must authenticate normally. Restarting mongod without --auth to create the user first is a valid fallback if the localhost exception isn\'t working for some reason (an unusual bindIp, a proxy in front of mongod), but it is not the standard, documented path.',
    },
    {
    ...
    { q: 'How does MongoDB handle connection pooling and what settings should you tune?', a: 'Connection pooling: the MongoDB driver maintains a pool of persistent TCP connections to the server. Reusing connections avoids the overhead of TCP handshake and TLS negotiation on every operation. Default pool size: maxPoolSize defaults to 100 in most drivers. How it works: when the application requests a connection, the driver checks out an idle connection from the pool. If none are available and the pool is not full, a new connection is created. If the pool is full, the request waits (up to waitQueueTimeoutMS). Key settings: maxPoolSize: maximum connections in the pool. Set based on expected concurrent operations. minPoolSize: minimum idle connections kept warm. maxIdleTimeMS: how long idle connections are kept before being closed. connectTimeoutMS: how long to wait when establishing a new connection. socketTimeoutMS: how long to wait for a response on an existing connection. serverSelectionTimeoutMS: how long the driver waits to find an available server. Sizing the pool: a pool size larger than MongoDB can handle causes connection backpressure on the server. Total connections across all app instances should stay well below the MongoDB maxIncomingConnections limit (default 65536, itself automatically capped lower based on the OS\'s available file descriptors).' },
