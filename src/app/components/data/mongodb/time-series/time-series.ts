...
async function getMovingAvg(hostname: string, windowMinutes = 5) {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the metadata field in MongoDB time series collections and how is it used?', a: 'The metaField: a field in each document that identifies the source or series of the measurement. Examples: a temperature sensor network: metaField: "metadata", and each document has metadata: { deviceId: "sensor-01", location: "warehouse-A" }. A stock price feed: metaField: "ticker", and each document has ticker: "AAPL". How metaField affects bucketing: MongoDB creates separate buckets for documents with different metaField values. All readings from sensor-01 in the same time window go into the same bucket. Readings from sensor-02 go into a different bucket. Optimization: the metaField acts as the partition key for internal organization. Queries filtering on metaField values benefit from bucket-level filtering. Best practices: put ALL identifying information in the metaField. Cardinality: if the metaField has very high cardinality (unique per document), each document gets its own bucket — no compression benefit. Aim for a metaField that groups 10-1000 related measurements together per time window. The metaField is immutable after insert. You cannot update the metaField value of an existing document. If a device is renamed, insert new documents with the new name — you cannot rename old documents. metaField is not required: if omitted, all documents in the same time window share a bucket (useful for single-series data).' },
    { q: 'How do you downsample time series data using MongoDB aggregation?', a: 'Downsampling: reduce high-resolution data to lower resolution for long-term retention or efficient dashboard queries. Pattern: read raw data, aggregate to coarser time buckets, write results to a summary collection. Example: downsample per-second sensor readings to per-hour averages: db.sensorData.aggregate([ { $match: { timestamp: { $gte: startOfDay, $lt: endOfDay } } }, { $group: { _id: { hour: { $dateTrunc: { date: "", unit: "hour" } }, deviceId: ".deviceId" }, avgTemp: { $avg: "" }, maxTemp: { $max: "" }, minTemp: { $min: "" }, readingCount: { $sum: 1 } } }, { $out: "sensorDataHourly" } ]). Schedule this aggregation nightly via a cron job or Atlas Scheduled Trigger. Time series for the summary: make sensorDataHourly a time series collection too (hourly granularity). Apply TTL to retain only N days of per-second data while keeping hourly summaries indefinitely. $merge vs $out: $out replaces the output collection entirely. $merge allows upserting into the output collection without overwriting existing hours. Multi-tier retention: raw data (30 days), hourly summaries (1 year), daily summaries (indefinitely).' },
