...
async function recordScore(playerId: string, score: number) {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'Can $pull remove elements from a NESTED array inside an array of subdocuments (e.g. items.$[].variants), and what limitation applies?', a: '$pull operates on the array field it is given, and can target a nested array path (e.g. { $pull: { "items.0.variants": "red" } }) if you know the exact parent index, but $pull cannot use the all-positional $[] together with a condition to pull from EVERY subdocument\'s nested array in one atomic call the way $[] combined with $set can for simple field updates — removing a value from a nested array across all parent subdocuments generally requires either targeting each parent index explicitly, using arrayFilters with a compatible update operator, or restructuring the update as a $set that reassigns the filtered array (e.g. via aggregation-pipeline updates, which DO support $filter directly on nested arrays in a single pass).' },
