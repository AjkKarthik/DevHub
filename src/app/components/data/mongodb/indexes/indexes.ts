  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        '<strong>Hybrid index builds</strong> (MongoDB 4.2+): the old foreground/background distinction was eliminated — every index build now uses a hybrid protocol that takes an exclusive lock only at the very start (to initialize) and the very end (to commit), with reads and writes freely interleaved during the main scan phase in between. Your application stays available for the vast majority of the build\'s duration.',
