...
console.log('Q4 stage:', q4.queryPlanner.winningPlan.inputStage?.stage); // IXSCAN`,
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the ESR rule for compound index field ordering?', a: 'ESR rule: Equality, Sort, Range — the recommended field order for compound indexes. Equality fields first: fields used in equality matches (field: value) reduce the search space immediately. Sort fields next: placing sort fields after equality fields allows MongoDB to return sorted results directly from the index without an in-memory sort. Range fields last: fields with range conditions ($gt, $lt, $gte, $lte, $in with multiple values, regex) should come last because they create a range scan rather than a point lookup. Example: query { status: "active", createdAt: { $gte: lastWeek } } with sort by priority descending. Index: { status: 1, priority: -1, createdAt: 1 }. status (equality) first, priority (sort) second, createdAt (range) third. Why it matters: an index sorted by range fields before sort fields cannot use the index for sorting (requires in-memory sort). The planner may reject the index for the sort. Rule of thumb: think of index entries as a sorted array of tuples — equality narrows it to a slice, sort makes that slice ordered, range scans within the ordered slice.' },
