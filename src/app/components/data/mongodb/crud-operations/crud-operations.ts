...
async function getCart(userId: string) {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is bulk write and when is it more efficient than individual operations?', a: 'BulkWrite batches multiple write operations (insertOne, updateOne, updateMany, deleteOne, deleteMany, replaceOne) into a single database round trip. Efficiency gains: network latency: a single round trip for 1000 operations vs 1000 round trips. Server efficiency: MongoDB can process the batch in a tighter loop. MongoDB driver batching: the driver automatically batches bulk writes into 100K document or 48MB batches. ordered vs unordered: ordered (default): operations execute in order; stop on first error. unordered: all operations attempted; failures collected and reported. Unordered is faster for large batches (parallel execution on the server). When to use: data migrations — transform and write thousands of documents. ETL processes — processing a file of records. Batch status updates — mark a set of orders as processed. Syncing changes from another system. Return value: includes insertedCount, matchedCount, modifiedCount, deletedCount, upsertedCount, and upsertedIds for the entire batch. For extremely large batches, the driver automatically splits into multiple network requests while presenting a unified API.' },
