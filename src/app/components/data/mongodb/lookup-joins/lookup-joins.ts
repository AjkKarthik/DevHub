  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        'The pipeline form of $lookup — using <code>let</code> to bind outer-document fields, referenced with <code>$$</code> inside the sub-pipeline — is what MongoDB\'s own documentation calls a <strong>correlated</strong> subquery, since each input document can produce a different sub-pipeline result. This is the opposite of "uncorrelated": a pipeline $lookup that never references a let binding returns the identical result for every input document, and MongoDB\'s query planner can legitimately run it just once and reuse the result. The correlated form allows more complex join conditions and additional filtering within the joined pipeline, at the cost of being somewhat harder to optimize and reason about than a simple equality join.',
      ],
    },
  ];
