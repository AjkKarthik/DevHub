...
const report = await orders.aggregate([
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you join on an array field using $lookup?', a: 'Joining from an array field (one-to-many): if the input document has an array of IDs (e.g., tagIds: [id1, id2, id3]) and you want to look up each referenced document, $lookup handles this automatically. localField: "tagIds" will match any element of the tagIds array against foreignField. Each matching document from the from collection is included in the result array. Example: { $lookup: { from: "tags", localField: "tagIds", foreignField: "_id", as: "resolvedTags" } }. If tagIds = [1, 2, 3], resolvedTags will contain the tag documents for IDs 1, 2, and 3. Joining to an array field (many-to-one): if you want to find all input documents where a from collection document matches any element of a field array, use the pipeline form with $expr and $in: { $lookup: { from: "orders", let: { catId: "" }, pipeline: [{ $match: { $expr: { $in: ["$$catId", "$categoryIds"] } } }], as: "ordersInCategory" } }.' },
