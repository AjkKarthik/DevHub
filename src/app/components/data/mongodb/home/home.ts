...
const ALL_TOPICS: Topic[] = [
  ...
  { title: 'Time Series Collections',    route: '/mongodb', badge: 'Schema Design', available: false,
    description: 'MongoDB 5.0+ native time series support — efficient IoT/metrics storage with automatic bucketing and compression.',
    keyPoints: ['db.createCollection("metrics", {timeseries:{timeField:"ts",metaField:"sensor"}})', 'Automatic bucketing: MongoDB internally groups documents for compression', 'Granularity: seconds/minutes/hours — match to your ingestion rate', 'TTL on time series: expireAfterSeconds to auto-expire old data', 'Query patterns: range on timeField; filter on metaField; aggregate for rollups'] },
