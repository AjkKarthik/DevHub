...
const invoices = await orders.aggregate([
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How would you use $mergeObjects to apply a set of "default" field values to documents that might be missing some fields?', a: '{ $replaceRoot: { newRoot: { $mergeObjects: [{ status: "pending", priority: "normal" }, "$$ROOT"] } } } — putting the defaults document FIRST and the actual document ($$ROOT) second means the real document\'s fields win when present (since later fields overwrite earlier ones), but any field missing from the real document falls back to the default value supplied first. This is a common pattern for backfilling documents written before a new field existed, without needing a separate migration script to touch every existing document — the merge happens on read, computed on the fly.' },
  ];
