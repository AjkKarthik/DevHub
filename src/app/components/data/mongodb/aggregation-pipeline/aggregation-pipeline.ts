...
const report = await orders.aggregate([
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'Why can $facet sometimes be SLOWER than running the same sub-pipelines as separate queries, despite fetching everything in one round-trip?', options: ['$facet is never slower — it is always strictly faster', 'Each sub-pipeline inside $facet independently processes the FULL set of input documents that reach the $facet stage — if one sub-pipeline needs different filtering than another, MongoDB cannot apply a single index-optimized $match before the fan-out that suits every sub-pipeline equally, so some sub-pipelines end up scanning more documents than a tailored separate query would', 'It uses more network bandwidth than separate queries', '$facet does not support indexes at all'], answer: 1, explanation: '$facet avoids N round-trips by fanning out ONE input document set to multiple sub-pipelines within a single database call — but that also means every sub-pipeline pays the cost of processing whatever documents survive the pre-$facet $match, even if a given sub-pipeline only actually needs a small subset of them. Two entirely separate, independently-optimized queries can each use the most specific index and filter for their own exact need, sometimes outperforming a single $facet that has to serve every sub-pipeline\'s requirements off the same upstream document set.' },
