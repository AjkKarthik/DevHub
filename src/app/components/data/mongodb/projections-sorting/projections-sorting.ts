...
async function getUserPage(
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is a projection in MongoDB and what are the two projection modes?', options: ['A projection is a sort specification that controls the order of fields within a returned document', 'A projection specifies which fields to include or exclude from query results; inclusion mode (1 values) returns only listed fields; exclusion mode (0 values) returns all fields except listed ones — these two modes cannot be mixed except for the _id field', 'A projection can combine inclusion and exclusion in any order to give fine-grained control over which document fields are returned', 'A projection applies only to embedded documents and arrays; top-level fields are always returned in full'], answer: 1, explanation: 'Inclusion projection: { name: 1, price: 1 } — returns only name, price, and _id (always included unless explicitly excluded). Exclusion projection: { description: 0, internalNotes: 0 } — returns all fields EXCEPT description and internalNotes. Cannot mix (with one exception): { name: 1, description: 0 } is INVALID — you cannot include some fields while explicitly excluding others in the same projection. Exception: { _id: 0, name: 1 } IS valid — _id can be explicitly excluded even in an inclusion projection. Why this rule: MongoDB needs to know whether to start from an empty document and add fields (inclusion) or start from the full document and remove fields (exclusion). Mixing would be ambiguous. Expressions in projections: $slice, $elemMatch, and aggregation expressions (like $expr) can be used in projections but have special rules — they can appear in inclusion projections.' },
    { q: 'How does compound sorting work in MongoDB and what is the sort key order significance?', options: ['Compound sort in MongoDB always sorts all fields in the same direction (all ascending or all descending)', 'In a compound sort, documents are sorted by the first sort key first; within documents that tie on the first key, they are sorted by the second key, and so on — the order of keys in the sort object matters', 'MongoDB sorts compound keys simultaneously, not sequentially, so the key order does not affect the result', 'Compound sorting is only available with indexes — without a matching compound index, MongoDB falls back to sorting by _id only'], answer: 1, explanation: 'Compound sort: { status: 1, createdAt: -1 }. Primary key: status ascending. Within the same status: createdAt descending (newest first). The sort object key order defines the priority. Direction mixing: each field in a compound sort can have an independent direction. { status: 1, price: -1 } — ascending status, descending price. Index alignment: for a compound sort to use an index without a blocking SORT stage, the index field order and directions must match (or be perfectly inversed). Index { status: 1, price: -1 } covers { status: 1, price: -1 } (same) and { status: -1, price: 1 } (reversed). Does not cover { status: 1, price: 1 }. Explain: SORT stage in the explain output means MongoDB is doing an in-memory sort (no index covered the sort). For production, ensure sort fields are covered by the index.' },
    { q: 'What is the natural sort order in MongoDB and when does it apply?', options: ['Natural order is alphabetical order by field name within a document', 'Natural order is the order in which documents are stored on disk — for collections without a sort, find() returns documents in natural order, which is approximately insertion order for non-fragmented collections', 'Natural order is always reverse insertion order because MongoDB appends new documents to the front of the collection', 'Natural order sorts by _id ObjectId timestamp, making it equivalent to creation time order'], answer: 1, explanation: 'Natural order: the default order when no sort is specified in find(). Approximately insertion order for uncapped, non-fragmented collections. NOT guaranteed to be stable or insertion order after updates and deletes (documents can be moved on disk). Cannot be relied upon: never assume natural order equals insertion order in production. Relying on natural order for deterministic results is a bug waiting to manifest. sort({ $natural: 1 }) — explicitly use natural order (forward). sort({ $natural: -1 }) — reverse natural order (useful for capped collections: find().sort({ $natural: -1 }).limit(10) returns the most recently inserted documents without a createdAt index. Capped collections: natural order IS guaranteed to be insertion order for capped collections (append-only design). sort({ $natural: -1 }) on a capped collection reliably returns newest documents. Sorting by _id: since ObjectId encodes creation time, sorting by { _id: 1 } approximates chronological order for most workloads.' },
    { q: 'How do aggregation projections differ from find() projections?', options: ['Aggregation $project and find() projections are identical in syntax and semantics', 'Aggregation $project supports full aggregation expressions for computing new fields, renaming fields, and complex transformations; find() projections only support include/exclude and a few special operators', 'find() projections support all aggregation expressions; $project is only needed for performance optimization within long pipelines', 'Aggregation $project automatically includes all fields not mentioned; find() projections must explicitly include all desired fields'], answer: 1, explanation: 'find() projection: limited to: include (1) or exclude (0) existing fields. Special operators: $slice, $elemMatch, $meta. Cannot rename fields or compute new values. Aggregation $project: full aggregation expression support. Rename fields: { newName: "" }. Compute values: { totalWithTax: { $multiply: ["", 1.1] } }. Conditional: { label: { $cond: [...] } }. String operations: { upperName: { $toUpper: "" } }. Type conversion: { priceInt: { $toInt: "" } }. Nested access: { "address.city": 1 } in find() vs { city: ".city" } in $project for field renaming. Default behavior: unlike find(), in $project you must explicitly include all desired fields (except _id). Any field not listed is excluded. Use $addFields instead if you want to add computed fields while preserving all existing fields.' },
  ];
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you project nested and array fields in MongoDB?', a: 'Nested field projection: use dot notation to project specific nested fields. { "address.city": 1, "address.zipCode": 1 } — returns only city and zipCode from the address object. The rest of address is excluded. This applies at any depth of nesting. Array field projection: { tags: 1 } — returns the complete tags array. $slice: { comments: { $slice: 5 } } — returns only the first 5 comments. $slice: [10, 5] — returns 5 comments starting at index 10. $elemMatch (in projection): { comments: { $elemMatch: { author: "Alice" } } } — returns only the FIRST comment by Alice (not all comments). Projection on array of objects: { "items.name": 1, "items.price": 1 } — returns the items array but each element contains only name and price. Nested exclusion: { "profile.privateNotes": 0 } — excludes only the privateNotes field from the profile object, returning everything else. The exclusion applies inside the nested object. Important: you cannot mix inclusion and exclusion on sub-fields of the same embedded object.' },
    { q: 'How do you sort on a computed field or a field within an array?', a: 'Sort on computed field: you cannot sort directly on a computed value in find(). Use aggregation with $addFields to compute the field first, then $sort. Example: sort by the length of the description field. { $addFields: { descLen: { $strLenCP: "" } } }, { $sort: { descLen: -1 } }. Sort on array field: sorting by an array field sorts by the minimum array element (for ascending) or the maximum (for descending) — MongoDB extracts the optimal element from the array for sorting purposes. Example: { prices: [10, 5, 20] }.sort({ prices: 1 }) — MongoDB uses 5 (minimum) for ascending sort. To sort by a specific element: use aggregation $addFields to extract { $arrayElemAt: ["", 0] } then sort on the new field. Sort by array length: add a count field and sort by it. { $addFields: { tagCount: { $size: "" } } }, { $sort: { tagCount: -1 } }. Index support: indexes on array fields (multikey indexes) can support range queries and equality filters but sorting by multikey indexes has restrictions — sort stages often require in-memory sort when multikey indexes are involved.' },
    { q: 'What is the $meta projection operator and what does it expose?', a: '$meta exposes metadata about a document that is generated by query processing, not stored in the document itself. Currently supported metadata values: textScore: the relevance score from a $text full-text search query. { score: { $meta: "textScore" } } adds a score field to the output containing the text search relevance score. Used in the projection AND sort: { score: { $meta: "textScore" } } in both projection and sort. searchScore: the relevance score from an Atlas Search $search query. searchSequenceToken: Atlas Search pagination token. indexKey: debugging metadata about which index key was used (for diagnostics). searchHighlights: highlights from Atlas Search. How to use with $text: db.articles.find({ $text: { $search: "mongodb performance" } }, { score: { $meta: "textScore" } }).sort({ score: { $meta: "textScore" } }). This finds articles matching the text query, projects the relevance score, and sorts by relevance (most relevant first). The $meta projection is special — it is one of the cases where you can use it alongside inclusion or exclusion projections.' },
    { q: 'What happens to sort performance when there is no supporting index?', a: 'In-memory sort: when no index can satisfy the sort, MongoDB performs an in-memory sort. The SORT stage appears in the explain output. Memory limit: the in-memory sort limit is 100MB per query (without allowDiskUse). If sort exceeds 100MB, the query fails with ExceededMemoryLimit. For large result sets, set allowDiskUse: true. Performance impact: without index support, MongoDB reads ALL matching documents into memory, sorts them, then applies skip and limit. Even for small result sets, this is less efficient than index-based sort. Signs of missing sort index: SORT stage in explain queryPlanner.winningPlan. High keysExamined or docsExamined relative to returned documents. Slow query log entries for queries with ORDER BY equivalent. Fixing with indexes: create a compound index matching the query filter AND the sort. ESR rule applies: equality fields, then sort fields, then range fields. Verification: after creating the index, check that explain shows IXSCAN feeding directly into the result without a SORT stage. Ideal: the index provides both filtering and sorting in a single scan, with skip and limit applied to the cursor rather than the sorted result set.' },
  ];
