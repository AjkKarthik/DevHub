...
async function searchProducts(col: Collection<Product>, opts: SearchOptions) {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What server-side setting can disable $where entirely, and why might a hosted MongoDB provider enable it by default?', options: ['There is no way to disable $where — it is always available', 'The security.javascriptEnabled configuration option (or an equivalent Atlas project setting) disables server-side JavaScript execution entirely, blocking $where and mapReduce; hosted providers often disable it by default since JS execution on shared infrastructure is a bigger security and resource-isolation risk than on a dedicated self-managed server', '$where can only be disabled by upgrading to MongoDB Enterprise', 'Disabling $where also disables all aggregation pipeline operators'], answer: 1, explanation: 'security.javascriptEnabled: false in mongod configuration (or an equivalent toggle in Atlas) globally disables server-side JavaScript execution, which blocks not just $where but also the legacy mapReduce command\'s custom JS functions. Multi-tenant hosted environments (like shared Atlas clusters) are more exposed to the injection and resource-isolation risks of allowing arbitrary JS execution on the server, which is why some managed offerings disable it by default even though a self-hosted mongod leaves it enabled unless an admin explicitly turns it off.' },
