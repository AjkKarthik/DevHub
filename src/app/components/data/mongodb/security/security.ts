...
async function secureLogin(username: string, password: string) {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is MongoDB Client-Side Field Level Encryption (CSFLE) and how does it protect data?', options: ['CSFLE encrypts the entire MongoDB connection using a client-managed TLS certificate instead of the server certificate', 'CSFLE encrypts specific document fields on the client before they are sent to MongoDB, so the database server never sees the plaintext values — only the application with the encryption key can decrypt', 'CSFLE is a server-side encryption feature where the mongod process encrypts specified fields before writing to disk using keys stored in a separate key management service', 'CSFLE compresses specified fields before encryption to reduce storage overhead while maintaining AES-256 encryption for all fields'], answer: 1, explanation: 'CSFLE: the MongoDB driver encrypts specified fields before the data leaves the application. MongoDB stores only ciphertext. Even a compromised MongoDB server cannot read the encrypted fields. Key management: a Customer Master Key (CMK) encrypts data encryption keys (DEKs). CMK is stored in a KMS (AWS KMS, Azure Key Vault, GCP KMS, or a local key — dev only). DEKs are stored in a MongoDB collection (the key vault). Schema map: the application defines which fields to encrypt and with which encryption algorithm. Encryption algorithms: Deterministic (AEAD AES-256-CBC): same plaintext always produces same ciphertext. Enables equality queries on encrypted fields. Less secure (patterns visible). Randomized (AEAD AES-256-CBC): each encryption produces different ciphertext. More secure. Cannot query for equality (MongoDB cannot compare ciphertext). Queryable Encryption (MongoDB 7.0+): allows equality and range queries on randomly encrypted fields using structured encryption. Atlas Search does not work on encrypted fields (ciphertext is stored, not plaintext). Use case: PII fields (SSN, credit card numbers), health data requiring HIPAA compliance.' },
