...
async function addReview(productId: ObjectId, userId: ObjectId, rating: number, text: string) {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the Extended Reference Pattern in MongoDB schema design?', options: ['The Extended Reference Pattern stores the full referenced document inline in every parent document, creating redundant copies for faster reads', 'The Extended Reference Pattern embeds a small, frequently-accessed subset of a referenced document (the key fields) alongside the reference ID, reducing the need for join lookups while keeping the full document in its own collection', 'The Extended Reference Pattern is a MongoDB Atlas feature that automatically pre-fetches referenced documents and caches them on the application server', 'The Extended Reference Pattern references the same document from multiple parent collections to avoid data duplication'], answer: 1, explanation: 'Problem: an order document references a customer by customerId. Every time an order is displayed, you need to fetch the customer name and address. This requires a $lookup join or two round trips. Extended Reference solution: embed a subset of the customer data directly in the order: { orderId: 123, customerId: "abc", customerName: "Jane", customerAddress: "123 Main St" }. Benefits: the full customer document remains in the customers collection. A $lookup is no longer needed for the most common read. Tradeoffs: embedded data can go stale if the customer updates their name or address. Mitigation strategies: only embed data that rarely changes (like a name — not an email that changes frequently). Use event sourcing to propagate updates. Accept eventual consistency for historical records (an order from 2020 should keep the address from 2020, not be updated when the customer moves). When to use: when a subset of referenced data is accessed on almost every read of the parent document and that subset rarely changes.' },
    ...
    { q: 'What is the Bucket Pattern in MongoDB and what data problems does it address?', options: ['The Bucket Pattern creates one document per day for time series data and accumulates all events from that day into an array within the document', 'The Bucket Pattern groups many small related documents into larger bucket documents covering a time range or count range, reducing the total number of documents and index size for time series and IoT data', 'The Bucket Pattern distributes documents across a fixed number of buckets (shards) using a consistent hashing algorithm for even load distribution', 'The Bucket Pattern is a pagination technique that groups query results into fixed-size buckets to avoid skip-based pagination overhead'], answer: 1, explanation: 'Problem: IoT sensor data arrives at 1 record per second per device. Over a year, one device generates 31 million documents. Index size grows enormous. Many small documents means high overhead per document (BSON header, _id, index entries). Bucket Pattern solution: instead of one document per reading, group readings into buckets of e.g., 1 hour or 200 readings: { deviceId: "sensor-01", bucketStart: ISODate(...), bucketEnd: ISODate(...), readings: [ { ts: ..., temp: 22.5 }, ... ], count: 200, minTemp: 18.0, maxTemp: 24.1 }. Benefits: 200x fewer documents and index entries. Pre-computed aggregates (min, max, avg) avoid repeated aggregation scans. Range queries on bucketStart use an index efficiently. Tradeoffs: queries for individual readings require array scan within the bucket document. Bucket documents can be large — balance bucket size with document size limit (16MB). MongoDB 5.0+ has native time series collections that implement this pattern automatically.' },
