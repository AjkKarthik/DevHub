...
const qna: QnaItem[] = [
  ...
  { q: 'Why does gRPC\'s RetryInfo error detail matter for building a correct client retry policy, beyond just knowing a status code is "retryable"?', a: 'Simply knowing a status code like UNAVAILABLE is generically retryable is not enough to retry safely — retrying immediately in a tight loop against a server that is overloaded makes the overload WORSE (a retry storm), and different failure causes warrant different backoff behavior. The RetryInfo detail message lets the SERVER tell the client exactly how long to wait before retrying (a retry_delay field) based on its own knowledge of the failure (e.g. "I\'m rate-limiting you, retry in 30s" vs "transient network blip, retry in 100ms") — a client that respects this server-specified delay behaves far better under real load conditions than one applying a fixed or purely client-guessed backoff strategy for every retryable status code uniformly.' },
