...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What does the stateless constraint in REST require?', options: ['The server never stores any data persistently in a database', 'Each request must contain all information the server needs to process it; the server does not store client session state between requests', 'The client stores all application state and never needs the server after the initial handshake', 'HTTP cookies and sessions are prohibited in properly designed RESTful APIs'], answer: 1, explanation: 'Stateless: each HTTP request is self-contained and the server cannot rely on context from previous requests. Clients send credentials (JWT, API key) with every request. Benefits: horizontal scalability (any server can handle any request since there is no session affinity), reliability, and visibility (each request tells the full story). Trade-off: larger request payloads. This is why stateless tokens like JWT are preferred over server-side sessions in REST APIs.' },
