...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What are the caching challenges unique to GraphQL compared to REST?', options: ['GraphQL cannot use any caching because queries are unique', 'GraphQL typically uses POST requests with unique query bodies, making HTTP-level CDN caching difficult; REST GET requests are naturally cacheable by CDNs and browsers', 'GraphQL responses cannot be cached in any form', 'Caching is identical between GraphQL and REST because both use HTTP'], answer: 1, explanation: 'REST HTTP caching: GET /users/123 can be cached by CDN, browser, or proxy. The same URL always returns the same resource. ETags and Cache-Control headers work naturally. GraphQL caching challenges: GraphQL queries typically use POST (unique bodies, not cacheable by default). Even GET-based GraphQL queries have long query strings making CDN routing harder. Persisted queries (APQ) with GET requests enable CDN caching of specific known queries. Application-level caching: Apollo Client and Relay maintain normalized client-side caches keyed by object type and ID. Server-level caching: use Redis to cache resolved data per field or per operation. Fragment-level caching (Apollo cache hints). REST is recommended when HTTP caching is critical (high-traffic, read-heavy APIs served by CDN).' },
  { q: 'What is a BFF (Backend for Frontend) pattern and how does it relate to GraphQL?', options: ['BFF is a GraphQL-specific pattern for handling frontend authentication', 'A separate backend layer for each client type that aggregates and transforms data from multiple microservices — GraphQL is often used to implement the BFF as it naturally supports flexible field selection', 'A browser caching technique specific to GraphQL subscriptions', 'A pattern where the frontend directly calls backend microservices without an intermediary'], answer: 1, explanation: 'BFF (Backend for Frontend): instead of one generic API, create a dedicated API layer for each client type: mobile BFF, web BFF, third-party partner BFF. Each BFF aggregates data from multiple microservices and shapes it specifically for its client. GraphQL as BFF: GraphQL is a natural fit for BFF because: field selection (each client queries only its fields). Schema stitching/federation (aggregate multiple service schemas in one BFF). Type safety (the schema serves as a contract between the BFF and frontend). Subscription support for real-time needs. Benefits: each client gets optimal data shapes without the server needing per-client endpoints. Schema evolution is localized to the BFF. Teams own their BFF without coordinating with every downstream service.' },
