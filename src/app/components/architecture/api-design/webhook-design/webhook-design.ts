...
const qna: QnaItem[] = [
  ...
  { q: 'Why is it important to test webhook handlers against the provider\'s Redeliver/replay feature specifically, rather than only against your own local simulator payloads?', a: 'A hand-written local simulator only sends whatever payload shape you assumed the provider sends — if your assumption about a field\'s type, an edge case (a refund event with a null "reason" field, for instance), or the exact signature header format is subtly wrong, a simulator built on that same wrong assumption will never catch the bug, since it validates your handler against your own possibly-incorrect mental model rather than reality. The provider\'s own Redeliver feature sends the ACTUAL historical payload exactly as their production system generated it (with real signatures, real edge-case field values from actual past events), which is the only way to catch a handler bug caused by a mismatch between what you assumed the payload looked like and what the provider genuinely sends.' },
];
