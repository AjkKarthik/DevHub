...
const qna: QnaItem[] = [
  ...
  { q: 'What breaks a CI-automated SDK generation pipeline if the OpenAPI spec has valid syntax but poor semantic quality (e.g. missing operationId, inconsistent naming)?', a: 'Generators derive method and model names FROM the spec\'s metadata — an endpoint missing an explicit operationId falls back to an auto-generated name derived from the HTTP method and path (like get_users_by_id_orders_get), producing unreadable, unstable method names in every generated SDK that can change unexpectedly whenever the path changes slightly. Inconsistent naming conventions across endpoints (userId in one path, user_id in another) similarly propagate directly into generated model field names across every target language, producing SDKs with jarring inconsistency baked in. This is why teams that automate SDK generation in CI also run spec linters (Spectral) as a gate before generation — a spec that "validates" as syntactically correct OpenAPI can still produce genuinely unusable generated SDKs without semantic quality rules enforced separately.' },
