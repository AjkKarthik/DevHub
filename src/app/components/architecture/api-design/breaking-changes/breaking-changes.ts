...
const quiz: QuizQuestion[] = [
  ...
  { q: 'How do automated tools detect breaking changes in OpenAPI specifications?', options: ['Breaking change detection requires manual review — no automated tool can reliably detect semantic breaking changes', 'Tools like openapi-diff, Optic, and Spectral compare two OpenAPI spec versions and flag breaking changes such as removed paths, added required fields, changed types, and modified response schemas', 'Breaking changes can only be detected at runtime via consumer error monitoring — static spec analysis misses too many cases', 'Only major version increments trigger breaking change detection — minor changes are assumed non-breaking by all tooling'], answer: 1, explanation: 'Automated breaking change detection: openapi-diff — compares two OpenAPI specs and classifies changes as breaking, potentially breaking, or non-breaking. Breaking: path removed, required parameter added, response field removed or type changed, security scheme removed. Potentially breaking: new required response field (strict clients may fail). Non-breaking: new optional parameter, new path, new optional response field. Optic (API Change Management): tracks OpenAPI spec changes over time, flags breaking changes in CI, generates a human-readable diff. buf breaking: detects breaking changes in Protocol Buffer schemas. graphql-inspector: compares two GraphQL schemas. CI integration: on every PR, compare the PR spec against the production spec. Fail the PR if breaking changes are detected and the branch is not a new major version branch. Limitation: tools detect structural breaking changes, not semantic ones. Changing the meaning of a field value (status "1" was "active" now means "inactive") is not detectable by tooling.' },
...
const qna: QnaItem[] = [
  ...
  { q: 'How do you communicate breaking changes to API consumers effectively?', a: 'Multi-channel communication: different consumers monitor different channels — use all of them. Changelog: a versioned changelog in docs or CHANGELOG.md with breaking changes clearly labeled. Response headers: Deprecation and Sunset headers (RFC 8594) embedded in responses reach consumers who monitor headers programmatically. API portal / docs: update documentation before the change, including a migration guide with before/after examples. Email notification: send to all registered developers with sufficient advance notice. SDK warnings: include runtime deprecation warnings in your SDK that log when deprecated methods are called. Status page: announce deprecation timelines — consumers monitoring status pages will see it. Content of announcement: what is changing, why, when (exact date), how to migrate (step-by-step examples), what happens if they do not migrate (410 Gone after sunset), who to contact. Measure effectiveness: monitor deprecated endpoint traffic after announcement. If usage is not declining, follow up directly with the most active consumers. A sunset date that passes with active consumers is a production outage — track it proactively and extend if needed.' },
  { q: 'What is the difference between a breaking change and a behavior change?', a: 'Breaking change: alters the API interface in an incompatible way — clients must update their code to keep working. Examples: removed field, changed type, added required field. Detected by schema comparison tools (openapi-diff, Optic). Clearly versioned and announced. Behavior change: the API interface remains identical but the behavior changes. Examples: changing sort order of results (ascending to descending). Changing precision of floating-point calculations. Fixing a bug that clients depended on as a feature. Changing time zone handling for date calculations. NOT detected by schema comparison tools. NOT caught by contract tests unless they verify the specific behavior. Risk: behavior changes can be equally impactful as structural breaking changes. A client that depends on a specific sort order will silently produce wrong results after a sort order change — no validation error, just wrong behavior. Process for behavior changes: document existing behavior explicitly in prose documentation, not just the schema. Write behavioral tests that capture documented semantics. Treat any change to documented behavior as a breaking change requiring versioning and announcement.' },
