...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What actually changed between RFC 7807 and its successor RFC 9457, given both define the same Problem Details JSON fields?', options: ['RFC 9457 removed the "instance" field entirely for privacy reasons', 'RFC 9457 is primarily a clarification and registration update — it moved Problem Details to Standards Track (7807 was only Informational), clarified extension member handling, and formally registered the media type, without changing the core type/title/status/detail/instance field set', 'RFC 9457 replaced JSON with a binary format for performance', 'RFC 9457 requires OAuth2 authentication for all error responses'], answer: 1, explanation: 'The core Problem Details fields (type, title, status, detail, instance) are unchanged between the two RFCs — RFC 9457\'s main contribution was elevating the specification from Informational to Standards Track status (giving it more formal weight for adoption) and clarifying ambiguities around extension members and media type registration that implementers had run into with 7807. This is why frameworks and documentation now cite 9457 as the current reference, even though code written against 7807\'s field definitions continues to work identically — there was no breaking change to the wire format itself.' },
...
const qna: QnaItem[] = [
  ...
  { q: 'Why do many production APIs deliberately return 404 Not Found instead of 403 Forbidden when a user requests a resource they are not authorized to see, even though 403 is the "technically correct" status code?', a: 'Returning 403 for a resource a user is not authorized to access CONFIRMS to the caller that the resource exists (they just aren\'t allowed to see it) — for sensitive resources (another user\'s private data, an internal admin record), this is itself an information leak: an attacker probing sequential IDs can map out which IDs are valid just from the 403-vs-404 pattern, without ever seeing the actual data. Returning 404 for both "does not exist" and "exists but you can\'t see it" denies the attacker that enumeration signal at the cost of a less semantically precise status code — a deliberate security-over-purity tradeoff many APIs make for authorization failures on individually-identifiable private resources, while still using genuine 403 for coarser cases like "you don\'t have the admin role" where confirming the endpoint\'s existence carries no real risk.' },
