...
const qna: QnaItem[] = [
  ...
  { q: 'What is OWASP API Security Top 10 and what are the most critical API vulnerabilities?', a: 'OWASP API Security Top 10 (2023): API1 Broken Object Level Authorization: accessing other users data by changing an object ID. API2 Broken Authentication: weak or missing authentication mechanisms. API3 Broken Object Property Level Authorization: returning or accepting more fields than the user is authorized for (mass assignment, excessive data exposure). API4 Unrestricted Resource Consumption: no rate limiting, allowing DoS via resource exhaustion. API5 Broken Function Level Authorization: accessing admin endpoints as a regular user. API6 Unrestricted Access to Sensitive Business Flows: abusing business logic (e.g., skipping payment in a checkout flow). API7 Server Side Request Forgery: using the API to probe internal infrastructure. API8 Security Misconfiguration: default passwords, verbose error messages, unnecessary features enabled. API9 Improper Inventory Management: undocumented or deprecated API versions. API10 Unsafe Consumption of APIs: trusting third-party APIs without validation.' },
