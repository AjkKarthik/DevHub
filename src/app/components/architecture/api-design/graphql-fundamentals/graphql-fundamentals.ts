...
const quiz: QuizQuestion[] = [
  ...
  { q: 'Why does DataLoader\'s batching rely on JavaScript\'s event loop microtask timing rather than an explicit "flush now" call from the developer?', options: ['DataLoader actually requires an explicit flush() call after every batch of .load() calls', 'DataLoader defers each queued key to the end of the current tick (via process.nextTick or a microtask) rather than firing immediately, giving every synchronously-executing resolver in that GraphQL query a chance to register its .load() calls before the batch function actually runs — this is what allows resolvers across different fields to transparently share one batched query without any coordination code between them', 'DataLoader uses a fixed 10-millisecond timer to decide when to flush', 'DataLoader only batches requests within the same HTTP connection, not the same tick'], answer: 1, explanation: 'Because GraphQL resolvers for sibling fields (e.g. each order\'s customer field) all execute synchronously within the same tick before any asynchronous I/O actually happens, DataLoader can queue up every .load(id) call made during that tick and defer the actual batched fetch until the tick ends (via a microtask/process.nextTick) — by which point it has collected every ID any resolver in that request wanted, letting it issue one batched query for all of them. This is a deliberate design choice: DataLoader works because JS resolvers naturally run to completion (their synchronous parts) before yielding, which is precisely the window DataLoader exploits to collect a full batch with zero coordination code between unrelated resolvers.' },
