...
const qna: QnaItem[] = [
  ...
  { q: 'A GET /users/123/orders?status=shipped endpoint mixes both a path parameter and a query parameter — why is putting "status" in the path (e.g. /users/123/orders/shipped) considered incorrect design?', a: 'The identity rule of thumb applies here: 123 (the user) is required for the request to make sense at all and identifies WHICH parent resource\'s orders you want — it correctly belongs in the path. status=shipped is a FILTER on that same collection, not a separate resource identity — /users/123/orders/shipped would misleadingly suggest "shipped" is itself a nested resource or sub-collection under orders, when it is really just one of many possible filter values (pending, cancelled, delivered) applied to the SAME collection endpoint. Modeling filters as path segments also breaks combinability — you cannot cleanly express "shipped AND created after date X" as path segments the way you can as ?status=shipped&createdAfter=2024-01-01 query parameters.' },
