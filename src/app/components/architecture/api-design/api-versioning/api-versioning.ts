...
const quiz: QuizQuestion[] = [
  ...
  { q: 'Why does header-based versioning (API-Version: 2) interact poorly with HTTP caching layers unless configured carefully, while URL path versioning does not have this problem?', options: ['Header versioning has no interaction with caching at all', 'A cache normally keys entries by URL alone — two requests to the same URL but with different API-Version headers would be treated as the SAME cache entry unless the cache is explicitly configured to vary its cache key by that header (via the Vary response header), risking a client silently receiving a cached response meant for a different API version', 'Header versioning always disables caching entirely by protocol requirement', 'URL path versioning cannot be cached at all, making header versioning strictly superior for cacheable endpoints'], answer: 1, explanation: 'Since the URL is the default cache key, a request to /orders/123 with API-Version: 1 and another to the same /orders/123 with API-Version: 2 look identical to a cache that only keys on the URL — the cache may serve the v1 response to a v2 request or vice versa. Fixing this requires the server to send Vary: API-Version so caches correctly treat different header values as different cache entries. URL path versioning (/v1/orders/123 vs /v2/orders/123) avoids this entirely because the version is baked into the URL itself, which is always part of the cache key by default — one reason URL path versioning remains popular despite being less "RESTfully pure."' },
  ...
  { q: 'Why do most public APIs expose only the MAJOR version in the URL (/v1/, /v2/) and never expose MINOR or PATCH versions to callers at all?', options: ['MINOR and PATCH versions do not exist in API semantic versioning', 'Exposing MINOR/PATCH would force clients to specify an exact version for every call, defeating the purpose of non-breaking changes being automatically available — clients on /v1/ should transparently receive all backward-compatible MINOR/PATCH improvements without any action, which is only possible if those sub-versions are invisible to the URL', 'MINOR and PATCH versions are only relevant for internal APIs, never public ones', 'URL versioning technically cannot support more than one number'], answer: 1, explanation: 'The entire point of MINOR and PATCH increments is that they are safe for existing clients to receive automatically without any migration effort — if callers had to specify /v1.3.2/ explicitly, every non-breaking improvement would still require a client-side change to adopt, which defeats the purpose of calling it "non-breaking" in the first place. By exposing only MAJOR in the URL, a client pinned to /v1/ transparently and automatically receives every backward-compatible MINOR and PATCH improvement the API ships, with a new URL segment (/v2/) reserved specifically for the rare case that genuinely requires a deliberate migration.' },
];
...
const qna: QnaItem[] = [
  ...
  {
    q: 'If a deprecated API version\'s scheduled sunset date arrives but monitoring shows it is still receiving significant traffic, what should you do?',
    a: 'Extend the sunset date rather than removing the version on schedule — a hard calendar-based cutoff that breaks active consumers, even ones who ignored deprecation warnings, causes real production incidents for real users (and support burden for you), which is almost always worse than a delayed removal. Before extending, identify WHO is still calling it (via API key/traffic analysis) and reach out directly if possible — sometimes traffic persists because a specific high-value customer\'s integration was never migrated, and a direct conversation resolves it faster than a blanket deadline extension. Treat the sunset date as a planning target communicated to consumers, not an inviolable technical constraint that must fire regardless of real-world impact.',
  },
  ...
  { q: 'Why should a sunsetted endpoint keep returning 410 Gone for months after removal, rather than simply removing the route entirely (causing a connection error or 404)?', a: 'A 410 Gone response with an explanatory body ("this endpoint was retired on X date, see migration guide at Y") gives a developer debugging a broken integration an immediately actionable, human-readable signal about exactly what happened and what to do next. Removing the route entirely produces a generic connection failure or ambiguous 404 (which could also mean "this specific resource ID does not exist," a totally different problem) — forcing the developer to guess whether the issue is a typo, a networking problem, or a retired endpoint, wasting debugging time on both sides. Keeping the informative 410 response alive for an extended window costs almost nothing to maintain and meaningfully improves the experience for the inevitable slow-to-update clients who eventually hit the retired route.' },
