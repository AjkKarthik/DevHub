...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the difference between Facade and Adapter?', options: ['Facade wraps a single class; Adapter wraps a system', 'Facade simplifies a complex subsystem behind a unified interface; Adapter translates an existing interface to a different one that the client expects', 'Adapter is always bidirectional; Facade is unidirectional', 'They are the same pattern with different names in different programming communities'], answer: 1, explanation: 'Facade and Adapter are both wrappers but solve different problems. Adapter: you have interface A and interface B; you create an adapter that implements A and delegates to B, solving incompatibility. The goal is interface compatibility. Facade: you have a complex subsystem with many classes; you provide a simple interface hiding the complexity. The goal is simplification. An Adapter wraps one incompatible interface; a Facade wraps a complex system. A Facade could internally use Adapters to integrate different subsystem components.' },
