...
const quiz: QuizQuestion[] = [
  ...
  { q: 'How does Bridge differ from Adapter?', options: ['Bridge adapts existing incompatible classes; Adapter decouples design hierarchies', 'Adapter is designed upfront to separate concerns; Bridge is applied retroactively to make incompatible things work', 'Bridge is designed upfront to decouple abstraction from implementation; Adapter is applied retroactively to make existing incompatible interfaces work together', 'They are identical patterns with different names'], answer: 2, explanation: 'Bridge is designed upfront: the architect anticipates that both the abstraction and implementation will vary independently and structures the design with Bridge from the start. Adapter is applied retroactively: you have existing code with incompatible interfaces and you wrap one to make it compatible with the other. Bridge is about future extensibility; Adapter is about present compatibility. Both use composition, but their purpose and design context differ.' },
