...
const qna: QnaItem[] = [
  ...
  { q: 'What is Pure Fabrication in GRASP and when should you apply it?', a: 'Pure Fabrication: invent a class that does not represent a concept in the domain but is created purely to achieve low coupling, high cohesion, or reuse. Example: saving an order to a database is not naturally a responsibility of the Order class (order does not know about databases). Creating a database-aware Order class would lower cohesion. Solution: invent an OrderRepository class (pure fabrication) to handle persistence. It has no domain meaning but achieves better design. Repository, Data Access Objects (DAOs), Service classes, and Helper utilities are often Pure Fabrications. Apply when placing a responsibility on a real domain class would reduce its cohesion or increase coupling.' },
