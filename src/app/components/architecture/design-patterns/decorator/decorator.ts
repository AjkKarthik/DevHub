...
const quiz: QuizQuestion[] = [
  ...
  { q: 'How does Decorator differ from Inheritance for adding behavior?', options: ['Decorator adds behavior at runtime via composition; inheritance adds behavior at compile time via subclassing', 'They are identical but Decorator requires less code', 'Inheritance is more flexible because it can override any method; Decorator can only add behavior around method calls', 'Decorator works only with interfaces; inheritance works with abstract classes'], answer: 0, explanation: 'Inheritance adds behavior statically: you create a subclass before the program runs. Decorator adds behavior dynamically at runtime: wrap objects on the fly, stack multiple decorators, and remove them without changing classes. Inheritance causes a subclass explosion when you need multiple independent features: LoggingService, CachingService, LoggingCachingService, CachingRetryService, etc. Decorator composes independently: cache(log(retry(service))) stacks all three without a separate class for each combination. However, decorators cannot access private members of the wrapped class as a subclass could.' },
