...
const qna: QnaItem[] = [
  ...
  },
  {
    ...
    a: 'No. C# attributes are compile-time metadata annotations — they do not wrap objects at runtime. The two common AOP frameworks that use attribute-style syntax actually work by two DIFFERENT mechanisms: Castle DynamicProxy generates a real runtime proxy object that wraps the original — genuinely Decorator-shaped composition. PostSharp instead weaves aspect code directly into the compiled IL at build time, with no wrapper object created at all — closer to compile-time code insertion than to Decorator\'s runtime object composition, even though both use attributes as the trigger syntax.',
  },
