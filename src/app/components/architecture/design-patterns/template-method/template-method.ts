...
const qna: QnaItem[] = [
  ...
  { q: 'How does the Hollywood Principle in Template Method relate to the broader Inversion of Control concept used in DI containers and frameworks?', a: 'Template Method is one of the earliest and most concrete expressions of Inversion of Control: instead of your code calling a library\'s functions in whatever order it wants (traditional procedural flow), the framework/base class owns the control flow and calls back into YOUR code (the overridden hook methods) at predetermined points — this is the exact same inversion later generalized by DI containers ("the container constructs and calls your classes, not the other way around") and web frameworks ("the framework calls your controller action, you don\'t call the framework\'s request loop"). Recognizing Template Method as an early, narrow instance of IoC helps explain why IoC containers feel conceptually familiar even to developers who learned Template Method first — both invert who is "in charge" of the call sequence.' },
