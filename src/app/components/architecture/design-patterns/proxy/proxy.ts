...
const quiz: QuizQuestion[] = [
  ...
  { q: 'Can Decorators be stacked (multiple decorators wrapping one object) the way you can with multiple middleware, and does the same apply to Proxy?', options: ['Neither pattern supports stacking multiple wrappers', 'Yes for Decorator — stacking is a core use case (LoggingDecorator wrapping a CachingDecorator wrapping the real object); Proxy is typically used singly per concern since each proxy type (virtual, protection, remote) usually addresses one specific access-control concern rather than being designed for composition', 'Only Proxy supports stacking; Decorator does not', 'Both patterns explicitly forbid stacking by design'], answer: 1, explanation: 'Decorator is explicitly designed for composability — wrapping a component in multiple decorators (each adding one independent behavior) is the pattern\'s primary value proposition, and the order of stacking often matters (logging outside caching vs caching outside logging behave differently). Proxy CAN technically be stacked too, but its typical use addresses a single access-control concern (lazy creation, authorization check, remote call marshalling) at a specific point, so multiple proxies wrapping one object is less common in practice than multiple decorators, even though nothing structurally prevents it.' },
...
const qna: QnaItem[] = [
  ...
  { q: 'What is the Smart Reference Proxy and how is it used?', a: 'Smart Reference is actually the CLASSIC fourth GoF proxy type (Remote, Virtual, Protection, Smart Reference) — this page\'s own theory and quick reference substitute the more commonly-used "Caching Proxy" as the fourth named type instead, since caching is the far more frequent real-world use case, but Smart Reference is worth knowing as the original. It adds behavior around object access without the client knowing: reference counting proxy: tracks how many clients hold a reference to the real object; when the count drops to zero, the proxy releases the real object. Thread-safe proxy: wraps all method calls with a lock to make a non-thread-safe service thread-safe transparently. Null safety proxy: checks for null before delegating and returns safe defaults on null. COM objects in Windows use reference counting proxies. C++ shared_ptr is a Smart Reference that counts references and deletes the managed object when the count reaches zero. .NET COM interop wrappers are Smart Reference Proxies managing native object lifetime.' },
  { q: 'How does Proxy interact with dependency injection?', a: 'Proxies integrate with DI by registering the proxy as the implementation for an interface while the real service is injected into the proxy. Example: register IOrderService with a proxy in DI: services.AddScoped<OrderService>(); services.AddScoped<IOrderService>(sp => new CachingOrderProxy(sp.GetRequiredService<OrderService>(), sp.GetRequiredService<IMemoryCache>())). All code that depends on IOrderService receives the proxy transparently. This pattern enables transparent cross-cutting concern application: all callers get caching or authorization without code change (logging specifically is this page\'s own Decorator counterpart, not a Proxy concern — see Proxy vs Decorator vs Adapter above). .NET provides Scrutor library and ASP.NET Core middleware pipeline as higher-level alternatives to manual proxy DI wiring.' },
];
