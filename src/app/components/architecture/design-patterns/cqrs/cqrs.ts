...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is Command-Query Separation (CQS) at the method level, and how does CQRS extend that same idea to the architectural level?', options: ['CQS and CQRS are unrelated concepts that happen to share similar names', 'CQS (Bertrand Meyer) says a single method should either change state (a command, returns void) or return data (a query, no side effects) but never both; CQRS applies that same command/query split not to individual methods but to entire models, handlers, and potentially data stores', 'CQS applies only to databases; CQRS applies only to APIs', 'CQS is the modern replacement for CQRS'], answer: 1, explanation: 'CQS is a method-design guideline: asking a question (query) should not change the answer (no side effects), and a command that changes state should not also return domain data. CQRS takes this same principle and scales it up architecturally — instead of just each METHOD being purely a command or a query, entire separate MODELS (and optionally separate data stores) are built for the write side and read side, applying the same "don\'t mix reading and writing" discipline at a much larger structural scale than a single method.' },
