...
const qna: QnaItem[] = [
  ...
  { q: 'What is the difference between "accidental duplication" and "duplication of knowledge" as DRY was originally defined by the Pragmatic Programmer?', a: 'DRY\'s original formulation targets duplication of KNOWLEDGE — the same business rule, algorithm, or fact expressed in multiple places, such that a business change requires updating all of them in sync (miss one, and the system becomes internally inconsistent). Accidental duplication is when two pieces of code happen to look textually similar right now but represent genuinely unrelated concerns that would only coincidentally need the same change — validating a US zip code and validating a product SKU might both currently be "5-character regex checks," but they represent different business rules that will diverge independently, so merging them into one shared function creates false coupling rather than removing a real duplication of knowledge.' },
