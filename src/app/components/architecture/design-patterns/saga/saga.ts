...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is a compensating transaction and what constraints must it satisfy?', options: ['A transaction that compensates for slow database performance by caching results', 'A transaction that semantically undoes the effect of a previously committed local transaction, since the local transaction cannot be rolled back after the service has committed', 'A transaction that compensates for a failed network call by retrying it', 'A refund transaction for failed payment processing only'], answer: 1, explanation: 'A compensating transaction reverses the business effect of a committed local transaction. It cannot be a database rollback (the local transaction is already committed). It is a new transaction that applies the reverse business operation: reserved inventory -> release inventory. cancel shipment -> credit the account. Compensating transactions must be: idempotent (can be retried safely). Commutative (order-independent with other concurrent operations where possible). Eventually produce a consistent state. Not all operations are compensable: a sent email or a printed document cannot be truly undone; you can only mitigate (send a cancellation email, but cannot unsend).' },
];
...
const qna: QnaItem[] = [
  ...
  { q: 'What is the difference between Saga and Two-Phase Commit (2PC)?', a: '2PC uses a global lock: the coordinator locks all participating services resources during the prepare phase, then either commits all or aborts all atomically. Provides strong consistency (ACID). Drawbacks: requires all services to support the 2PC protocol, blocks resources during the protocol (availability reduction), coordinator failure can leave participants in a prepared-but-not-committed state (in-doubt transactions). Saga has no global lock: each service commits locally immediately. Services release resources right away. Provides eventual consistency (BASE). Saga failures require compensations rather than rollback. 2PC is correct for local transactions in the same database. Saga is appropriate for distributed transactions across independent services or databases.' },
