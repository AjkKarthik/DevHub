...
const qna: QnaItem[] = [
  ...
  { q: 'How does the Command pattern compare to Memento for implementing undo?', a: 'Command-based undo: each command stores enough information to reverse itself. TextInsertCommand stores what was inserted and its position; undo() deletes that text. Memory efficient: only the delta is stored, not the full state. Suitable when operations are well-defined and reversible. Memento-based undo: each undo step is a full state snapshot. Simple to implement when the state is not easily expressed as reversible operations. Easier to implement for complex state where individual operation reversal is hard to reason about. The choice depends on the domain: Command undo is preferred for most cases because it is memory-efficient; Memento undo is appropriate when the state is complex or operations are not straightforwardly invertible.' },
