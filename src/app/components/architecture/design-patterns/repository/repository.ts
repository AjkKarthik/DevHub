...
const qna: QnaItem[] = [
  ...
  { q: 'If a generic Repository<T> base class is used only as an internal IMPLEMENTATION DETAIL (never exposed as the public interface), does it still cause the interface-leakage problems associated with generic repositories?', a: 'No — the interface-leakage criticism applies specifically to exposing IRepository<T> (with its generic FindAll/Add/Remove surface) as the PUBLIC contract that calling code depends on, since that lets any caller bypass domain-specific query constraints. A generic base CLASS used purely to avoid rewriting boilerplate CRUD plumbing inside each concrete, narrowly-scoped repository (OrderRepository : RepositoryBase<Order>, exposing only OrderRepository-specific methods publicly) gets the code-reuse benefit without the interface-leakage cost, since callers only ever see and depend on the narrow, domain-specific public interface — the generic mechanics stay hidden as an internal reuse detail.' },
