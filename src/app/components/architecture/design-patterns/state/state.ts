...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the difference between State pattern and Strategy pattern?', options: ['State manages an internal context; Strategy is applied externally', 'Both use composition to delegate to interchangeable objects; Strategy implements different algorithms selected by the client; State implements different behaviors for different lifecycle phases selected by the state machine itself', 'Strategy objects know about each other; State objects are independent', 'State requires persistence; Strategy is always in-memory'], answer: 1, explanation: 'State and Strategy are structurally similar (both use a reference to an interface and delegation) but differ in intent and who controls the switching. Strategy: the context or client selects the strategy. The strategies are independent algorithms. The context typically uses one strategy at a time without transitioning based on the algorithms. State: the states know about each other (or the context) and trigger transitions. The object transitions through a defined set of states as part of its lifecycle. Strategy is about choosing how to perform an operation; State is about what operations are available as the object moves through its lifecycle.' },
];
