...
const qna: QnaItem[] = [
  ...
  { q: 'What are projections in Event Sourcing and how are they built?', a: 'A projection rebuilds a read model from the event stream. It subscribes to events (from the beginning of history for initial build, or from last processed position for incremental update) and applies each event to maintain a denormalized view. Example: an OrderSummaryProjection handles OrderPlaced (creates a summary record), OrderShipped (updates shipping status), and OrderCancelled (marks as cancelled). Projections can be rebuilt from scratch by replaying all events — this is a key advantage: add a new query requirement, write a new projection, and rebuild it from historical events in minutes. Projections are eventually consistent after the write model; they update asynchronously as events arrive.' },
