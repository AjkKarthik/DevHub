...
const quiz: QuizQuestion[] = [
  ...
  { q: 'Between when a service commits its local transaction (including the outbox insert) and when the event actually reaches the broker, what is that window called, and can it be eliminated?', options: ['It is called "hard consistency" and it can be reduced to exactly zero with enough hardware', 'It is a window of eventual consistency — some delay always exists between local commit and broker publish (relay polling interval or CDC lag), and it cannot be eliminated entirely without giving up the pattern\'s core benefit of decoupling the local transaction from the broker call', 'It does not exist — Outbox makes local commit and broker publish atomic', 'It only exists when using a polling relay, never with CDC'], answer: 1, explanation: 'The entire point of Outbox is to decouple "commit the local transaction" from "reliably get the event to the broker" so a broker outage never blocks or corrupts the local transaction — but that decoupling inherently means there is always SOME delay (a polling interval, or CDC replication lag) between commit and the event actually reaching the broker. Consumers and downstream systems must be designed around this real, small eventual-consistency window; systems that need the event visible to other services within the same transaction cannot use Outbox and need a different consistency model entirely.' },
...
const qna: QnaItem[] = [
  ...
  { q: 'What operational task does a CDC-based outbox relay require that a polling relay handles automatically, and why is it easy to overlook?', a: 'A polling relay typically deletes or marks outbox rows as processed after successfully publishing them, keeping the outbox table small automatically as a side effect of its normal operation. A CDC-based relay (Debezium) only READS the replication stream — it never touches the outbox table\'s rows itself, so nothing deletes old, already-captured rows unless a separate scheduled cleanup job is set up explicitly. Teams migrating from polling to CDC for its lower latency sometimes miss this, inheriting an outbox table that grows unboundedly because the automatic cleanup the polling approach provided for free is now nobody\'s job.' },
