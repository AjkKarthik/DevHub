...
const quiz: QuizQuestion[] = [
  ...
  {
    q: 'What does Visitor break in terms of encapsulation, and why is that considered an acceptable trade-off for the flexibility it provides?',
    options: [
      'Visitor requires making all element fields public so visitors can read internal state',
      'Visitor requires elements to expose enough of their internal structure (via public getters or the accept() mechanism) for external visitor classes to operate on, which pulls some operational logic outside the element class it conceptually belongs to',
      'Visitor has no encapsulation impact whatsoever',
      'Visitor only affects encapsulation in dynamically typed languages',
    ],
    answer: 1,
    explanation: 'Traditional OOP encourages keeping an object\'s behavior close to its data (methods operating on the class\'s own fields). Visitor deliberately moves operation logic OUTSIDE the element classes into separate visitor classes, which means elements often need to expose more of their internal state via public accessors than they otherwise would, so external visitors can compute against it. This is a conscious trade-off: Visitor sacrifices some encapsulation purity in exchange for being able to add new cross-cutting operations without modifying every element class — worthwhile when operations change far more often than the element type hierarchy does.',
  },
  ...
  { q: 'What specifically goes wrong if a Visitor implementation\'s Element.accept() method calls visitor.visit(this) using a base-class-typed "this" reference instead of relying on the element\'s own overridden accept()?', options: ['Nothing goes wrong — the result is identical either way', 'If accept() is not itself an overridden virtual method on each concrete element, the call resolves visitor.visit(this) using this\'s COMPILE-TIME type (often the base Element type) rather than its runtime type, silently invoking the wrong overload — defeating the whole point of double dispatch', 'The code fails to compile in all statically-typed languages', 'It only causes issues in interpreted languages, never in compiled ones'], answer: 1, explanation: 'Double dispatch works ONLY because accept() is itself a virtual/overridden method — when called on an object typed as the base Element class, the runtime correctly dispatches to the CONCRETE element\'s accept() override (first dispatch, based on element\'s runtime type), and inside THAT concrete method, this refers to the concrete type at compile time, so visitor.visit(this) correctly resolves to the matching overload (second dispatch, based on the concrete type known at that point). If a developer mistakenly implements accept() as a single non-virtual method on the base class instead of overriding it per concrete type, the second dispatch never gets its correct compile-time type, and the wrong (or a generic/base) visit overload silently gets called instead.' },
