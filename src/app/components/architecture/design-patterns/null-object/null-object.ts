...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the Null Object pattern and what null-check problem does it solve?', options: ['A pattern that makes all values nullable to simplify API design', 'A behavioral pattern that provides a default object with do-nothing behavior as a substitute for null, eliminating null checks throughout the codebase', 'A pattern that converts null references to empty collections', 'A pattern for safely calling methods on possibly-null references using optional chaining'], answer: 1, explanation: 'Without Null Object: every call site that uses an optional object must check for null: if (logger != null) logger.log(msg). Null checks scatter throughout the code and are easy to forget. With Null Object: instead of returning null, return a NullLogger that implements the Logger interface with do-nothing methods. Call sites use the logger without checking: logger.log(msg) is always safe. The NullLogger absorbs the call and does nothing. This pattern replaces conditional logic with polymorphism, following the Tell-Do not Ask principle.' },
  { q: 'What is the difference between Null Object and Optional/Maybe type?', options: ['They are identical approaches to the same problem', 'Null Object is a real object with do-nothing behavior that transparently replaces null; Optional/Maybe is a wrapper type that forces the caller to explicitly handle the absent case', 'Optional forces null checks while Null Object eliminates them', 'Null Object works only with interfaces; Optional works with any type'], answer: 1, explanation: 'Null Object: transparent substitution — the caller does not know it has a do-nothing object. No code change at call sites. The caller does not handle the absent case because the Null Object handles it silently. Appropriate when do-nothing is a valid and expected behavior. Optional/Maybe (Java Optional, C# Nullable, Haskell Maybe): explicit absence representation — the caller must unwrap and handle the empty case. Forces callers to acknowledge and handle absence. Appropriate when absence requires different caller behavior. Use Null Object when silence is appropriate; use Optional when callers must act differently on absence.' },
  { q: 'What are the risks of overusing the Null Object pattern?', options: ['Null Object makes code slower because of the extra polymorphic dispatch', 'Null Object can hide bugs: silent do-nothing behavior may mask missing data or configuration errors that should have been caught', 'Null Object breaks serialization because do-nothing objects cannot be serialized', 'Null Object requires interfaces which adds too many files to the codebase'], answer: 1, explanation: 'The main risk of Null Object: silence. A NullLogger silently swallows all log messages. If you intended to configure a real logger but misconfigured the DI container, the NullLogger will silently hide all logs, making debugging very difficult. A NullPaymentGateway that silently does nothing could mask a failed payment integration. Mitigation: use Null Object only where do-nothing is genuinely the correct behavior (optional logging, optional analytics, optional decorators). For required collaborators, fail fast with an exception rather than silently doing nothing.' },
];
...
const qna: QnaItem[] = [
  ...
  { q: 'When is Null Object a better choice than using a default value?', a: 'Null Object is better than null defaults when: the dependency is an object with methods rather than a simple value. Default values work well for primitives (use 0 instead of null for an int). For objects: a NullDiscountStrategy that returns the original price is clearer than checking if discount == null and applying no discount. The Null Object makes the intent explicit: this is an intentional no-op, not a missing value. If multiple methods of the dependency are called throughout the code, the Null Object eliminates all null checks at once, while a null check approach would require checks at each call site.' },
  { q: 'How does Null Object relate to the Strategy pattern?', a: 'Null Object often combines naturally with Strategy. A Strategy defines a family of algorithms. One member of the family is the NoOpStrategy (or NullStrategy) that does nothing. Example: a compression strategy with ZipStrategy, GzipStrategy, and NoCompressionStrategy. Instead of if (compressionEnabled) compress(data), always call strategy.compress(data). When no compression is needed, set the NoCompressionStrategy. The caller is unaware whether compression is happening; it just calls the interface method. This combination simplifies client code and keeps the strategy selection in the configuration or factory, not scattered throughout the codebase.' },
  { q: 'How do you implement Null Object when the interface returns values?', a: 'When the interface method has a return type, the Null Object must return a sensible default. Patterns: return an empty collection (IEnumerable<T> -> return Enumerable.Empty<T>()). Return a zero or default value for numeric types. Return an empty string for string types. Return another Null Object for object return types: a NullCustomerRepository.FindById() returns a NullCustomer. Return false for boolean predicates. Return Task.CompletedTask for async void-like methods. The key is that the returned value should allow the caller to proceed without errors: empty collection is safe to iterate, empty string is safe to display, false can be handled by the caller as a valid absence.' },
  { q: 'What is the relationship between Null Object and the Liskov Substitution Principle?', a: 'A Null Object must be a valid substitution for the real object (Liskov Substitution Principle). It must implement the same interface and satisfy the same invariants expected by callers. A Null Object that throws NotImplementedException on some methods violates LSP and is worse than a null check. A Null Object that returns incorrect types or throws unexpected exceptions breaks callers that do not expect these behaviors. Correct Null Object: all methods execute without side effects and return safe defaults. If satisfying LSP is impossible (the method has a postcondition that the Null Object cannot meet), Null Object is the wrong pattern; use Optional or explicit null handling instead.' },
];
