...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What does the Dependency Inversion Principle (DIP) state?', options: ['Dependencies should be inverted by reversing method call order', 'High-level modules should not depend on low-level modules; both should depend on abstractions. Abstractions should not depend on details; details should depend on abstractions', 'All dependencies should be managed by a central dependency container', 'Dependencies should flow downward from high-level to low-level modules only'], answer: 1, explanation: 'DIP is the D in SOLID. Without DIP: high-level OrderService depends directly on low-level SqlOrderRepository. Changing the repository requires changing OrderService. With DIP: both depend on IOrderRepository (the abstraction). OrderService depends on the interface. SqlOrderRepository implements the interface. The direction of source code dependency is inverted: the low-level module conforms to the interface defined by the high-level module (or defined in a shared abstractions layer). This makes high-level business logic independent of infrastructure details.' },
