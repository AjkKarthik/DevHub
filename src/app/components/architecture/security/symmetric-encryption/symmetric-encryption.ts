...
const qna: QnaItem[] = [
  ...
  { q: 'What is envelope encryption and how does AWS KMS implement it?', a: 'Envelope encryption: a data encryption key (DEK) is generated locally and used to encrypt the data. The DEK is then encrypted (wrapped) by a master key in KMS. Only the encrypted DEK is stored alongside the data. The plaintext DEK is only in memory during encryption/decryption operations. AWS KMS implementation: GenerateDataKey API: KMS generates a DEK and returns both the plaintext DEK and the DEK encrypted with the CMK (customer master key). Application encrypts data with plaintext DEK, then discards the plaintext DEK from memory. Store: encrypted data + encrypted DEK together. Decrypt: call KMS Decrypt with the encrypted DEK to get the plaintext DEK. Decrypt the data locally. Benefits: the bulk data never goes to KMS (only the small DEK does). Low latency for large data encryption. KMS access controls determine who can decrypt the DEK.' },
