...
const qna: QnaItem[] = [
  ...
  { q: 'Why is pinning the intermediate CA\'s public key generally safer than pinning the leaf (server) certificate\'s public key directly?', a: 'The leaf certificate is reissued frequently (every 90 days with Let\'s Encrypt, for example) as part of normal renewal, and each reissuance can involve a NEW key pair — pinning directly to a leaf key means the pin breaks on every routine renewal unless the app is updated in lockstep, which is operationally fragile for mobile apps with slow release cycles. The intermediate CA certificate that signs your leaf certificates changes far less often (often years between rotations) and can sign many different leaf certificates over its lifetime, so pinning to the intermediate\'s key tolerates routine leaf certificate renewal without breaking, while still preventing an attacker from presenting a certificate from a completely different, untrusted CA.' },
