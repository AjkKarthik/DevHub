...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the key difference between RBAC and ABAC access control models?', options: ['RBAC is for APIs; ABAC is for database access only', 'RBAC grants access based on user role membership; ABAC evaluates multiple attributes (user, resource, environment, action) for fine-grained access decisions', 'ABAC is simpler than RBAC and suitable for small applications', 'RBAC is dynamic; ABAC is static and requires manual policy updates'], answer: 1, explanation: 'RBAC (Role-Based Access Control): users are assigned to roles. Roles have permissions. Access check: does user have this role? Simple to implement and audit. Scales well for broad permission categories. ABAC (Attribute-Based Access Control): policies consider multiple attributes: user (department=engineering, clearance=secret). Resource (owner=userID, classification=confidential). Action (read, write, delete). Environment (time=business-hours, location=corporate-network). Example ABAC policy: allow read on documents where document.department == user.department and time is within business hours. ABAC enables fine-grained, context-aware access. More complex to implement and audit. PBAC (Policy-Based Access Control) is often used as a synonym for ABAC.' },
