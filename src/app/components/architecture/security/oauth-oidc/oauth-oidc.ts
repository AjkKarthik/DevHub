...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the difference between OAuth 2.0 and OpenID Connect (OIDC)?', options: ['OAuth 2.0 is for authorization only; OIDC adds an authentication layer on top of OAuth 2.0 by introducing the ID token', 'OIDC is older than OAuth 2.0 and is the foundation OAuth 2.0 is built on', 'OAuth 2.0 issues ID tokens; OIDC issues access tokens for API calls', 'OIDC replaces OAuth 2.0 completely and should always be used instead'], answer: 0, explanation: 'OAuth 2.0: an authorization framework. Answers: can this application access resource X on behalf of user Y? Issues access tokens for calling protected APIs. Does NOT define who the user is. OIDC (OpenID Connect): an identity layer on top of OAuth 2.0. Answers: who is this user? Adds the ID token (a JWT) that contains claims about the user identity: sub (user ID), email, name, profile. The userinfo endpoint provides additional claims. OIDC standardizes how identity is exchanged, while OAuth 2.0 standardizes authorization. When building authentication (Who is the user?): use OIDC. When building API authorization (Is this request allowed?): use OAuth 2.0 access tokens.' },
