...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is dynamic secrets and how does HashiCorp Vault generate them?', options: ['Secrets that are randomly generated on the client side each time they are needed', 'Credentials generated on-demand by the secrets manager with a limited TTL; Vault creates a real database user or cloud IAM role when requested and automatically revokes it when the TTL expires', 'Secrets that change every 30 seconds like TOTP', 'Encrypted secrets that are dynamically decrypted when the application reads them'], answer: 1, explanation: 'Dynamic secrets: instead of storing a long-lived database password, the application requests a credential from Vault. Vault creates a real, unique credential (creates a PostgreSQL user, generates an AWS IAM user) specifically for this application instance with a limited TTL (1 hour, 24 hours). When the TTL expires, Vault automatically revokes the credential (drops the PostgreSQL user, deletes the IAM user). Benefits: if the credential is leaked, it expires automatically. No credential sharing between service instances. Audit trail: Vault logs exactly which service requested and used each credential. Minimal blast radius: a leaked short-lived credential does far less damage than a long-lived shared password.' },
