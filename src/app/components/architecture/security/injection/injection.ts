...
const qna: QnaItem[] = [
  ...
  },
  {
    ...
    a: 'First-order injection: attacker sends malicious input in the same request that executes the vulnerable query. <strong>Second-order injection</strong>: attacker stores malicious data in the database (safely, via parameterized query), and later that data is retrieved and used unsafely in a different query — often in a context that "trusts" DB data. Example: user registers with username <code>admin\'--</code>. The registration is safe. Later, an admin lookup does <code>SELECT * FROM logs WHERE user = \'" + dbUsername + "\'"</code> — the stored value causes injection. Prevention: parameterize ALL queries, even when the value comes from the database.',
  },
