...
const qna: QnaItem[] = [
  ...
  { q: 'Why does adopting zero trust typically increase per-request latency and operational complexity compared to perimeter security, and why do organizations accept that cost?', a: 'Perimeter security does its expensive verification once (at the network edge, on initial connection) and then implicitly trusts everything inside — cheap per-request, but catastrophic if that boundary is ever breached. Zero trust re-verifies identity, device posture, and authorization on EVERY request and EVERY service-to-service call, which adds real overhead (extra round-trips for token validation, mutual TLS handshakes, continuous policy evaluation) that perimeter-only security never pays. Organizations accept this cost because the perimeter model\'s core assumption — that everything inside the network can be trusted — has repeatedly proven false in practice (compromised employee laptops, breached third-party vendors, lateral movement after a single point of entry), making the per-request verification cost of zero trust a worthwhile tradeoff against the catastrophic cost of undetected lateral movement.' },
  ...
  { q: 'Does adding more security FEATURES to an application always reduce its attack surface?', a: 'No — this is a common misconception. Every new feature, endpoint, or third-party integration is itself a POTENTIAL new entry point, regardless of whether it is a "security" feature or not. Adding an admin dashboard, a webhook receiver, or even a security-monitoring agent each expands what an attacker could target. Reducing attack surface is fundamentally about REMOVING and MINIMIZING exposure (unused ports, default accounts, unnecessary endpoints) — not about adding more code, which nearly always expands the surface rather than shrinking it, even when the added code\'s intent is protective.' },
