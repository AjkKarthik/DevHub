...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the difference between an allowlist and a denylist for input validation and which is preferred?', options: ['Allowlists are faster; denylists are more comprehensive', 'A denylist blocks known-bad patterns; an allowlist permits only known-good patterns. Allowlists are preferred because denylists cannot anticipate all attack variations', 'Allowlists are for user input; denylists are for network traffic', 'Both are equivalent; the choice depends on the number of valid values'], answer: 1, explanation: 'Denylist (blocklist): block input matching known-bad patterns. Example: reject input containing <script>, DROP TABLE, ../. Problem: attackers find bypass encoding or variations not in the list. Denylists are inherently incomplete and require constant updates. Allowlist (safelist/whitelist): permit only input matching known-good patterns. Example: username must match [a-zA-Z0-9_-]{3,32}. Anything not matching is rejected. Allowlists are robust: even novel attack strings are rejected if they do not match the expected format. Use allowlists for: character sets (what characters are valid), data types (numeric, UUID, email format), value ranges (age between 0-150), string lengths. Combine with output encoding: allowlists on input, encoding on output for defense in depth.' },
];
