...
const quiz: QuizQuestion[] = [
  ...
  { q: 'Why is submitting a domain to the HSTS preload list described as a "difficult to reverse" decision, and what would force reversal?', options: ['Preload list submissions expire automatically after 90 days, so reversal is never actually needed', 'Once a domain is baked into browser vendors\' shipped preload lists, removal requires the domain to be removed from the list AND for users to update to a browser version released after the removal — meaning HTTPS becomes effectively mandatory for that domain (and often all subdomains) for as long as users run older browser versions, even if the site later needs to serve plain HTTP for some legitimate reason', 'Preloading can be reversed instantly via a DNS TXT record', 'Reversal only affects Chrome, not other browsers, so it is not a real risk', 'None of the above'], answer: 1, explanation: 'The HSTS preload list is compiled directly into browser source code and shipped with browser releases — a site cannot simply remove the header and expect old cached behavior to disappear, since preloaded browsers never even attempt an HTTP connection to check for an updated header. Removing a domain from the master list stops it from being included in FUTURE browser builds, but users on already-released browser versions retain the old preloaded enforcement indefinitely, meaning a domain (and if includeSubDomains was set, every subdomain including ones that might not support HTTPS) can be locked out of ever serving plain HTTP again for a very long tail of legacy browser installations.' },
