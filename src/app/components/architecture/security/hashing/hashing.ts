...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is hash length extension attack and which hash algorithms are vulnerable?', options: ['An attack that extends the output length of a hash function', 'An attack that appends data to a hashed message and produces a valid hash of the extended message without knowing the original input; MD5, SHA-1, SHA-256, and SHA-512 are vulnerable; SHA-3 and HMAC are not', 'An attack that recovers the original input by extending the hash output with brute force', 'A collision attack specific to the Merkle-Damgard construction used in older hash functions'], answer: 1, explanation: 'Length extension attack: if you know H(secret + message) and the length of secret, you can compute H(secret + message + padding + extension) without knowing secret. This breaks naive HMAC construction where the server computes SHA256(secret + data). Vulnerable: MD5, SHA-1, SHA-256, SHA-512 (Merkle-Damgard construction). Not vulnerable: SHA-3 (sponge construction), HMAC (the HMAC construction specifically prevents this by applying the key correctly). Fix: always use HMAC instead of hash(key + data). If you are implementing a custom MAC, use HMAC with an approved algorithm.' },
];
