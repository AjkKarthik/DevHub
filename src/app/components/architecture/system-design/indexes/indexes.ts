...
-- FK columns are almost always joined or filtered — always index them`,
    explanation: 'Foreign key columns are JOIN targets. Without an index, every join requires a sequential scan of the child table. MySQL InnoDB actually auto-creates an index on the FK column if none exists (not merely a warning); PostgreSQL does neither — create FK indexes manually there.',
  },
  {
