...
const quiz: QuizQuestion[] = [
  ...
  { q: 'Two requests carrying the SAME idempotency key arrive at two different application server instances at almost the same instant (the client retried aggressively before the first response came back). If the idempotency-key store lookup and the "process the charge" step are not atomic together, what race condition can still cause a double charge despite the idempotency key being correctly implemented?', options: ['This scenario is impossible since idempotency keys are globally unique by definition', 'Both instances can independently check the store, both see "key not found yet" (since the first request hasn\'t finished processing and recording its result), and both proceed to charge the card before either one\'s result gets written back — the idempotency guarantee only holds if checking-and-reserving the key is one atomic operation, not a check followed by a separate write', 'The payment processor itself automatically deduplicates by card number', 'Idempotency keys only work for single-server deployments'], answer: 1, explanation: 'A correct idempotency implementation needs the "check if this key was already processed" and "claim this key for processing" to happen as a single atomic operation (e.g. an INSERT with a UNIQUE constraint on the idempotency key that fails for the second concurrent attempt, or a distributed lock/SETNX before doing any charge work) — if the implementation instead does a plain read-then-write (SELECT to check, then INSERT the result after processing), two nearly-simultaneous requests can both pass the check before either has written its result, both proceed to charge the card, and the idempotency key\'s row simply gets overwritten by whichever finishes last, silently hiding that a double-charge occurred. This exact race condition is why payment API idempotency implementations (Stripe\'s included) rely on a database uniqueness constraint or equivalent atomic claim, not a naive check-then-act pattern.' },
