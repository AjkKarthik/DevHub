...
# If primary fails, secondary claims VIP in ~3 seconds by default
# (default advert_int=1s x 3 missed advertisements) --
# tune advert_int lower for faster failover if needed`,
...
const qna: QnaItem[] = [
  ...
  { q: 'What is a connection draining period and why is it important for deployments?', a: 'Connection draining (also called deregistration delay) is a grace period during which a backend being removed from the load balancer pool continues to receive existing connections until they complete, while new connections are routed elsewhere. Without draining, removing a server during a deployment immediately terminates all in-flight requests to that server, causing errors for users mid-request. With draining, the server receives no new traffic but completes its current requests up to the drain timeout — AWS ALB\'s own default deregistration delay is 300 seconds (5 minutes), configurable from 0 to 3600 seconds; NGINX and other self-managed LBs are typically tuned much shorter (30-60 seconds) since there\'s no fixed vendor default. Load balancers like AWS ALB and NGINX support this natively. It is essential for zero-downtime rolling deployments and graceful service shutdown.' },
