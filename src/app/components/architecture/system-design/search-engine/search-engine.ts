...
- Autocomplete`,
  ...
  indexStructure: \`
    ...
    Shards: 30 primary shards -- 1 TB / 30 ~= 33 GB per shard (fits the
      10-50 GB recommended shard-size range), which works out to
      ~16.7M docs per shard (500M docs / 30), not the storage size
...
const qna: QnaItem[] = [
  ...
  { q: 'How does Elasticsearch handle horizontal scaling via sharding?', a: 'Elasticsearch divides an index into shards, each of which is an independent Lucene index. A primary shard handles writes and can have one or more replica shards for redundancy and read scaling. The default primary shard count is 1 per index (as of Elasticsearch 7.0+ -- earlier versions defaulted to 5), so production indexes almost always need an explicit number_of_shards setting chosen at creation time based on expected data volume. Documents are distributed across shards by consistent hashing of the document ID. Each shard is allocated to a node in the cluster; adding nodes allows Elasticsearch to redistribute shards automatically. Read requests fan out to all shards in parallel and results are merged. The number of primary shards is fixed at index creation time because changing it changes routing; plan shard count based on expected data volume, targeting shard sizes of 10-50 GB each for optimal performance.' },
  { q: 'An autocomplete system using a popularity-ranked in-memory trie serves stale suggestions — a query that trended yesterday still ranks #1 even though a new, more relevant query has since overtaken it in actual search volume. What architectural gap causes this staleness, and what design change fixes it without rebuilding the trie on every single search?', a: 'A trie built once from a snapshot of historical popularity data is, by construction, only as fresh as its last rebuild — if the trie is rebuilt hourly (or less often) via a background batch job, real-time shifts in query popularity have no way to surface until the next rebuild cycle runs, no matter how fast individual lookups are. The standard fix is decoupling "fast prefix lookup" from "freshness of ranking data": keep the trie (or FST) for structural prefix matching, but back the popularity SCORE for each candidate with a separately, more frequently updated source — e.g. a Redis sorted set incremented in near-real-time as queries come in, or a streaming aggregation pipeline (Kafka + a windowed counter) that periodically pushes updated scores into the serving layer — so the trie\'s candidate SET can rebuild on a slower cadence while the popularity ORDERING used to rank those candidates stays much fresher.' },
