...
const qna: QnaItem[] = [
  ...
  { q: 'A team instruments all their services directly with the Jaeger client SDK (not OpenTelemetry) to get tracing working quickly. Two years later they want to switch trace backends to evaluate Grafana Tempo. What does the choice to skip OpenTelemetry cost them at migration time?', a: 'Instrumenting directly against a specific backend\'s SDK (the Jaeger client, in this case) couples every instrumented call site in the codebase to that backend\'s API and data model — switching backends later means re-instrumenting the application code itself, not just swapping a configuration or export target. This is precisely the problem the OpenTelemetry Collector architecture solves: instrumenting against the vendor-neutral OpenTelemetry SDK once, then routing the resulting telemetry to whichever backend(s) you choose via Collector exporters, means a backend migration (Jaeger → Tempo, or adding a second backend for a transition period) is purely a Collector configuration change with zero application code changes. The upfront cost of adopting OpenTelemetry instead of a backend-specific SDK pays for itself the first time a team needs to change or add a backend.' },
