...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is cache stampede and how do you prevent it?', options: ['When a cache grows too large and evicts too many items', 'When many requests simultaneously query the database after a popular cache entry expires', 'When write-through caching causes database overload', 'When cache replication falls behind the primary'], answer: 1, explanation: 'Cache stampede occurs when a popular cache entry expires and many concurrent requests simultaneously find a cache miss and all query the database at once, overwhelming it. Prevent it with: probabilistic early expiration (refresh before expiry when request volume is high), request coalescing (only one request fetches from DB while others wait for the result), background refresh (asynchronously refresh entries before they expire), or using a lock so only one thread fetches from DB while others serve stale data.' },
