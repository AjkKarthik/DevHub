...
const qna: QnaItem[] = [
  ...
  },
  {
    ...
    a: 'CDN PoPs have massive aggregate bandwidth — Cloudflare\'s network passed 500 Tbps of provisioned external capacity in 2026 (the sum of every transit/peering/IX port across 330+ cities; actual traffic served on any given day is a small fraction of that provisioned total). They absorb volumetric attacks at the edge before packets reach your origin. They also provide IP reputation filtering, rate limiting, and WAF (Web Application Firewall) that can block application-layer attacks.',
  },
  ...
  { q: 'A CDN is configured with origin shield, but the origin server still sees a spike in requests every time a specific piece of content goes viral and its TTL happens to expire simultaneously across many edge nodes. Why doesn\'t the shield fully absorb this spike the way it normally reduces steady-state origin load?', a: 'Origin shield reduces the NUMBER of distinct origin-facing paths (many edge PoPs collapse to one shield node\'s worth of requests), but it does not by itself solve simultaneous cache-miss coordination at the shield layer — if the shield node\'s own cached copy of that popular object also expires and a burst of edge requests for it arrive in the same instant, the shield node can still forward multiple near-concurrent requests to origin for the same object before the first one\'s response comes back and repopulates the shield\'s cache. This is the same cache-stampede problem that plagues application caches, just one layer up the hierarchy — mitigating it fully requires the shield layer to also implement request coalescing/locking for in-flight cache-miss requests, not just fewer origin-facing paths.' },
