...
const theory: TheoryPoint[] = [
  ...
  },
  {
    ...
    points: [
      ...
      'DynamoDB: distributed KV + document; scales storage and throughput automatically by splitting across partitions — but each individual partition is still capped (~10 GB, ~3,000 RCU / ~1,000 WCU), so a high-cardinality partition key still matters.',
...
const quiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    answer: 2,
    explanation: 'ACID: Atomicity (all-or-nothing), Consistency (constraints maintained), Isolation (concurrent transactions don\'t interfere — though how STRICTLY depends on the isolation level actually in use, which is often weaker than full serializability by default), Durability (committed data survives crashes). Not related to availability or scale.',
  },
  { q: 'What properties does the ACID acronym describe in relational databases?', options: ['Availability, Consistency, Integrity, Durability', 'Atomicity, Consistency, Isolation, Durability', 'Atomicity, Concurrency, Integrity, Distribution', 'Availability, Correctness, Isolation, Data-retention'], answer: 1, explanation: 'ACID properties: Atomicity means all operations in a transaction succeed or all fail together with no partial commits. Consistency means a transaction can only bring the database from one valid state to another, respecting all constraints. Isolation means concurrent transactions do not interfere with each other — "execute as if they were serial" describes the strictest (Serializable) isolation level specifically, not a guarantee every database gives by default (PostgreSQL defaults to Read Committed; MySQL/InnoDB defaults to Repeatable Read). Durability means committed transactions survive system failures. These properties are what make relational databases suitable for financial systems, inventory management, and other scenarios where data integrity is critical.' },
