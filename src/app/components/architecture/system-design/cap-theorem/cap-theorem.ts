...
const qna: QnaItem[] = [
  ...
  { q: 'How does DynamoDB choose between consistency and availability?', a: 'DynamoDB offers two consistency models per read request. Eventual consistency (the default) reads from any replica and may return data that is up to a second stale if a write has not yet propagated. Strongly consistent reads always contact the partition leader and return the latest committed data but use twice the read capacity units. Writes in DynamoDB are always synchronously replicated to a quorum of replicas in the region before acknowledging success, so writes are consistent. The choice is per-operation: read-heavy workloads that can tolerate slight staleness should use eventual consistency for cost and latency savings; critical reads that need the latest value should use strong consistency.' },
  { q: 'What is the PACELC theorem and how does it extend CAP?', a: 'PACELC extends CAP by acknowledging that even when no partition occurs, distributed systems face a tradeoff between latency and consistency. CAP only addresses behavior during partitions. PACELC states: if there is a Partition, choose between Availability and Consistency; Else (no partition), choose between Latency and Consistency. Cassandra is PA/EL: during partitions it favors availability, and during normal operation it favors low latency over strict consistency. HBase is PC/EC: it favors consistency both during partitions and during normal operation, accepting higher latency to maintain strict consistency across replicas.' },
  { q: 'How do you implement eventual consistency with conflict resolution in a distributed system?', a: 'Eventual consistency means replicas may temporarily diverge but will converge. Conflict resolution strategies: Last Write Wins (LWW) uses timestamps or logical clocks to keep the most recent write, discarding concurrent conflicting writes. This is simple but can lose data. Vector clocks track causal relationships between writes and detect true conflicts without overwriting writes that are causally unrelated. Merge functions (like CRDTs) define commutative, associative merge operations so any order of applying updates produces the same result. CRDTs (Conflict-Free Replicated Data Types) are particularly powerful: counters, sets, and maps can be designed to merge without conflict, enabling strong eventual consistency.' },
];
