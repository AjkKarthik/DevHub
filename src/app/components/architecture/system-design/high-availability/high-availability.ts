...
const quickRef: QuickRefItem[] = [
  ...
  { name: 'Active-active',    type: 'keyword', desc: 'Multiple nodes all handle traffic. Load is distributed; failover is instant.' },
...
const theory: TheoryPoint[] = [
  ...
  },
  {
    ...
    points: [
      ...
      'No failover delay — remaining nodes absorb load immediately on peer failure.',
...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What does 99.99% availability (four nines) mean in terms of downtime?', options: ['The system is available for 99.99% of users at any given moment', 'The system can be down for at most approximately 52 minutes per year', 'The system must respond within 99.99ms for all requests', 'Four nines means the system has four redundant replicas'], answer: 1, explanation: 'Availability percentage translates to allowed downtime: 99.9% (three nines) = 8.7 hours/year, 99.99% (four nines) = 52 minutes/year, 99.999% (five nines) = 5.25 minutes/year. Each nine is approximately 10x more stringent than the previous. Achieving high availability requires eliminating single points of failure, automating failover, and designing for rapid recovery. The target availability drives decisions about redundancy, geographic distribution, and how much to invest in resilience mechanisms.' },
...
const qna: QnaItem[] = [
  ...
  { q: 'How do you design for zero-downtime deployments?', a: 'Zero-downtime deployment requires the new version to serve traffic while the old version is still running. Strategies: blue-green deployment maintains two identical environments; switch traffic from blue (old) to green (new) after health checks pass. Rolling deployment replaces instances one at a time, keeping the majority serving traffic. Canary deployment sends a small percentage of traffic to the new version first, monitoring for errors before full rollout. All strategies require the new version to be backward compatible with existing data formats and APIs during the transition window. Database migrations must be non-breaking: add columns but do not drop them until the old code is completely removed.' },
  { q: 'What is an error budget and how is it used to balance reliability and feature velocity?', a: 'An error budget is the amount of unreliability permitted by the SLO. If the SLO is 99.9% availability, the error budget is 0.1% of requests allowed to fail, or about 8.7 hours of downtime per year. The error budget is shared between planned outages (maintenance), incidents, and risk taken with deployments. If the error budget is consumed, engineering focus shifts to reliability work and risky deployments are paused. If the budget is healthy, the team can move faster and take more deployment risks. Error budgets align incentives between product teams (who want features fast) and SRE teams (who want reliability) by giving both a shared metric to optimize.' },
