...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the difference between a TCP and UDP protocol and when do you choose each?', options: ['TCP is for web traffic; UDP is for database traffic only', 'TCP provides reliable ordered delivery with error checking and retransmission; UDP is connectionless with no delivery guarantees but lower overhead', 'UDP is more secure than TCP because it has no connection state', 'TCP is faster than UDP for all use cases because of its optimization algorithms'], answer: 1, explanation: 'TCP establishes a connection via three-way handshake and guarantees that all data arrives in order and without corruption, retransmitting lost packets automatically. Adds latency and overhead. Use for: HTTP, databases, file transfer, anything where data integrity matters. UDP sends packets without establishing a connection and makes no delivery guarantees. Use for: real-time video streaming (a few dropped frames are acceptable), online gaming (low latency matters more than every packet arriving), DNS queries (fast and small; the client retries if no response), and VoIP.' },
  { q: 'What is a CDN and how does it use DNS to route users to the nearest edge node?', options: ['A CDN uses anycast routing at the IP layer so DNS is not involved', 'A CDN uses GeoDNS to return different IP addresses to users based on their location, routing them to the nearest edge node', 'DNS returns a fixed IP for CDN domains and edge routing happens at the application layer', 'CDNs use a central server that proxies to edge nodes based on the origin server location'], answer: 1, explanation: 'CDN domains are delegated to the CDN DNS infrastructure, which runs GeoDNS servers that return different IP addresses based on the geographic location of the DNS resolver. When a user in Tokyo resolves cdn.example.com, the CDN DNS returns the IP of a Tokyo edge node. When a user in New York resolves it, they get a New York edge IP. This DNS-based routing happens transparently to the application. Some CDNs also use anycast, where the same IP is advertised from multiple locations and BGP routing directs users to the nearest point.' },
];
...
const qna: QnaItem[] = [
  ...
  { q: 'What is TLS and how does it provide secure communication?', a: 'TLS (Transport Layer Security) provides encryption, authentication, and integrity for data in transit. The TLS handshake: the client sends supported cipher suites and TLS version. The server returns its certificate (containing its public key and identity signed by a CA). The client verifies the certificate against trusted CAs. A session key is established via key exchange (ECDHE provides forward secrecy). All subsequent communication is encrypted with symmetric encryption using the session key. TLS 1.3 reduced handshake overhead significantly. For internal service-to-service communication, use mutual TLS (mTLS) where both sides present and verify certificates, preventing any unauthorized service from connecting.' },
  { q: 'What is DNS TTL and how does it affect system design decisions?', a: 'DNS TTL (Time to Live) specifies how long DNS resolvers cache a record before re-querying the authoritative DNS server. High TTL (3600-86400 seconds): reduces DNS query load and speeds up resolution since clients use cached values. Disadvantage: DNS changes propagate slowly and cannot be quickly reversed. Low TTL (60-300 seconds): allows rapid DNS failover and blue-green deployments by quickly pointing traffic to new IP addresses. Disadvantage: higher DNS query volume and slightly higher resolution latency on cache misses. For production systems, use low TTLs on critical records that may need failover, and gradually lower the TTL before planned migrations to ensure caches expire before you switch.' },
  { q: 'What is a service mesh and how does it differ from a traditional API gateway?', a: 'An API gateway sits at the edge of the network and handles north-south traffic: requests coming from external clients to internal services. It handles authentication, rate limiting, and routing for inbound requests. A service mesh handles east-west traffic: internal service-to-service communication. It operates at the infrastructure layer via sidecar proxies injected into each service pod, providing mTLS between all services, load balancing, circuit breaking, and distributed tracing without requiring application code changes. The two are complementary: use an API gateway for external-facing concerns and a service mesh for internal microservice communication security and observability.' },
];
