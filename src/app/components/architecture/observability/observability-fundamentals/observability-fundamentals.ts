...
const quiz: QuizQuestion[] = [
  ...
  { q: 'A team has dashboards, alerts, and on-call rotations for known failure modes, but engineers still struggle to diagnose novel incidents. What is most likely missing?', options: ['More dashboards covering additional known metrics', 'A larger on-call rotation to spread the diagnostic workload', 'Genuine observability — the ability to ask arbitrary new questions about system state from its outputs, not just check pre-defined conditions', 'A faster alerting pipeline to reduce time-to-notification'], answer: 2, explanation: 'This scenario describes a team with strong MONITORING but weak OBSERVABILITY — they can detect that something known is wrong, but cannot investigate something they did not anticipate. More dashboards or faster alerts only help with failure modes someone already thought to instrument for. What is missing is the ability to explore arbitrary, ad-hoc questions against rich, high-cardinality data (metrics, logs, and traces together) — the defining capability that separates observability from monitoring.' },
