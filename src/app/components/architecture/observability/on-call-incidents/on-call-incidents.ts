...
const qna: QnaItem[] = [
  ...
  { q: 'How do you build and maintain effective incident runbooks?', a: 'Runbook design principles: trigger: which specific alert does this runbook address? First steps: what is the first diagnostic command to run? Decision tree: if metric X is high, check Y; if Y is also high, follow path A; if Y is normal, follow path B. Escalation: when to call in additional help and who to call. Mitigation steps: what actions reduce user impact immediately (rollback, scale, redirect traffic)? Verification: how do you confirm the mitigation worked? Resolution: what is the permanent fix? Runbook maintenance: run through the runbook during every incident. Note anything that was wrong or missing. Update the runbook within 24 hours of the incident. Treat stale runbooks as a reliability risk. Automation: for highly repetitive runbooks, automate the diagnostic and remediation steps. The engineer approves the action; the automation executes it. Common pitfall: runbooks that are too detailed but not actionable. Focus on: what to check, what to do, when to escalate. Not: explaining how the system works (that belongs in architecture documentation).' },
