...
const qna: QnaItem[] = [
  ...
  { q: 'Why is "archive dashboards not viewed in 90 days" a risky rule to apply uniformly, and what exception should it have?', a: 'A dashboard\'s view count reflects how often someone LOOKS at it during normal operations, not how valuable it is — a dashboard built specifically for diagnosing a rare, severe failure mode (a disaster-recovery runbook dashboard, a quarterly capacity-planning view) may legitimately go unopened for 90+ days precisely BECAUSE the problem it addresses is rare, not because it is useless. Blindly deleting by view-count alone risks removing exactly the dashboard you\'ll desperately need during the next incident of that rare type. A better rule tags dashboards by purpose (routine-monitoring vs incident-response vs periodic-review) and only applies the 90-day archival threshold to the routine-monitoring category, leaving incident-response and periodic-review dashboards exempt from view-count-based cleanup regardless of how rarely they\'re opened.' },
