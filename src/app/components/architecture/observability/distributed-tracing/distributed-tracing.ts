...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is the purpose of baggage propagation in distributed tracing?', options: ['Baggage carries compressed trace span data between services to reduce storage costs', 'Baggage is arbitrary key-value data that propagates with the trace context across all services in a request, allowing user IDs or feature flags set at the entry point to be available in every downstream service without passing them explicitly', 'Baggage stores the sampling decision to prevent it from changing between services', 'Baggage is a deprecated W3C specification replaced by the traceparent header'], answer: 1, explanation: 'W3C Baggage specification: key-value pairs that travel alongside the trace context headers. Set at the entry point (API gateway or first service): userId=123, tenantId=abc. All downstream services automatically receive these values IN APPLICATION CODE (readable via the OTel API), letting business logic access them without threading them through every function call. Use cases: make userId available for logging or business decisions in any downstream service. Pass feature flag values to all services for A/B test analysis. Security consideration: baggage is visible to all downstream services and should not contain sensitive data. Size limits: keep baggage small (a few hundred bytes maximum). Important: baggage values do NOT automatically become span attributes or show up as filterable/searchable in the tracing backend — that requires either manually calling span.setAttribute() with the baggage value, or configuring the separate, opt-in BaggageSpanProcessor component. Baggage on its own is propagated context data for application code, not automatic trace-backend searchability.' },
];
