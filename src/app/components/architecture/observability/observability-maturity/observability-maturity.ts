...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What are the typical levels of an observability maturity model?', options: ['Beginner, Intermediate, Advanced, Expert — based on team experience with tools', 'Level 0 no observability (stdout logs only), Level 1 basic infrastructure monitoring (host/uptime), Level 2 application observability (RED metrics, structured logs, basic tracing), Level 3 SLO-driven reliability (SLIs, SLOs, error budgets, burn rate alerting), Level 4 proactive observability (continuous profiling, chaos engineering, anomaly detection)', 'Bronze, Silver, Gold, Platinum — based on the number of monitored services', 'DORA metrics levels: low, medium, high, elite — applied to observability capability'], answer: 1, explanation: 'Observability maturity levels: Level 0 (None): stdout logs, no metrics, no tracing, no dashboards — failures discovered by user reports. Level 1 (Infrastructure visibility): host/process-level metrics, uptime checks, basic alerting — answers "is the host alive?" but not "are users affected?" Level 2 (Application visibility): RED metrics per service, structured logging with correlation IDs, distributed tracing — answers "which service is failing?" within minutes. Level 3 (SLO-driven): SLIs and SLOs defined and agreed with product, error budgets tracked, burn rate alerts replace threshold alerts — reliability decisions become data-driven. Level 4 (Proactive): continuous profiling, regularly-run chaos engineering, anomaly detection, all three pillars correlated — shifts from reacting to incidents to preventing them. Higher levels build on lower ones; skipping straight to Level 4 tooling without a solid Level 1-2 foundation rarely works.' },
