...
const qna: QnaItem[] = [
  ...
  { q: 'What is the relationship between SLOs, error budgets, and feature release velocity?', a: 'The SLO error budget is the primary mechanism for balancing reliability and feature velocity. When the error budget is healthy: product teams have data-driven confidence to release frequently. SREs support releases rather than blocking them. When the error budget is at risk: deployment frequency decreases automatically. SREs and product teams agree on reliability investments before resuming features. This creates a self-regulating system: frequent deployments that cause incidents slow down naturally. Systems that are stable can deploy frequently. Unlike policy-based release gates (which are negotiated and emotional), the error budget is objective. The product team never has to argue about whether a system is reliable enough — the data answers it. Reliability becomes a shared goal because everyone loses when the budget is exhausted (both the feature roadmap and reliability suffer).' },
