...
const quiz: QuizQuestion[] = [
  ...
  { q: 'What is a dead man switch alert and why is it critical for monitoring reliability?', options: ['An alert that fires when a server has not restarted in more than 30 days', 'An alert that must fire regularly to confirm the monitoring pipeline itself is working; if it stops firing it means the monitoring system has silently failed', 'An alert triggered automatically when an on-call engineer does not acknowledge a page within 15 minutes', 'An alert for IoT devices that stops reporting heartbeat signals to the monitoring system'], answer: 1, explanation: 'Dead man switch (watchdog alert): a synthetic alert that fires every few minutes confirming the monitoring pipeline is working end-to-end. If Prometheus, Alertmanager, or the notification channel fails, this alert stops firing. A secondary system (PagerDuty heartbeat, Cronitor) detects the silence and sends a meta-alert. Without a dead man switch, your monitoring can be silently broken while you believe everything is fine. This is the most critical alert because it is the only alert that confirms all other alerts can work.' },
  { q: 'What is alert inhibition in Alertmanager and when should it be configured?', options: ['A permanent suppression rule for alerts from decommissioned hosts', 'An Alertmanager mechanism where a higher-severity alert suppresses related lower-severity alerts to prevent flooding when a parent failure causes many downstream failures', 'An alert routing strategy that sends the same alert to multiple notification channels simultaneously', 'A silence applied manually to an alert rule during scheduled maintenance windows'], answer: 1, explanation: 'Alert inhibition: when a datacenter-down alert fires, it inhibits all service-down and component-error alerts that are consequences of that outage. Without inhibition, one root cause triggers 50 alerts all paging on-call engineers. With inhibition, only the root cause alert fires. Configured via inhibit_rules in Alertmanager: source match (the parent alert condition) and target match (alerts to suppress when the parent is active). Common use case: a host-down alert inhibits all service alerts from that host. A region-unavailable alert inhibits all service alerts in that region.' },
];
