...
const qna: QnaItem[] = [
  ...
  { q: 'How does eBPF-based network observability compare to traditional approaches like service mesh sidecars?', a: 'Service mesh sidecar (Istio Envoy): proxy intercepts all traffic at the network level. Provides L7 metrics (HTTP, gRPC). Adds latency overhead (2-5ms per hop) from the proxy forwarding. Requires sidecar injection into every pod. Resource overhead: CPU and memory per sidecar instance. eBPF-based networking (Cilium): kernel-level packet processing. Zero additional hop for traffic (kernel bypasses user-space proxy). Lower latency than sidecar-based approaches. Same L7 visibility as Envoy but without the overhead. Requires Cilium as the CNI plugin. Both approaches: automatic mTLS, network policy enforcement, traffic metrics. Comparison: Cilium with Hubble is significantly more efficient than Istio Envoy for high-throughput workloads. Istio has a more mature ecosystem and wider adoption. Service meshes and eBPF are converging — Cilium supports Istio integration with eBPF data plane replacing Envoy in some configurations.' },
