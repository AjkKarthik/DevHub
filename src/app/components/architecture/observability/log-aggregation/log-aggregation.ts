...
const qna: QnaItem[] = [
  {
    q: 'Can you run Loki and Elasticsearch side by side for different log categories in the same organization, rather than picking exactly one?',
    a: 'Yes, and many organizations do exactly this deliberately: route high-volume, structured application/service logs (the bulk of day-to-day operational log volume) to Loki for its cost efficiency and native Grafana integration, while routing security audit logs, compliance-sensitive logs, or logs needing rich full-text search and complex analytics to a separate Elasticsearch cluster where its stronger search and access-control features are worth the added operational cost. This avoids forcing a single one-size-fits-all choice onto genuinely different log use cases with different requirements — the tradeoff is operating two log pipelines instead of one, which adds its own tooling and on-call complexity that must be weighed against the benefit of using the right tool for each category.',
  },
  {
    q: 'Why is "convert repetitive logs to metrics" often the single highest-leverage cost-reduction technique compared to sampling or compression alone?',
    a: 'Sampling and compression still store SOME representation of every event category — sampling keeps a fraction of raw log lines, compression shrinks but doesn\'t eliminate storage per event. Converting a high-volume repetitive event (a cache hit, a successful health check) into a metric counter instead replaces potentially millions of individual log lines per day with a single incrementing number scraped periodically — the storage cost drops from "proportional to event count" to "proportional to scrape interval," a fundamentally different cost curve rather than just a smaller constant factor. This is why teams doing serious log cost optimization look first at "which of our highest-volume logs are actually just counting something" before reaching for sampling rates or compression settings — the metric conversion addresses the volume at its source rather than shrinking an already-large stream.',
  },
