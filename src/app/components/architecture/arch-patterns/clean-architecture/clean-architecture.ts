...
const useCase = new SendWelcomeEmailUseCase(
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the dependency rule in Clean Architecture?', options: ['Dependencies must flow outward from the UI to the database', 'Source code dependencies must always point inward: outer layers may depend on inner layers but inner layers must not depend on outer layers', 'All layers must depend on a shared infrastructure layer', 'Dependencies between layers are bidirectional to enable communication in both directions'], answer: 1, explanation: 'The dependency rule: source code dependencies can only point inward, toward the domain. Outer layers (frameworks, UI, database) know about inner layers, but inner layers know nothing about outer layers. The domain entities have no knowledge of the database, HTTP, or frameworks. This means frameworks are interchangeable and the core business logic can be tested without a database or web server. Dependency inversion is the mechanism: inner layers define interfaces; outer layers provide implementations.' },
  ...
  qna: QnaItem[] = [
    ...
    { q: 'When is Clean Architecture overkill for a project?', a: 'Clean Architecture adds upfront complexity: multiple layers, interface definitions, dependency injection setup, and more classes per feature. For small projects or MVPs where requirements change rapidly, this overhead slows initial development without enough benefit. CRUD applications with minimal business logic gain little from Clean Architecture because there are few complex business rules to protect from framework coupling. If the team is small and the deployment timeline is short, simpler architecture (a standard MVC pattern or a feature-sliced structure) may be more pragmatic. Apply Clean Architecture when the domain has significant business logic complexity, when multiple delivery mechanisms are needed, or when long-term maintainability over years matters more than rapid initial delivery.' },
  ];
