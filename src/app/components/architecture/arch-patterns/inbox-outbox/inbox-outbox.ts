...
console.log('Published rows:', outboxTable.filter(r => r.publishedAt).length);`,
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How does Debezium enable the outbox pattern without polling?', a: 'Naive outbox relay implementations poll the outbox table with a SELECT WHERE published = false query on a schedule. This adds database load and introduces polling delay. Debezium is a change data capture (CDC) tool that connects to the database replication stream (WAL in PostgreSQL, binlog in MySQL) and captures row-level changes in real time. By connecting Debezium to the outbox table, every INSERT into the outbox is captured from the replication stream and forwarded to Kafka within milliseconds, without polling. This provides low-latency event publishing without burdening the database with polling queries. Debezium handles source offsets and guarantees at-least-once event capture from the replication stream.' },
