...
getMobileFeed().then(r => {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is a common anti-pattern where a BFF starts to resemble the general-purpose API it was meant to replace?', options: ['Deploying the BFF on the same infrastructure as backend services', 'A "shared" BFF gradually accumulating support for multiple unrelated client types, reintroducing the same over/under-fetching compromise BFF was created to avoid', 'Using HTTPS for BFF-to-client communication', 'Writing the BFF in the same language as the frontend'], answer: 1, explanation: 'A common failure mode: teams start with one BFF per client type, but under time pressure a new client type gets bolted onto an EXISTING BFF instead of getting its own — over time that "shared" BFF re-accumulates conditional logic to serve multiple divergent client needs, recreating the exact over/under-fetching compromise that motivated BFF in the first place. Keeping BFF boundaries strictly aligned to one client type, even when it feels like duplicated effort short-term, is what preserves the pattern\'s benefit long-term.' },
  ...
  qna: QnaItem[] = [
    {
      q: 'How do you version a BFF\'s API contract without breaking the frontend that consumes it?',
      a: 'Since a BFF is owned by (and typically deployed in lockstep with) the frontend team that consumes it, the most common approach is coordinated deployment — the BFF and frontend release together as one unit, so there is no need for the BFF to support multiple concurrent API versions the way a general-purpose public API would. When a BFF must support multiple frontend versions simultaneously (e.g. a mobile app with users who haven\'t updated), standard API versioning approaches apply (URL versioning, header-based versioning), but this is treated as the exception rather than the default, since it works against BFF\'s core benefit of tight, low-ceremony frontend-backend coordination.',
    },
    {
      q: 'What is the downside of the "GraphQL as the BFF layer" combined approach?',
      a: 'Using GraphQL as the BFF gives up some of the deliberate simplicity that makes plain-REST BFFs easy to reason about: a GraphQL resolver layer can silently accumulate N+1 query problems (each field resolver independently calling a backend service) unless carefully batched with tools like DataLoader, and a single overly-complex or malicious query can fan out into a large number of expensive downstream service calls unless query cost analysis/depth limiting is enforced — a plain REST BFF endpoint has a fixed, predictable aggregation cost per call, while a GraphQL BFF\'s cost varies per query shape, requiring more operational safeguards to keep predictable.',
    },
    {
    ...
    { q: 'What skill-set challenge does frontend-team BFF ownership create, and how do teams typically address it?', a: 'A frontend/mobile team asked to own a server-side BFF now needs backend competencies — API design, service-to-service authentication, observability, deployment pipelines for a backend service — that may not be the team\'s core strength, creating a skills gap the team-ownership model assumes away. Common mitigations: platform/DevEx teams provide a standardized BFF scaffold or template (auth, logging, tracing pre-wired) so frontend teams only write aggregation/shaping logic, not backend plumbing from scratch, and backend-leaning engineers sometimes embed within frontend teams specifically to own the BFF\'s operational concerns while the frontend team owns its business logic and shape.' },
