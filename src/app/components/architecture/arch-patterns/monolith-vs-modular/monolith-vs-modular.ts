...
registerOrdersModule(appServices, catalogService);`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'When should you choose a modular monolith over microservices?', options: ['Always; microservices are never the right choice', 'When the team is small, the domain is not yet fully understood, and the operational overhead of microservices would slow down development without providing sufficient benefit', 'When the application requires less than 1000 requests per second', 'Only when using programming languages that do not support containerization'], answer: 1, explanation: 'Choose a modular monolith when: the team is smaller than roughly 10-15 engineers (matching this page\'s own rule of thumb for when microservices start to pay off) and the overhead of separate deployment pipelines per service outweighs the autonomy benefit. The domain is new or uncertain, and premature service decomposition along the wrong boundaries creates coupling that is expensive to fix. The application does not have extremely divergent scaling needs per component. You want the option to extract services later from a well-structured foundation. Microservices shine at scale: large teams, well-understood domains, and components with genuinely different scaling requirements.' },
  ];
