...
const contextMapRelationships = [
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is a bounded context in Domain-Driven Design?', options: ['A database schema that boundaries which tables belong to each service', 'An explicit boundary within which a domain model is defined and applicable, with its own ubiquitous language that may differ from other contexts', 'A microservice with a single responsibility and well-defined API', 'A security boundary that limits which users can access domain data'], answer: 1, explanation: 'A bounded context defines the scope within which a particular domain model and its ubiquitous language apply consistently. The same concept may have different meanings in different bounded contexts: a product in the catalog context has name, description, and images; the same product in the fulfillment context has weight, dimensions, and warehouse location. Bounded contexts make these differences explicit rather than creating a single overloaded model that tries to serve all contexts. Each context has its own codebase, schema, and team.' },
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How does a bounded context relate to a microservice?', a: 'A bounded context is a domain modeling concept; a microservice is an architectural deployment unit. They are related but not identical. A single bounded context may be implemented as one microservice (common), multiple microservices (a large context split for independent deployability), or even a module in a modular monolith. One microservice should not span multiple bounded contexts, as that creates a monolith that mixes two domains. The boundary of a bounded context defines the natural boundary for a microservice: each bounded context has its own data store, its own model, and its own team. Starting a microservices migration by identifying bounded contexts first prevents creating chatty micro-monoliths that are just a distributed monolith.' },
  ];
