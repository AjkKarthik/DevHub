...
console.log('Rebuilt count:', rebuilt.count); // 1`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between a "single database, two models" CQRS implementation and a "separate read/write stores" implementation?', options: ['They are the same thing described differently', 'Single-database CQRS uses one physical database with separate write-optimized and read-optimized query/entity models mapped over it (a low-complexity starting point); separate-stores CQRS uses genuinely different databases (e.g. Postgres for writes, Elasticsearch for reads) synced asynchronously, adding eventual consistency and infrastructure complexity', 'Single-database CQRS does not use commands at all', 'Separate-stores CQRS is required for CQRS to count as CQRS'], answer: 1, explanation: 'CQRS is fundamentally about separating models, not necessarily separating physical storage — a lightweight CQRS implementation can query the same relational database through two different model shapes (a rich write-side entity model and a flat read-side DTO/view model) with zero eventual-consistency concerns, since both read the same up-to-date data. Only when read and write scale/shape needs diverge enough to justify a genuinely separate read store (with an async projection/sync mechanism keeping it updated) does CQRS introduce eventual consistency — many teams successfully use "logical" CQRS without ever needing separate physical stores.' },
