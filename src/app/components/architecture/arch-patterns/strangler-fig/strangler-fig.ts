...
// ['Widget Pro', 'Basic Widget'] (sorted by score: 0.9, 0.6)`,
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you handle data migration during a Strangler Fig migration?', a: 'Data is usually the hardest part of a Strangler Fig migration. Strategies: dark launch - the new service is deployed and receives a copy of traffic in shadow mode, building a shadow database that can be compared to the legacy database for correctness. Sync-first migration: migrate the data to a new schema first, then switch the code. Dual write: for a period, both the legacy and new service write to their respective databases while a background sync job keeps them consistent. Eventual cutover: once confidence is high, route all writes to the new service and stop syncing. Read from both for a period to handle any records missed during migration. The key is to avoid having a big-bang data migration that is hard to roll back.' },
