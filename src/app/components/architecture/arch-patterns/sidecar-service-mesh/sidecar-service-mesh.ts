...
const trafficSplit = {
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is mTLS (mutual TLS) and how does a service mesh provide it automatically?', a: 'Standard TLS authenticates only the server to the client. mTLS authenticates both sides: the server presents a certificate to the client, and the client presents a certificate to the server. This ensures not only that the client is talking to the right server, but also that only authorized services can connect. In a service mesh, the control plane (istiod in Istio) acts as a CA and issues short-lived certificates to each sidecar proxy. The sidecar automatically handles certificate rotation, TLS handshake, and peer authentication for every service-to-service call. Application code connects to localhost and the sidecar handles encrypted mTLS communication to other service sidecars.' },
