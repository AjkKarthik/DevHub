...
try { order.cancelLine('l2'); } // throws: last line
  ...
  qna: QnaItem[] = [
    ...
    { q: 'What is the "reference by ID, not by object" rule between aggregates, and why does it matter?', a: 'An aggregate should reference OTHER aggregates only by their identifier (e.g. customerId: string), never by holding a direct in-memory object reference to the other aggregate root. This matters because a direct object reference tempts developers to reach across the boundary and modify the other aggregate\'s state as part of the same operation, silently violating the invariant that only one aggregate is modified per transaction. ID-only references force any cross-aggregate interaction to go through an explicit, intentional step (loading the other aggregate separately, or firing a domain event), keeping transactional boundaries honest and making the "one aggregate, one transaction" rule structurally enforced by the code rather than just a convention developers might forget.' },
