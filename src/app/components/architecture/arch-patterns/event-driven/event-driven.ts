...
placeOrder('cust-1', 49.99).then(id => console.log('Order placed:', id));`,
  ...
  qna: QnaItem[] = [
    ...
    { q: 'How do you handle event schema evolution in an event-driven system?', a: 'Events in a durable event log (Kafka, event store) cannot be changed after publication because existing consumers may need to replay them. Strategies: backward compatibility - add new optional fields with defaults; consumers that do not know about the new field ignore it and old consumers continue to work. Forward compatibility - remove fields by keeping them in the schema but ignoring them in producers; consumers that read the field must handle its absence. Versioning: include an event version field and use upcasters in consumers to transform old versions to the current schema during processing. Schema Registry (Confluent, AWS Glue) enforces compatibility rules before publishing and stores all schema versions. Avro and Protobuf provide better evolution support than JSON because they define explicit schemas with compatibility semantics.' },
