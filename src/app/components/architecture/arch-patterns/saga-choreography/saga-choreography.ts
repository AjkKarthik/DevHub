...
await bookTrip(true);  // hotel fails → nothing to compensate`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What does compensating transaction mean in the context of Sagas?', options: ['An additional transaction added to improve the performance of the original', 'A transaction that reverses or undoes a previously committed step in a saga to restore the system to its pre-saga state when a later step fails', 'A transaction that compensates for the delay caused by asynchronous processing', 'An additional validation step that runs before the main saga transaction to ensure correctness'], answer: 1, explanation: 'Since each saga step commits immediately (individual local transactions), there is no rollback mechanism for earlier committed steps. Compensating transactions are explicit business operations that reverse the effect of a completed step. Examples: if order placement succeeded and inventory reservation succeeded but payment fails, the saga must release the reserved inventory (compensating for the reservation step) and cancel the order. Compensating transactions must be explicitly designed and implemented as domain operations; they are not automatic rollbacks. Not all operations are compensatable: sending an email cannot be unsent.' },
  ];
