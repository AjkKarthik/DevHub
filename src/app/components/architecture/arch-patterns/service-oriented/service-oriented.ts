...
getOrder('ORD-123').then(console.log);`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'How does Service-Oriented Architecture (SOA) differ from microservices?', options: ['SOA and microservices are identical concepts with different branding', 'SOA uses heavyweight enterprise middleware like ESB and SOAP; microservices use lightweight protocols, are smaller in scope, and avoid centralized integration infrastructure', 'SOA is for on-premise deployments; microservices are only for cloud deployments', 'Microservices are a subset of SOA that restricts service size to single functions'], answer: 1, explanation: 'SOA emerged in the 2000s using enterprise standards: SOAP, WSDL, and ESB (Enterprise Service Bus). Services were coarser-grained and the ESB handled routing, transformation, and orchestration centrally. The ESB became a bottleneck and single point of failure. Microservices (2010s+) use lightweight REST or gRPC, avoid centralized middleware, are much smaller in scope, and use dumb pipes and smart endpoints. Microservices evolved from SOA to address its drawbacks: ESB complexity, heavyweight protocols, and coarse service granularity.' },
