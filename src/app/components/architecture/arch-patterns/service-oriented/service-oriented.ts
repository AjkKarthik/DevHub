  ...
  theory: TheoryPoint[] = [
    ...
    },
    {
      ...
      points: [
        ...
        '"Microservices = SOA done right" is a widely-repeated industry characterization (not a single attributable quote from one named author) — it captures that microservices kept SOA\'s service-oriented integration ideas while rejecting the centralized ESB that made classic SOA hard to change.',
      ],
    },
    {
...
getOrder('ORD-123').then(console.log);`,
  ...
  qna: QnaItem[] = [
    ...
    {
      q: 'What does "smart endpoints, dumb pipes" mean?',
      a: 'The SOA/ESB model is the opposite: smart middleware (ESB) handles routing, transformation, retry — the "pipe" carries logic. Microservices invert this: dumb pipes (HTTP, Kafka, carrying messages only) and smart endpoints (services own their own logic). The inversion removes the ESB as a single point of failure and central coupling axis.',
    },
