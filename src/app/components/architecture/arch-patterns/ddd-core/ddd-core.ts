...
try { account.debit(new Money(200, 'USD')); } // throws: Insufficient funds
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between an Entity and a Value Object in DDD?', options: ['Entities are mutable; Value Objects are always read-only', 'Entities have identity that persists through state changes; Value Objects are defined entirely by their attributes and have no identity', 'Entities belong to the domain model; Value Objects belong to the infrastructure layer', 'Value Objects can only contain primitive types like string and number'], answer: 1, explanation: 'Entity: defined by its identity, not its attributes. Two entities with the same attributes but different IDs are different objects. Customer John Smith at ID 123 is distinct from Customer John Smith at ID 456. State changes do not change identity. Value Object: defined entirely by its attributes; there is no identity. Two Money values of 10 USD are interchangeable. Value Objects are immutable: to change an address, replace it with a new Address value object. Value Objects should be used where possible because they simplify reasoning: no identity tracking, no lifecycle management, freely copyable.' },
