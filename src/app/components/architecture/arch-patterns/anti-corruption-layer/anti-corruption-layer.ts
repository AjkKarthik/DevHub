...
// { city: 'Oslo', temperatureCelsius: 0, condition: 'unknown' }`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is a sign an ACL has been implemented incorrectly and is "leaking" instead of properly isolating your domain?', options: ['The ACL has more than 100 lines of code', 'Your domain entities or use cases contain conditional logic or field names that reference the external system\'s specific quirks (e.g. checking a legacy status code value directly, or naming a domain field after the external system\'s column name)', 'The ACL is implemented as a separate class rather than a function', 'The ACL is tested with unit tests instead of integration tests'], answer: 1, explanation: 'A correctly-implemented ACL fully absorbs the external system\'s vocabulary, quirks, and data shapes at the translation boundary, presenting your domain with clean, domain-native concepts — if your domain code still needs to know that the legacy system uses status code "3" for cancelled orders, or names a field CUST_NM instead of customerName, the external system\'s concepts have leaked past the ACL rather than being translated away by it, defeating the pattern\'s entire purpose.' },
  ...
  qna: QnaItem[] = [
    {
      q: 'Should an ACL make outbound calls to the external system itself, or only translate data that already arrived through another integration layer?',
      a: 'A full ACL implementation typically owns the entire round-trip: it exposes a domain-native interface to the rest of your application (e.g. PaymentGateway.charge(Money)), and internally is responsible for both calling the external system (making the Stripe API request) AND translating the response back into domain terms — this way the rest of the domain never needs to know an HTTP call, a specific SDK, or the external API\'s error codes exist at all. Splitting "make the call" and "translate the result" into separate layers is also valid, but risks the translation-only layer becoming a thin pass-through that domain code bypasses under time pressure, reintroducing the exact leak the ACL exists to prevent.',
    },
    {
