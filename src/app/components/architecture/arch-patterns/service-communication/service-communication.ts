...
messageBus.subscribe('orders.checked-out', async (e: OrderCheckedOutEvent) => {
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'Why does changing a gRPC proto field\'s number (not just its name) break wire compatibility, even though field numbers are usually invisible in generated client code?', options: ['Field numbers have no effect on the wire format at all', 'Protobuf\'s binary encoding identifies each field by its NUMBER, not its name — the field name exists only for generated code readability, so reusing or renumbering a field number causes old and new services to misinterpret each other\'s binary payloads as a completely different field', 'Field numbers only matter for gRPC streaming, not unary calls', 'Renumbering fields is always safe as long as the type stays the same'], answer: 1, explanation: 'A .proto message field like string customer_id = 3; is encoded on the wire using tag number 3, not the name customer_id — the name is purely a code-generation convenience. If a later schema change reuses number 3 for a different field (even accidentally), an old client sending number-3 data and a new server expecting number-3 to mean something else will silently misinterpret the bytes rather than erroring, which is why Protobuf best practice treats field numbers as permanent and never reuses them, even after a field is deprecated and removed.' },
    { q: 'What debugging difficulty does asynchronous messaging introduce that synchronous HTTP calls largely avoid, even when both eventually succeed?', options: ['Asynchronous messaging has no debugging difficulty beyond synchronous calls', 'Tracing a single business operation across multiple decoupled consumers reacting to an event at different, unpredictable times makes it hard to reconstruct a linear causal chain — unlike a synchronous call stack, which shows the full request path in one place', 'Async messages cannot be logged at all', 'Debugging is only harder for messages larger than 1MB'], answer: 1, explanation: 'A synchronous HTTP call chain gives you a linear, in-order call stack (or a trace with clear parent-child spans) showing exactly which service called which, in what order, all within roughly the same time window. An event published once might be consumed by three services at three different times (immediately, after a 5-second queue delay, after a 2-hour batch job) — reconstructing "what caused what" requires deliberate distributed tracing with correlation IDs propagated through every hop, since there is no natural single call stack tying the whole business operation together the way a synchronous chain provides for free.' },
  ...
  qna: QnaItem[] = [
    ...
    { q: 'If you add a new consumer group to an existing Kafka topic today, will it receive events published BEFORE it was created?', a: 'Yes, up to the topic\'s retention window — unlike a traditional pub/sub topic where a new subscriber only receives events published after it subscribes, a new Kafka consumer group starts reading from whatever offset it chooses (earliest or latest), and "earliest" replays every retained event on the topic from the beginning, even ones published long before the consumer group existed. This is a key architectural difference from classic message-broker pub/sub (like Service Bus topics): Kafka\'s log-based retention means a brand-new consumer can retroactively process historical events, enabling patterns like rebuilding a read model from scratch by replaying the full event history.' },
