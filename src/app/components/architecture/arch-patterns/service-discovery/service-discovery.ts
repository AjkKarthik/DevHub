...
console.log(registry.getHealthy('catalog').length); // 1`,
  ...
  quiz: QuizQuestion[] = [
    ...
    { q: 'What is the difference between client-side and server-side service discovery?', options: ['Client-side discovery is for internal services; server-side is for external clients', 'In client-side discovery the calling service queries the registry and does its own load balancing; in server-side discovery the client calls a load balancer or gateway that queries the registry and routes the request', 'Client-side discovery uses DNS; server-side discovery uses a service registry API', 'Server-side discovery requires the Eureka registry; client-side uses Consul'], answer: 1, explanation: 'Client-side discovery: the client library queries the service registry (Consul, Eureka) directly and applies client-side load balancing (round-robin, least connections) to select an instance. The client knows about multiple instances and can do sophisticated routing. Server-side discovery: the client sends the request to a load balancer or API gateway which queries the registry and routes the request. The client is unaware of the underlying instances. Server-side is simpler for the client but requires a highly available load balancer. Kubernetes DNS-based discovery is server-side: the client calls a Service DNS name and kube-proxy handles routing.' },
